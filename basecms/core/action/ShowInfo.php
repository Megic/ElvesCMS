...
$newstemp_r=$Elves->fetch1("select tempid,temptext,showdate from ".GetTemptb("melvenewstemp")." where tempid='$r[newstempid]'");

//替换模板变量
function DtGetHtml($add,$newstemp_r,$mid,$tbname,$line,$page_line,$start,$page,$search){
	global $public_r,$class_r,$class_zr,$class_tr,$fun_r,$Elves,$dbtbpre,$emod_r,$level_r;
	//更新点击
	$Elves->query("update {$dbtbpre}elve_".$tbname." set onclick=onclick+1 where id='$add[id]' limit 1");
	...
	if($pf&&strstr($add[$pf],$expage))//有分页
	{
		...
		if($page)
		{
			$ti_r=explode('[/!--Elvesnews.page--]',$n_r[$page]);
			if(count($ti_r)>=2)
			{
				$ptitle=$ti_r[0];
				$add[$pf]=$ti_r[1];
		...
		if(strstr($newstemptext,'[!--title.select--]'))
		{
			for($j=0;$j<$thispagenum;$j++)
			{
				...
				{
					$ti_r=explode('[/!--Elvesnews.page--]',$n_r[$j]);
	...
	{
		$newstempstr=str_replace('[!--linkusername--]',"<a href='".$public_r[newsurl]."core/space/?userid=".$add[userid]."' target=_blank>".$add[username]."</a>",$newstempstr);
	}
	else
	{
	...
	{
		$next_r=$Elves->fetch1("select isurl,titleurl,classid,id,title from {$dbtbpre}elve_".$class_r[$add[classid]][tbname]." where id>$add[id] and classid='$add[classid]' order by id limit 1");
		if(empty($next_r[id]))
		{
	...
	{
		$next_r=$Elves->fetch1("select isurl,titleurl,classid,id,title from {$dbtbpre}elve_".$class_r[$add[classid]][tbname]." where id<$add[id] and classid='$add[classid]' order by id desc limit 1");
		if(empty($next_r[id]))
		{
			$infonext="<a href='".$classurl."'>".$fun_r['HaveNoNextLink']."</a>";
		}
