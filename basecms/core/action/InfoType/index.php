...
$ttr=$Elves->fetch1("select typeid,tname,mid,yhid,tnum,listtempid,tpath,ttype,maxnum,reorder,tid,tbname,timg,intro,pagekey,listdt,repagenum from {$dbtbpre}melveinfotype where typeid='$ttid'");
$tbname=$ttr['tbname'];
...
$pagekey=$ttr['pagekey'];
$pagedes=$ttr['intro'];
$classimg=$ttr['timg']?$ttr['timg']:$public_r[newsurl].'core/data/images/notimg.gif';
...
if($totalnum<1)
{
	$totalquery="select count(*) as total from {$dbtbpre}elve_".$tbname." where ".$yhadd.$add;
...
$search.='&totalnum='.$num;
$query="select ".ReturnSqlListF($mid)." from {$dbtbpre}elve_".$tbname." where ".$yhadd.$add;
