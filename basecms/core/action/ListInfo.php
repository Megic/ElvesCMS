...
$classimg=$public_r[newsurl].'core/data/images/notimg.gif';
...
if($classid)
{
	...
	else
	{
		...
		$cr=$Elves->fetch1("select classpagekey,intro,classimg,cgroupid from {$dbtbpre}melveclass where classid='$trueclassid'");
		//权限
		if($cr['cgroupid'])
		{
		...
		$pagedes=$cr['intro'];
		$classimg=$cr['classimg']?$cr['classimg']:$public_r[newsurl].'core/data/images/notimg.gif';
...
$tempr=$Elves->fetch1("select tempid,temptext,subnews,listvar,rownum,showdate,modid,subtitle,docode from ".GetTemptb("melvelisttemp")." where tempid='$tempid'");
...
if($totalnum<1)
{
	$totalquery="select count(*) as total from {$dbtbpre}elve_".$tbname.ReturnYhAndSql($yhadd,$add,1);
...
$query="select ".ReturnSqlListF($mid)." from {$dbtbpre}elve_".$tbname.ReturnYhAndSql($yhadd,$add,1);
...
$listtemp=Class_ReplaceSvars($listtemp,$url,$pageclassid,$pagetitle,$pagekey,$pagedes,$classimg,$addr,$pageelve);
