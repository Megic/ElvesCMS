...
$search_r=$Elves->fetch1("select searchid,keyboard,result_num,orderby,myorder,tbname,tempid,andsql,trueclassid from {$dbtbpre}melvesearch where searchid='$searchid'");
...
$query="select * from {$dbtbpre}elve_".$search_r[tbname].($add?' where '.substr($add,5):'');
...
if($search_r['tempid'])
{
	$tempr=$Elves->fetch1("select temptext,subnews,listvar,rownum,showdate,modid,subtitle,docode from ".GetTemptb("melvesearchtemp")." where tempid='".$search_r['tempid']."' limit 1");
}
elseif(empty($class_r[$search_r[trueclassid]][searchtempid]))
{
	$tempr=$Elves->fetch1("select temptext,subnews,listvar,rownum,showdate,modid,subtitle,docode from ".GetTemptb("melvesearchtemp")." where isdefault=1 limit 1");
}
else
{
	$tempr=$Elves->fetch1("select temptext,subnews,listvar,rownum,showdate,modid,subtitle,docode from ".GetTemptb("melvesearchtemp")." where tempid='".$class_r[$search_r[trueclassid]][searchtempid]."' limit 1");
}
$have_class=1;
//替换公共模板变量
...
$listtemp=str_replace("[!--elve.num--]",$num,$listtemp);
