...
//返回
$getfrom=DoingReturnUrl($getfrom,$_POST['elvefrom']);
//搜索用户组
...
{
	$tbnamenum=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvetable where tbname='$s_tbname' limit 1");
	if(!$tbnamenum)
	{
		printerror("SearchNotRecord",$getfrom,1);
	}
	$tbname=$s_tbname;
	//模型id
	$thestemp_r=$Elves->fetch1("select modid from ".GetTemptb("melvesearchtemp")." where tempid='$s_tempid'");
...
if(empty($class_r[$trueclassid][searchtempid]))
{
	if(empty($modid))
	{
		$tempr=$Elves->fetch1("select modid from ".GetTemptb("melvesearchtemp")." where isdefault=1 limit 1");
	}
	else
	{
		$tempr[modid]=$modid;
...
$mr=$Elves->fetch1("select searchvar,tbname from {$dbtbpre}melvemod where mid='$tempr[modid]'");
...
$query="select count(*) as total from {$dbtbpre}elve_".$tbname.($allwhere?' where '.substr($allwhere,5):'');
$search_r=$Elves->fetch1("select searchid from {$dbtbpre}melvesearch where checkpass='$checkpass' limit 1");
...
if($searchid)
{
    $search_num=$Elves->gettotal($query);
	$sql=$Elves->query("update {$dbtbpre}melvesearch set searchtime='$searchtime',result_num='$search_num',onclick=onclick+1,orderby='$orderby',myorder='$myorder',tempid='$s_tempid' where searchid='$searchid'");
	if(empty($search_num))
	{
		$searchid=0;
	}
