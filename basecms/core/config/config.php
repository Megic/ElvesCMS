...
$elve_config['db']['dbport']='3306';	//端口，不填为按默认
$elve_config['db']['dbusername']='root';	//数据库用户名
$elve_config['db']['dbpassword']='';	//数据库密码
$elve_config['db']['dbname']='mask';	//数据库名
...
//页面编码设置
$elve_config['sets']['pagechar']='utf-8';	//安装ElvesCMS的编码版本
$elve_config['sets']['setpagechar']=1;	//页面默认字符集,0=关闭 1=开启
$elve_config['sets']['elang']='gb';	//语言包

//后台相关配置
$elve_config['esafe']['openonlinesetting']=3;	//开启后台在线配置参数(0为关闭,1为开启防火墙配置,2为开启安全配置,3为全开启)
$elve_config['esafe']['openeditdttemp']=1;	//开启后台在线修改动态模板(0为关闭,1为开启)

//易通行系统配置
$elve_config['epassport']['open']=0;	//是否开启易通行系统(1为开启，0为关闭)

//其它配置
$elve_config['sets']['txtpath']=elve_PATH.'d/txt/';	//文本型数据存放目录
$elve_config['sets']['saveurlimgclearurl']=0;	//远程保存图片自动去除图片的链接(0为保留,1为去除)
$elve_config['sets']['deftempid']=0;	//默认模板组ID



//-------ElvesCMS.Seting.member-------

//会员系统相关配置
$elve_config['member']['tablename']="{$dbtbpre}melvemember";	//会员表
$user_tablename=$elve_config['member']['tablename'];	//会员表
$elve_config['member']['changeregisterurl']="ChangeRegister.php";    //多会员组中转注册地址
$elve_config['member']['registerurl']="";							//会员注册地址
$elve_config['member']['loginurl']="";								//会员登录地址
$elve_config['member']['quiturl']="";								//会员退出地址
$elve_config['member']['chmember']=0;//是否使用原版会员表信息,0为原版,1为非原版
...
$elve_config['member']['saltnum']=6;//SALT随机码字符数
$elve_config['member']['utfdata']=0;//数据是否是GBK编码,0为正常数据,1为GBK编码

$elve_config['memberf']['userid']='userid';//用户ID字段
$elve_config['memberf']['username']='username';//用户名字段
$elve_config['memberf']['password']='password';//密码字段
$elve_config['memberf']['rnd']='rnd';//随机密码字段
$elve_config['memberf']['email']='email';//邮箱字段
$elve_config['memberf']['registertime']='registertime';//注册时间字段
$elve_config['memberf']['groupid']='groupid';//会员组字段
$elve_config['memberf']['userfen']='userfen';//积分字段
$elve_config['memberf']['userdate']='userdate';//有效期字段
$elve_config['memberf']['money']='money';//帐户余额字段
$elve_config['memberf']['zgroupid']='zgroupid';//到期转向会员组字段
$elve_config['memberf']['havemsg']='havemsg';//提示短消息字段
$elve_config['memberf']['checked']='checked';//审核状态字段
$elve_config['memberf']['salt']='salt';//SALT加密字段
$elve_config['memberf']['userkey']='userkey';//用户密钥字段
...
//-------ElvesCMS.Seting.area-------

//后台安全设置
$elve_config['esafe']['loginauth']='';	//登录认证码,如果设置登录需要输入此认证码才能通过
$elve_config['esafe']['ecookiernd']='yBKgUzzg4tVdhBwAqVanJfVLkBzqMy';	//后台登录COOKIE认证码(填写10~50个任意字符，最好多种字符组合)
$elve_config['esafe']['ckhloginfile']=0;	//后台是否启用文件验证,0为验证,1为不验证
$elve_config['esafe']['ckhloginip']=0;	//后台是否验证登录IP,0为不验证,1为验证
$elve_config['esafe']['ckhsession']=0;	//后台是否启用SESSION验证,0为不验证,1为验证
$elve_config['esafe']['theloginlog']=0;	//是否记录登陆日志(0为记录,1为不记录)
$elve_config['esafe']['thedolog']=0;		//是否记录操作日志(0为记录,1为不记录)
$elve_config['esafe']['ckfromurl']=0;	//是否启用来源地址验证,0为不验证,1为全部验证,2为后台验证,3为前台验证

//COOKIE设置
$elve_config['cks']['ckdomain']='';		//cookie作用域
$elve_config['cks']['ckpath']='/';		//cookie作用路径
$elve_config['cks']['ckvarpre']='megic';		//前台cookie变量前缀
$elve_config['cks']['ckadminvarpre']='megicadmin';		//后台cookie变量前缀
$elve_config['cks']['ckrnd']='9tRbUg6R852vHsrtAsxir8uuQzbBvb';	//COOKIE验证随机码(填写10~50个任意字符，最好多种字符组合)
$elve_config['cks']['ckrndtwo']='iHgPWDknhgqgzRH7Xjjbd9deU2Pq7r';	//COOKIE验证随机码2(填写10~50个任意字符，最好多种字符组合)

//网站防火墙配置
$elve_config['fw']['eopen']=0;	//开启防火墙(0为关闭,1为开启)
$elve_config['fw']['epass']='';	//防火墙加密密钥(填写10~50个任意字符，最好多种字符组合)
$elve_config['fw']['adminloginurl']='';	//允许后台登陆的域名,设置后必须通过这个域名才能访问后台
$elve_config['fw']['adminhour']='';	//允许登陆后台的时间：0~23小时，多个时间点用半角逗号格开
$elve_config['fw']['adminweek']='';	//允许登陆后台的星期：星期0~6，多个星期用半角逗号格开
$elve_config['fw']['adminckpassvar']='';	//后台预登陆验证变量名
$elve_config['fw']['adminckpassval']='';	//后台预登陆认证码
$elve_config['fw']['cleargettext']='';	//屏蔽提交敏感字符，多个用半角逗号格开

//-------ElvesCMS.Seting.area-------


//文件类型
$elve_config['sets']['tranpicturetype']=',.jpg,.gif,.png,.bmp,.jpeg,';	//图片
$elve_config['sets']['tranflashtype']=',.swf,.flv,.dcr,';	//FLASH
$elve_config['sets']['mediaplayertype']=',.wmv,.asf,.wma,.mp3,.asx,.mid,.midi,';	//mediaplayer
$elve_config['sets']['realplayertype']=',.rm,.ra,.rmvb,.mp4,.mov,.avi,.wav,.ram,.mpg,.mpeg,';	//realplayer
...
//-------ElvesCMS.Public.Cache-------

//------------e_public
$public_r=array('sitename'=>'Elves网站管理系统',
'newsurl'=>'http://localhost/mask/',
...
'smalltextlen'=>160,
'defaultgroupid'=>1,
'fileurl'=>'http://localhost/mask/d/file/',
'install'=>0,
'phpmode'=>0,
'dorepnum'=>300,
'loadtempnum'=>50,
'bakdbpath'=>'bdata',
'bakdbzip'=>'zip',
'downpass'=>'V4CYBXGpbeWwqiAi9p8b',
'filechmod'=>1,
'loginkey_ok'=>0,
'tbname'=>'news',
'limittype'=>0,
'redodown'=>1,
'downsofttemp'=>'[ <a href=\"#elve\" onclick=\"window.open(\'[!--down.url--]\',\'\',\'width=300,height=300,resizable=yes\');\">[!--down.name--]</a> ]',
'onlinemovietemp'=>'[ <a href=\"#elve\" onclick=\"window.open(\'[!--down.url--]\',\'\',\'width=300,height=300,resizable=yes\');\">[!--down.name--]</a> ]',
'lctime'=>1222406370,
'candocode'=>1,
'opennotcj'=>0,
...
'keytog'=>2,
'keytime'=>30,
'keyrnd'=>'JrVyb6GFrGB6q2GZDT8DjBJJJaCBuR',
