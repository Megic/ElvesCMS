...
$offset=$page*$line;
$query="select ".ReturnSqlListF($modid)." from {$dbtbpre}elve_".$class_r[$classid]['tbname']." where ".$yhadd.$add;
$totalnum=intval($_GET['totalnum']);
if($totalnum<1)
{
	$totalquery="select count(*) as total from {$dbtbpre}elve_".$class_r[$classid]['tbname']." where ".$yhadd.$add;
