...
function DoWapIconvVal($str){
	global $elve_config,$iconv,$pr;
	if($elve_config['sets']['pagechar']!='utf-8')
	{
		$char=$elve_config['sets']['pagechar']=='big5'?'BIG5':'GB2312';
...
<card id="Elvescms_wml" title="<?php echo $title;?>">
<?php
}
...
?>
<p><br/><small>Powered by ElvesCMS</small></p>
</card></wml>
<?php
	$str=ob_get_contents();
	ob_end_clean();
...
function ReturnWapStyle($add,$style){
	global $Elves,$dbtbpre,$pr,$class_r;
	...
	{
		$cr=$Elves->fetch1("select wapstyleid from {$dbtbpre}melveclass where classid='$classid'");
		if($cr['wapstyleid'])
		{
	...
	$sr=$Elves->fetch1("select path from {$dbtbpre}melvewapstyle where styleid='$styleid'");
	$wapstyle=$sr['path'];
	if(empty($wapstyle))
	{
		$wapstyle=1;
...
$pr=$Elves->fetch1("select wapopen,wapdefstyle,wapshowmid,waplistnum,wapsubtitle,wapshowdate,wapchar from {$dbtbpre}melvepublic limit 1");
