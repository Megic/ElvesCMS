...
<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="tableborder">
  ...
  {
	  $bindr=$Elves->fetch1("select id,bindtime,loginnum,lasttime from {$dbtbpre}melvemember_connect where userid='$user[userid]' and apptype='$r[apptype]' limit 1");
	  if($bindr['id'])
	  {
		  $dourl='<a href="doaction.php?melve=DelBind&id='.$bindr['id'].'" onclick="return confirm(\'确认要解除绑定?\');">解除绑定</a>';
	  }
	  else
	  {
		  $dourl='<a href="index.php?apptype='.$r['apptype'].'&elve=1">立即绑定</a>';
	  }
  ?>
  <tr bgcolor="#FFFFFF">
    <td><div align="center">
