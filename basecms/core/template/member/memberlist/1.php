...
<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="tableborder">
  <form name="memberform" method="get" action="index.php">
	...
	while($r=$Elves->fetch($sql))
	{
		...
		//用户头像
		$userpic=$r['userpic']?$r['userpic']:$public_r[newsurl].'core/data/images/nouserpic.gif';
	?>
    <tr bgcolor="#FFFFFF"> 
      <td><div align="center"> 
          <?=$r['userid']?>
        </div></td>
      <td height="25"> <a href='<?=$public_r[newsurl]?>core/space/?userid=<?=$r['userid']?>' target='_blank'> 
        <?=$r['username']?>
        </a> </td>
      <td height="25"><div align="center"> 
          <?=$registertime?>
        </div></td>
      <td height="25"><div align="center"> [<a href="<?=$public_r[newsurl]?>core/member/ShowInfo/?userid=<?=$r['userid']?>" target="_blank">会员资料</a>] 
          [<a href="<?=$public_r[newsurl]?>core/space/?userid=<?=$r['userid']?>" target="_blank">会员空间</a>]</div></td>
    </tr>
    <?
  	}
  	?>
