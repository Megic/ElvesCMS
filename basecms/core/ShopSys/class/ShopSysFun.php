...
//减少库存
function Shopsys_CutMaxnum($ddid,$buycar,$havecut,$shoppr,$elve=0){
...
function AddBuycar($classid,$id,$pn=1,$add){
	...
	//验证产品是否存在
	$infor=$Elves->fetch1("select id,classid,pmaxnum,price,buyfen from {$dbtbpre}elve_".$class_r[$classid][tbname]." where id='$id' limit 1");
...
function ShopSys_GetPre($precode,$totalmoney,$user,$classids){
	...
	$prer=$Elves->fetch1("select id,prename,precode,premoney,pretype,reuse,endtime,groupid,classid,musttotal from {$dbtbpre}melveshop_precode where precode='$precode' limit 1");
	if(!$prer['id'])
	{
		printerror("EmptyPreCode","history.go(-1)",1);
	}
...
function AddDd($add){
	...
	{
		$pr=$Elves->fetch1("select pid,pname,price from {$dbtbpre}melveshopps where pid='$add[psid]' and isclose=0");
		if(empty($pr['pid']))
		{
			printerror("NotPsid","history.go(-1)",1);
		}
	...
	{
		$payr=$Elves->fetch1("select payid,payname,payurl,userpay,userfen from {$dbtbpre}melveshoppayfs where payid='$add[payfsid]' and isclose=0");
		if(empty($payr['payid']))
		{
			printerror("NotPayfsid","history.go(-1)",1);
		}
	...
	if($userid)
	{
		...
		$user=$Elves->fetch1("select ".eReturnSelectMemberF('userid,money,userfen')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' and ".egetmf('rnd')."='$rnd' limit 1");
		if(!$user['userid'])
		{
	...
	if($payr[userfen])	//直接扣点
	{
		...
		else
		{
			if($userid)
			{
				...
				//扣除点数
				$usql=$Elves->query("update ".eReturnMemberTable()." set ".egetmf('userfen')."=".egetmf('userfen')."-".$buyallfen." where ".egetmf('userid')."='$userid'");
				if($usql)
				{
					$mess="AddDdSuccessa";
					$payby=1;
	...
	elseif($payr[userpay])	//帐号余额扣除
	{
		    if($userid)
			{
				...
				//扣除金额
				$usql=$Elves->query("update ".eReturnMemberTable()." set ".egetmf('money')."=".egetmf('money')."-".$buyallmoney." where ".egetmf('userid')."='$userid'");
				if($usql)
				{
					$mess="AddDdSuccessa";
					$payby=2;
	...
	{
		$Elves->query("delete from {$dbtbpre}melveshop_precode where id='".intval($prer['id'])."'");
	}
	if($sql)
	{
...
function ReturnBuycardd($shoppr){
	...
	for($i=0;$i<count($r)-1;$i++)
	{
		...
		//取得产品信息
		$productr=$Elves->fetch1("select title,tprice,price,isurl,titleurl,classid,id,titlepic,buyfen,pmaxnum from {$dbtbpre}elve_".$class_r[$classid][tbname]." where id='$id' limit 1");
...
function ShopDdToPay($ddid){
	...
	//是否登陆
	$user_r=islogin();
	$r=$Elves->fetch1("select ddid,payfsid,haveprice,checked,ddtime from {$dbtbpre}melveshopdd where ddid='$ddid' and userid='$user_r[userid]' limit 1");
	if(!$r['ddid'])
	{
		printerror("NotShopDdId","history.go(-1)",1);
	}
	...
	//支付方式
	$payr=$Elves->fetch1("select payid,payurl from {$dbtbpre}melveshoppayfs where payid='$r[payfsid]' and isclose=0");
...
function ShopSys_qDelDd($add){
	...
	$r=$Elves->fetch1("select ddid,outproduct,haveprice,checked,ddtime,havecutnum from {$dbtbpre}melveshopdd where ddid='$ddid' and userid='$user_r[userid]' limit 1");
	if(!$r['ddid'])
	{
		printerror("NotChangeShopDdid","history.go(-1)",1);
	}
	...
	$sql=$Elves->query("delete from {$dbtbpre}melveshopdd where ddid='$ddid' and userid='$user_r[userid]'");
	$sqladd=$Elves->query("delete from {$dbtbpre}melveshopdd_add where ddid='$ddid'");
	if($sql)
	{
...
function ShopSys_AddAddress($add){
	...
	$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}melveshop_address where userid='$user_r[userid]'");
	$isdefault=$num?0:1;
	$sql=$Elves->query("insert into {$dbtbpre}melveshop_address(addressname,userid,truename,oicq,msn,email,address,zip,mycall,phone,signbuild,besttime,isdefault) values('$add[addressname]','$user_r[userid]','$add[truename]','$add[oicq]','$add[msn]','$add[email]','$add[address]','$add[zip]','$add[mycall]','$add[phone]','$add[signbuild]','$add[besttime]','$isdefault');");
	if($sql)
	{
		printerror('AddAddressSuccess','address/AddAddress.php?melve=AddAddress',1);
	}
	else
	{
		printerror("DbError","history.go(-1)",1);
...
function ShopSys_DelAddress($add){
	...
	$sql=$Elves->query("delete from {$dbtbpre}melveshop_address where addressid='$addressid' and userid='$user_r[userid]'");
	if($sql)
	{
...
function ShopSys_DefAddress($add){
	...
	$sql1=$Elves->query("update {$dbtbpre}melveshop_address set isdefault=0 where userid='$user_r[userid]'");
	$sql=$Elves->query("update {$dbtbpre}melveshop_address set isdefault=1 where addressid='$addressid' and userid='$user_r[userid]'");
	if($sql)
	{
...
function ShopSys_GetAddress($addressid){
	...
	$address_r=$Elves->fetch1("select * from {$dbtbpre}melveshop_address where userid='$user_r[userid]' and ".$where." limit 1");
	return $address_r;
}
?>