...
$payr=$Elves->fetch1("select * from {$dbtbpre}melvepayapi where payid='$payid' and isclose=0 limit 1");
if(!$payr[payid])
{
	printerror('请选择支付平台','',1,0,1);
}
$buyr=$Elves->fetch1("select * from {$dbtbpre}melvebuygroup where id='$id'");
if(!$buyr['id'])
{
	printerror('请选择充值类型','',1,0,1);
}
...
esetcookie("paymgxe","BuyGroupPay",0);
