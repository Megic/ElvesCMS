...
define('InElvesCMS',TRUE);
define('elve_PATH',substr(dirname(__FILE__),0,-10));
...
require_once elve_PATH.'core/config/config.php';

if(!defined('ElvesCMSConfig'))
{
	exit();
}
...
//页面编码
if($elve_config['sets']['setpagechar']==1)
{
	if($elve_config['sets']['pagechar']=='gb2312'||$elve_config['sets']['pagechar']=='big5'||$elve_config['sets']['pagechar']=='utf-8')
	{
		@header('Content-Type: text/html; charset='.$elve_config['sets']['pagechar']);
	}
}
...
function db_connect(){
	global $elve_config;
	$dblink=do_dbconnect($elve_config['db']['dbserver'],$elve_config['db']['dbport'],$elve_config['db']['dbusername'],$elve_config['db']['dbpassword'],$elve_config['db']['dbname']);
	return $dblink;
}
...
function do_dbconnect($dbhost,$dbport,$dbusername,$dbpassword,$dbname){
	...
	{
		$q='';
		if($elve_config['db']['setchar'])
		{
			$q='character_set_connection='.$elve_config['db']['setchar'].',character_set_results='.$elve_config['db']['setchar'].',character_set_client=binary';
		}
		if($elve_config['db']['dbver']>='5.0')
		{
			$q.=(empty($q)?'':',').'sql_mode=\'\'';
		}
		if($q)
...
//设置COOKIE
function esetcookie($var,$val,$life=0,$elve=0){
	global $elve_config;
	$varpre=empty($elve)?$elve_config['cks']['ckvarpre']:$elve_config['cks']['ckadminvarpre'];
	return setcookie($varpre.$var,$val,$life,$elve_config['cks']['ckpath'],$elve_config['cks']['ckdomain']);
}

//返回cookie
function getcvar($var,$elve=0){
	global $elve_config;
	$tvar=empty($elve)?$elve_config['cks']['ckvarpre'].$var:$elve_config['cks']['ckadminvarpre'].$var;
	return $_COOKIE[$tvar];
}

//错误提示
function printerror($error="",$gotourl="",$elve=0,$noautourl=0,$novar=0){
	global $Elves,$editor,$public_r,$elve_config;
	if($editor==1){$a="../";}
	elseif($editor==2){$a="../../";}
	elseif($editor==3){$a="../../../";}
	else{$a="";}
...
//错误提示2：直接文字
function printerror2($error='',$gotourl='',$elve=0,$noautourl=0){
	...
	elseif($elve==7)//弹出对话框并关闭窗口
	{
		echo"<script>alert('".$error."');window.close();</script>";
	}
	else
	{
		@include(elve_PATH.'core/message/index.php');
	}
	db_close();
	exit();
}
...
//ajax错误提示
function ajax_printerror($result='',$ajaxarea='ajaxarea',$error='',$elve=0,$novar=0){
	global $Elves,$editor,$public_r,$elve_config;
	if($editor==1){$a="../";}
	elseif($editor==2){$a="../../";}
	elseif($editor==3){$a="../../../";}
...
function GetTemptb($temptb){
	global $public_r,$elve_config,$dbtbpre;
	if(!empty($elve_config['sets']['deftempid']))
	{
		$tempid=$elve_config['sets']['deftempid'];
	}
	else
	{
...
function LoadLang($file){
	global $elve_config;
	return "../data/language/".$elve_config['sets']['elang']."/".$file;
}
...
//处理提交字符
function RepPostStr($val,$elve=0){
...
//处理提交字符
function hRepPostStr($val,$elve=0){
	if($elve==1)
	{
		$val=ehtmlspecialchars($val,ENT_QUOTES);
	}
...
function eGetTableModids($tid,$tbname){
	...
	$sql=$Elves->query("select mid from {$dbtbpre}melvemod where ".$where);
...
function sub($string,$start=0,$length,$mode=false,$dot='',$rephtml=0){
	...
	$strcut = '';
	if(strtolower($elve_config['sets']['pagechar']) == 'utf-8') {
...
//返回内容伪静态
function eReturnRewriteInfoUrl($classid,$id,$elve=0){
	global $public_r;
	if(empty($public_r['rewriteinfo']))
	{
		$r['pageurl']=$public_r['newsurl']."core/action/ShowInfo.php?classid=$classid&id=$id";
		$r['rewrite']=0;
	}
	else
	{
...
//返回栏目列表伪静态
function eReturnRewriteClassUrl($classid,$elve=0){
	global $public_r;
	if(empty($public_r['rewriteclass']))
	{
		$r['pageurl']=$public_r['newsurl']."core/action/ListInfo/?classid=$classid";
		$r['rewrite']=0;
	}
	else
	{
...
//返回标题分类列表伪静态
function eReturnRewriteTitleTypeUrl($ttid,$elve=0){
	global $public_r;
	if(empty($public_r['rewriteinfotype']))
	{
		$r['pageurl']=$public_r['newsurl']."core/action/InfoType/?ttid=$ttid";
		$r['rewrite']=0;
	}
	else
	{
...
//返回TAGS列表伪静态
function eReturnRewriteTagsUrl($tagid,$tagname,$elve=0){
	global $public_r;
	$tagname=urlencode($tagname);
	if(empty($public_r['rewritetags']))
	{
		$r['pageurl']=$public_r['newsurl']."core/tags/?tagname=".$tagname;
		$r['rewrite']=0;
	}
	else
	{
...
function ReturnFirsttitleNameList($firsttitle,$isgood){
	global $Elves,$dbtbpre;
	$pubr=$Elves->fetch1("select firsttitlename,isgoodname from {$dbtbpre}melvepublic limit 1");
	//头条
...
function DoFileMkDir($file){
	$path=dirname($file.'Elvescms.txt');
	DoMkdir($path);
}
...
function sys_ReturnBqTitleLink($r){
	...
	else
	{
		...
		{
			$titleurl=$public_r['newsurl'].'core/public/jump/?classid='.$r['classid'].'&id='.$r['id'];
		}
		return $titleurl;
	}
}
...
function sys_ReturnBqTitleLinkDt($r){
	...
	if(empty($r['isurl']))
	{
		...
		{
			$titleurl=$public_r[newsurl]."core/action/ShowInfo/?classid=$r[classid]&id=$r[id]";
			return $titleurl;
		}
...
function eGetTableRowNum($tbname){
	global $Elves,$dbtbpre;
	$total_r=$Elves->fetch1("SHOW TABLE STATUS LIKE '".$tbname."';");
	return $total_r['Rows'];
}
...
function AddClassInfos($classid,$addallstr,$addstr,$checked=1){
	...
	$Elves->query("update {$dbtbpre}melveclass set ".$updatestr." where classid='$classid' limit 1");
}

//返回栏目信息数
function ReturnClassInfoNum($cr,$elve=0){
	global $Elves,$dbtbpre,$class_r;
	if($cr['islast'])
	{
		$num=$elve==0?$cr['infos']:$cr['allinfos'];
	}
	else
	{
		$f=$elve==0?'infos':'allinfos';
		$num=$Elves->gettotal("select sum(".$f.") as total from {$dbtbpre}melveclass where ".ReturnClass($class_r[$cr[classid]][sonclass]));
		$num=(int)$num;
	}
	return $num;
}
...
function ResetClassInfos($classid){
	global $Elves,$dbtbpre,$class_r;
	$infos=$Elves->gettotal("select count(*) as total from {$dbtbpre}elve_".$class_r[$classid]['tbname']." where classid='$classid'");
	$checkinfos=$Elves->gettotal("select count(*) as total from {$dbtbpre}elve_".$class_r[$classid]['tbname']."_check where classid='$classid'");
	$allinfos=$infos+$checkinfos;
	$Elves->query("update {$dbtbpre}melveclass set allinfos='$allinfos',infos='$infos' where classid='$classid' limit 1");
}
...
function DoUpdateAddDataNum($type='info',$stb,$addnum=1){
	...
	$sqladdupdate='';
	$time=time();
	$pur=$Elves->fetch1("select ".$lasttimef.",".$lastnumtbf.$sqladdf." from {$dbtbpre}melvepublic_update limit 1");
	if($stb)
	{
	...
	$Elves->query("update {$dbtbpre}melvepublic_update set ".$lastnumf."=".$lastnumf."+".$addnum.$sqladdupdate." limit 1");
}
...
function DoResetAddDataNum($type='info'){
	...
	$time=time();
	$Elves->query("update {$dbtbpre}melvepublic_update set ".$lasttimef."='$time',".$lastnumf."=0,".$lastnumtbf."='' limit 1");
}

//更新昨日信息数统计
function DoUpdateYesterdayAddDataNum(){
	global $Elves,$dbtbpre;
	$pur=$Elves->fetch1("select * from {$dbtbpre}melvepublic_update limit 1");
	$todaydate=date('Y-m-d');
	if($todaydate==date('Y-m-d',$pur['todaytimeinfo'])&&$todaydate==date('Y-m-d',$pur['todaytimepl']))
	{
		return '';
	...
	$time=time();
	$Elves->query("update {$dbtbpre}melvepublic_update set todaytimeinfo='$time',todaytimepl='$time',todaynuminfo=0,yesterdaynuminfo='$yesterdaynuminfo',todaynumpl=0,yesterdaynumpl='$yesterdaynumpl' limit 1");
}

//返回栏目自定义字段内容
function ReturnClassAddField($classid,$f){
	global $Elves,$dbtbpre,$navclassid;
	if(empty($classid))
	{
		$classid=$navclassid;
	}
	$fr=$Elves->fetch1("select ".$f." from {$dbtbpre}melveclassadd where classid='$classid' limit 1");
	if(strstr($f,','))
	{
		return $fr;
	}
...
function ReturnZtAddField($classid,$f){
	...
	$fr=$Elves->fetch1("select ".$f." from {$dbtbpre}melveztadd where ztid='$classid' limit 1");
	if(strstr($f,','))
	{
		return $fr;
	}
...
function ReturnPublicAddVar($myvar){
	...
	if(strstr($myvar,','))
	{
		...
		{
			$v=$myvr[$i];
			$vr=$Elves->fetch1("select varvalue from {$dbtbpre}melvepubvar where myvar='$v' limit 1");
	...
	{
		$vr=$Elves->fetch1("select varvalue from {$dbtbpre}melvepubvar where myvar='$myvar' limit 1");
		return $vr['varvalue'];
	}
}
...
//返回优化方案SQL
function ReturnYhSql($yhid,$yhvar,$elve=0){
	global $eyh_r;
	if(empty($yhid))
	{
		return '';
	...
	if($eyh_r[$yhid][$yhvar])
	{
		...
		$query='newstime>'.$t.(empty($elve)?'':' and ');
	}
	return $query;
}

//返回优化+条件SQL
function ReturnYhAndSql($yhadd,$where,$elve=0){
	if($yhadd.$where=='')
	{
		return '';
	}
	elseif($yhadd&&$where)
	{
		return $elve==1?' where '.$yhadd.$where:' where '.$yhadd.' and '.$where;
	}
	elseif($yhadd&&!$where)
	{
		return ' where '.$yhadd;
	...
	{
		return $elve==1?' where '.substr($where,5):' where '.$where;
	}
}
...
//返回内容查询字段
function ReturnSqlTextF($mid,$elve=0){
	global $emod_r;
	if(empty($mid))
	{
		return '*';
	}
	$f=($elve==0?'id,classid,':'').'ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard'.substr($emod_r[$mid]['tbmainf'],0,-1);
	return $f;
}
...
function ReturnInfoTbname($tbname,$checked=1,$stb=1){
	...
	{
		$r['tbname']=$dbtbpre.'elve_'.$tbname.'_check';
		$r['datatbname']=$dbtbpre.'elve_'.$tbname.'_check_data';
	}
	else//已审核
	{
		$r['tbname']=$dbtbpre.'elve_'.$tbname;
		$r['datatbname']=$dbtbpre.'elve_'.$tbname.'_data_'.$stb;
	}
	return $r;
}
...
function ReturnInfoMainTbname($tbname,$checked=1){
	global $dbtbpre;
	return empty($checked)?$dbtbpre.'elve_'.$tbname.'_check':$dbtbpre.'elve_'.$tbname;
}

//返回信息副表
function ReturnInfoDataTbname($tbname,$checked=1,$stb=1){
	global $dbtbpre;
	return empty($checked)?$dbtbpre.'elve_'.$tbname.'_check_data':$dbtbpre.'elve_'.$tbname.'_data_'.$stb;
}

//主表信息
function ReturnIndexTableInfo($tbname,$f,$classid,$id){
	global $dbtbpre;
	$r=$Elves->fetch1("select ".$f." from {$dbtbpre}elve_".$tbname."_index where id='$id' limit 1");
	return $r;
}
...
function eReturnFstb($fstb){
	...
	return $dbtbpre.'melvefile_'.$fstb;
}
...
function eCheckTbHaveField($tid,$tbname,$f){
	...
	$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvef where ".$where." limit 1");
	return $num;
}
...
function ReplaceListVars($no,$listtemp,$subnews,$subtitle,$formatdate,$url,$haveclass=0,$r,$field,$docode=0){
	global $Elves,$public_r,$class_r,$class_zr,$fun_r,$dbtbpre,$emod_r,$class_tr,$level_r,$navclassid,$etable_r;
	if($haveclass)
	{
	...
	for($i=1;$i<$fcount;$i++)
	{
		...
		elseif($f=='titlepic')//标题图片
		{
			if(empty($value))
		    {
				$value=$public_r[newsurl].'core/data/images/notimg.gif';
			}
			$spf=1;
		}
...
function ReplaceBefrom($befrom){
	...
	$r=$Elves->fetch1("select befromid,sitename,siteurl from {$dbtbpre}melvebefrom where sitename='$befrom' limit 1");
	if(empty($r[befromid]))
	{return $befrom;}
...
function ReplaceWriter($writer){
	...
	$r=$Elves->fetch1("select wid,writer,email from {$dbtbpre}melvewriter where writer='$writer' limit 1");
...
function BakDown($classid,$id,$pathid,$userid,$username,$title,$cardfen,$online=0){
	...
	$cardfen=(int)$cardfen;
	$classid=(int)$classid;
	$sql=$Elves->query("insert into {$dbtbpre}melvedownrecord(id,pathid,userid,username,title,cardfen,truetime,classid,online) values($id,$pathid,$userid,'$username','".addslashes($title)."',$cardfen,$truetime,$classid,'$online');");
}
...
function BakBuy($userid,$username,$buyname,$userfen,$money,$userdate,$type=0){
	...
	$Elves->query("insert into {$dbtbpre}melvebuybak(userid,username,card_no,cardfen,money,buytime,userdate,type) values('$userid','$username','$buyname','$userfen','$money','$buytime','$userdate','$type');");
}

//发送短消息
function eSendMsg($title,$msgtext,$to_username,$from_userid,$from_username,$isadmin,$issys,$elve=0){
	global $Elves,$dbtbpre;
	$tbname=$elve==1?$dbtbpre.'melvehmsg':$dbtbpre.'melveqmsg';
	$msgtime=date("Y-m-d H:i:s");
	$Elves->query("insert into ".$tbname."(title,msgtext,haveread,msgtime,to_username,from_userid,from_username,isadmin,issys) values('$title','$msgtext',0,'$msgtime','$to_username','$from_userid','$from_username','$isadmin','$issys');");
	//消息状态
	$userr=$Elves->fetch1("select ".eReturnSelectMemberF('userid,havemsg')." from ".eReturnMemberTable()." where ".egetmf('username')."='$to_username' limit 1");
	if(!$userr['havemsg'])
	{
		$newhavemsg=eReturnSetHavemsg($userr['havemsg'],0);
		$Elves->query("update ".eReturnMemberTable()." set ".egetmf('havemsg')."='$newhavemsg' where ".egetmf('userid')."='".$userr['userid']."' limit 1");
	}
}

//发送通知
function eSendNotice($title,$msgtext,$to_username,$from_userid,$from_username,$elve=0){
	global $Elves,$dbtbpre;
	$tbname=$elve==1?$dbtbpre.'melvehnotice':$dbtbpre.'melvenotice';
	$msgtime=date("Y-m-d H:i:s");
	$Elves->query("insert into ".$tbname."(title,msgtext,haveread,msgtime,to_username,from_userid,from_username) values('".$title."','".$msgtext."',0,'$msgtime','$to_username','$from_userid','$from_username');");
}
...
function SubSmalltextVal($value,$len){
	...
	$value=str_replace(array("\r\n","<br />","<br>","&nbsp;","[!--Elvesnews.page--]","[/!--Elvesnews.page--]"),array("","\r\n","\r\n"," ","",""),$value);
...
function SubSchallSmalltext($value,$len){
	$value=str_replace(array("\r\n","&nbsp;","[!--Elvesnews.page--]","[/!--Elvesnews.page--]"),array("","","",""),$value);
	$value=strip_tags($value);
	if($len)
	{
		$value=sub($value,0,$len,false);
...
function GetInfoFilename($classid,$id){
	...
	$infor=$Elves->fetch1("select isurl,groupid,classid,newspath,filename,id from {$dbtbpre}elve_".$class_r[$classid][tbname]." where id='$id' limit 1");
...
//格式化信息目录
function FormatPath($classid,$mynewspath,$melve=0){
	global $class_r;
	if($melve)
	{
		$newspath=$mynewspath;
	...
	$path=elve_PATH.ReturnSaveInfoPath($classid,$id);
	$returnpath="";
	$r=explode("/",$newspath);
	$count=count($r);
	for($i=0;$i<$count;$i++){
...
function eReturnFileTable($modtype,$fstb){
	...
	if($modtype==0)//信息
	{
		...
		$table=$dbtbpre.'melvefile_'.$fstb;
	}
	elseif($modtype==5)//公共
	{
		$table=$dbtbpre.'melvefile_public';
	}
	elseif($modtype==6)//会员
	{
		$table=$dbtbpre.'melvefile_member';
	}
	else//其他
	{
		$table=$dbtbpre.'melvefile_other';
	}
	return $table;
}
...
function eInsertFileTable($filename,$filesize,$path,$adduser,$classid,$no,$type,$id,$cjid,$fpath,$pubid,$modtype=0,$fstb=1){
	...
	if($modtype==0)//信息
	{
		...
		$fstb=(int)$fstb;
		$fstb=eReturnFileStb($fstb);
		$sql=$Elves->query("insert into {$dbtbpre}melvefile_".$fstb."(pubid,filename,filesize,adduser,path,filetime,classid,no,type,id,cjid,onclick,fpath) values('$pubid','$filename','$filesize','$adduser','$path','$filetime','$classid','$no','$type','$id','$cjid',0,'$fpath');");
	}
	elseif($modtype==5)//公共
	{
		$sql=$Elves->query("insert into {$dbtbpre}melvefile_public(filename,filesize,adduser,path,filetime,modtype,no,type,id,cjid,onclick,fpath) values('$filename','$filesize','$adduser','$path','$filetime',0,'$no','$type','$id','$cjid',0,'$fpath');");
	}
	elseif($modtype==6)//会员
	{
		$sql=$Elves->query("insert into {$dbtbpre}melvefile_member(filename,filesize,adduser,path,filetime,no,type,id,cjid,onclick,fpath) values('$filename','$filesize','$adduser','$path','$filetime','$no','$type','$id','$cjid',0,'$fpath');");
	}
	else//其他
	{
		$sql=$Elves->query("insert into {$dbtbpre}melvefile_other(filename,filesize,adduser,path,filetime,modtype,no,type,id,cjid,onclick,fpath) values('$filename','$filesize','$adduser','$path','$filetime','$modtype','$no','$type','$id','$cjid',0,'$fpath');");
	}
	return $sql;
}
...
function UpdateTheFileOther($modtype,$id,$checkpass,$tb='other'){
	...
	$sql=$Elves->query("update {$dbtbpre}melvefile_{$tb} set id='$id',cjid=0 where cjid='$checkpass'".$where);
}
...
function UpdateTheFileEditOther($modtype,$id,$tb='other'){
	...
	$sql=$Elves->query("update {$dbtbpre}melvefile_{$tb} set cjid=0 where id='$id'".$where);
}
...
//返回附件域名地址
function eReturnFileUrl($elve=0){
	global $public_r;
	if($elve==1)
	{
...
//格式化附件目录
function FormatFilePath($classid,$mynewspath,$melve=0){
	global $public_r;
	if($melve)
	{
		$newspath=$mynewspath;
...
//上传文件
function DoTranFile($file,$file_name,$file_type,$file_size,$classid,$elve=0){
...
function DoDelFile($r){
	...
	$delfile=elve_PATH.$fspath['filepath'].$path.$r['filename'];
	DelFiletext($delfile);
	//FileServer
	if($public_r['openfileserver'])
	{
...
//返回多专题
function sys_ReturnMoreZt($zt,$elve=0){
	$f=$elve==1?'ztid':'cid';
	$r=explode(',',$zt);
	$count=count($r);
	$return_r[0]=intval($r[0]);
...
function ReturnDownQzPath($path,$urlid){
	...
	{
		$r=$Elves->fetch1("select urlid,url,downtype from {$dbtbpre}melvedownurlqz where urlid='$urlid'");
		if($r['urlid'])
		{
...
function ReturnDSofturl($downurl,$qz,$path='../../',$isdown=0){
	...
	$url=$urlr['repath'];
	@include_once(elve_PATH."core/DownSys/class/enpath.php");//防盗链
	if($isdown)
	{
...
function DoSafeCheckFromurl(){
	global $elve_config;
	if($elve_config['esafe']['ckfromurl']==0||defined('ElvesCMSNFPage'))//不启用
	{
		return '';
	}
	...
	if($elve_config['esafe']['ckfromurl']==1)//全部启用
	{
	...
	elseif($elve_config['esafe']['ckfromurl']==2)//后台启用
	{
		if(defined('ElvesCMSAdmin')&&!stristr($fromurl,$domain))
		{
			echo"";
			exit();
		}
	}
	elseif($elve_config['esafe']['ckfromurl']==3)//前台启用
	{
		if(!defined('ElvesCMSAdmin')&&!stristr($fromurl,$domain))
		{
			echo"";
...
//验证IP
function eCheckAccessIp($elve=0){
	global $public_r;
	$userip=egetip();
	if($elve)//后台
	{
...
//验证操作时间
function eCheckTimeCloseDo($elve){
	global $public_r;
	if(stristr($public_r['timeclosedo'],','.$elve.','))
	{
		$h=date('G');
...
function RepPltextFace($text){
	...
	{
		$r=explode('##',$facer[$i]);
		$text=str_replace($r[0],"<img src='".$public_r['newsurl']."core/data/face/".$r[1]."' border=0>",$text);
	}
	return $text;
}
...
//设置验证码
function elveSetShowKey($varname,$val,$elve=0){
	global $public_r;
	$val=md5($val);
	$time=time();
	$checkpass=md5(md5($val.'ElvesCMS'.$time).$public_r['keyrnd']);
	$key=$time.','.$checkpass.','.$val;
	esetcookie($varname,$key,0,$elve);
}

//检查验证码
function elveCheckShowKey($varname,$postval,$dopr,$elve=0){
	global $public_r;
	$r=explode(',',getcvar($varname,$elve));
	$cktime=$r[0];
	$pass=$r[1];
	$val=$r[2];
	$time=time();
	...
	$checkpass=md5(md5(md5($postval).'ElvesCMS'.$cktime).$public_r['keyrnd']);
	if($checkpass<>$pass)
	{
		printerror('FailKey','',$dopr);
	}
...
//清空验证码
function elveEmptyShowKey($varname,$elve=0){
	esetcookie($varname,'',0,$elve);
}

//设置提交码
function DoSetActionPass($userid,$username,$rnd,$other,$elve=0){
	global $elve_config;
	$varname='actionepass';
	$date=date("Y-m-d-H");
	$pass=md5(md5($rnd.'-'.$userid.'-'.$date.'-'.$other).$elve_config['cks']['ckrnd'].$username);
	esetcookie($varname,$pass,0,$elve);
}

//清除提交码
function DoEmptyActionPass($elve=0){
	$varname='actionepass';
	esetcookie($varname,'',0,$elve);
}

//检测提交码
function DoCheckActionPass($userid,$username,$rnd,$other,$elve=0){
	global $elve_config;
	$varname='actionepass';
	$date=date("Y-m-d-H");
	$checkpass=md5(md5($rnd.'-'.$userid.'-'.$date.'-'.$other).$elve_config['cks']['ckrnd'].$username);
	$pass=getcvar($varname,$elve);
	if($checkpass<>$pass)
	{
		exit();
	}
...
function toReturnFname($tbname,$f){
	global $Elves,$dbtbpre;
	$r=$Elves->fetch1("select fname from {$dbtbpre}melvef where f='$f' and tbname='$tbname' limit 1");
	return $r[fname];
}
...
function ReturnPinyinFun($hz){
	global $elve_config;
	include_once(elve_PATH.'core/class/epinyin.php');
	//编码
...
//返回当前地址
function eReturnSelfPage($elve=0){
	if(empty($elve))
	{
...
function sys_CheckMemberGroup($groupid){
	if(!defined('InElvesCMSUser'))
	{
		include_once elve_PATH.'core/member/class/user.php';
	}
...
function GetTxtFieldText($pagetexturl){
	...
	$file=$elve_config['sets']['txtpath'].$pagetexturl.".php";
...
function MkDirTxtFile($date,$file){
	global $elve_config;
	$r=explode("/",$date);
	$path=$elve_config['sets']['txtpath'].$r[0];
	DoMkdir($path);
	$path=$elve_config['sets']['txtpath'].$date;
	DoMkdir($path);
...
//防火墙
function DoElvesCMSFireWall(){
	global $elve_config;
	if(!empty($elve_config['fw']['adminloginurl']))
	{
		$usehost=FWeReturnDomain();
		if($usehost!=$elve_config['fw']['adminloginurl'])
		{
			FWShowMsg('Login Url');
		}
	}
	if($elve_config['fw']['adminhour']!=='')
	{
		$h=date('G');
		if(!strstr(','.$elve_config['fw']['adminhour'].',',','.$h.','))
		{
			FWShowMsg('Admin Hour');
		}
	}
	if($elve_config['fw']['adminweek']!=='')
	{
		$w=date('w');
		if(!strstr(','.$elve_config['fw']['adminweek'].',',','.$w.','))
		{
			FWShowMsg('Admin Week');
		}
	}
	if(!defined('ElvesCMSAPage')&&$elve_config['fw']['adminckpassvar']&&$elve_config['fw']['adminckpassval'])
	{
		FWCheckPassword();
	}
}
...
function FWClearGetText($str){
	...
	if(empty($elve_config['fw']['cleargettext']))
	{
		return '';
	}
	$r=explode(',',$elve_config['fw']['cleargettext']);
	$count=count($r);
	for($i=0;$i<$count;$i++)
	{
		if(stristr($str,$r[$i]))
...
function FWSetPassword(){
	global $elve_config;
	if(!$elve_config['fw']['eopen']||!$elve_config['fw']['adminckpassvar']||!$elve_config['fw']['adminckpassval'])
	{
		return '';
	}
	$ip=$elve_config['esafe']['ckhloginip']==0?'127.0.0.1':egetip();
	$elveckpass=md5(md5($elve_config['fw']['adminckpassval'].'-Elvescms-'.$elve_config['fw']['epass']).'-'.$ip.'-'.$elve_config['fw']['adminckpassval'].'-webelves.org-');
	esetcookie($elve_config['fw']['adminckpassvar'],$elveckpass,0,1);
}
...
function FWCheckPassword(){
	...
	$ip=$elve_config['esafe']['ckhloginip']==0?'127.0.0.1':egetip();
	$elveckpass=md5(md5($elve_config['fw']['adminckpassval'].'-Elvescms-'.$elve_config['fw']['epass']).'-'.$ip.'-'.$elve_config['fw']['adminckpassval'].'-webelves.org-');
	if($elveckpass<>getcvar($elve_config['fw']['adminckpassvar'],1))
	{
		FWShowMsg('Password');
	}
}
...
function FWEmptyPassword(){
	global $elve_config;
	esetcookie($elve_config['fw']['adminckpassvar'],'',0,1);
}
?>