...
function SearchAllChangeChar($value){
	global $elve_config,$char,$targetchar,$iconv;
...
function LoadSearchAll($lid,$start,$userid,$username){
	global $Elves,$dbtbpre,$class_r,$fun_r,$public_r,$emod_r;
	$lid=(int)$lid;
	if(empty($lid))
	{
		printerror('ErrorUrl','');
	}
	$lr=$Elves->fetch1("select tbname,titlefield,infotextfield,loadnum,lastid from {$dbtbpre}melvesearchall_load where lid='$lid'");
	if(empty($lr['tbname']))
	{
		printerror('ErrorUrl','');
	}
	//不导入栏目
	$pr=$Elves->fetch1("select schallnotcid from {$dbtbpre}melvepublic limit 1");
	$line=$lr['loadnum'];
	if(empty($line))
	{
		$line=300;
	...
	$selectf='';
	$savetxtf='';
	$fsql=$Elves->query("select tid,f,savetxt,tbdataf from {$dbtbpre}melvef where (f='$lr[titlefield]' or f='$lr[infotextfield]') and tbname='$lr[tbname]' limit 2");
	...
	$b=0;
	$sql=$Elves->query("select id,stb,classid,isurl,newstime".$selectf." from {$dbtbpre}elve_".$lr['tbname']." where id>$start order by id limit ".$line);
	while($r=$Elves->fetch($sql))
	{
		...
		if($selectdtf)
		{
			$finfor=$Elves->fetch1("select id".$selectdtf." from {$dbtbpre}elve_".$lr['tbname']."_data_".$r[stb]." where id='$r[id]'");
		...
		$Elves->query("insert into {$dbtbpre}melvesearchall(sid,id,classid,title,infotime,infotext) values(NULL,'$r[id]','$r[classid]','".addslashes($title)."','$infotime','".addslashes($infotext)."');");
	}
	if(empty($b))
	{
		$lasttime=time();
		...
		$Elves->query("update {$dbtbpre}melvesearchall_load set lasttime='$lasttime',lastid='$newstart' where lid='$lid'");
		echo "<link rel=\"stylesheet\" href=\"../../data/images/css.css\" type=\"text/css\"><center><b>".$lr['tbname'].$fun_r[LoadSearchAllIsOK]."</b></center>";
		db_close();
		$Elves=null;
		exit();
	}
	echo"<link rel=\"stylesheet\" href=\"../../data/images/css.css\" type=\"text/css\"><meta http-equiv=\"refresh\" content=\"0;url=LoadSearchAll.php?melve=LoadSearchAll&lid=$lid&start=$newstart\">".$fun_r[OneLoadSearchAllSuccess]."(ID:<font color=red><b>".$newstart."</b></font>)";
	exit();
}
