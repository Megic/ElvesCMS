...
function DelFile($fileid,$userid,$username){
	...
	$fstb=(int)$_GET['fstb'];
	$r=$Elves->fetch1("select filename,path,classid,fpath from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid' limit 1");
	$sql=$Elves->query("delete from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid'");
	DoDelFile($r);
	if($sql)
	{
		//操作日志
...
function DelFile_all($fileid,$userid,$username){
	...
	//操作权限
	if($_POST['melve']=='TDelFile_all')
	{
		$userid=(int)$userid;
		$ur=$Elves->fetch1("select groupid,adminclass,filelevel from {$dbtbpre}melveuser where userid='$userid' limit 1");
		if($ur['filelevel'])
		{
			$gr=$Elves->fetch1("select dofile from {$dbtbpre}melvegroup where groupid='$ur[groupid]'");
			if(!$gr['dofile'])
			{
	...
	for($i=0;$i<count($fileid);$i++)
	{
		...
		$r=$Elves->fetch1("select filename,path,classid,fpath from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid[$i]' limit 1");
		$sql=$Elves->query("delete from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid[$i]'");
		DoDelFile($r);
    }
	if($sql)
	{
...
function DelPathFile($filename,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$dfnum=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvefile_1 where filename='$dfile'");
		if($dfnum)
		{
			$Elves->query("delete from {$dbtbpre}melvefile_1 where filename='$dfile'");
			//FileServer
...
function DoMarkSmallPic($add,$userid,$username){
	global $Elves,$class_r,$dbtbpre,$public_r,$efileftp_fr;
	//导入gd处理文件
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$r=$Elves->fetch1("select classid,filename,path,no,fpath from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid[$i]'");
