...
function ReturnAddF($modid,$rdata=0){
	...
	$record="<!--record-->";
	$field="<!--field--->";
	$mr=$Elves->fetch1("select tempvar,enter,listandf,setandf,listtempvar from {$dbtbpre}melvemod where mid='$modid'");
	//模板字段
	if($rdata==1)//内容
	{
...
function AddFeedback($add){
	...
	//版面是否存在
	$br=$Elves->fetch1("select bid,enter,mustenter,filef,groupid,checkboxf from {$dbtbpre}melvefeedbackclass where bid='$bid';");
	if(empty($br['bid']))
	{
	...
	$pr=$Elves->fetch1("select feedbacktfile,feedbackfilesize,feedbackfiletype from {$dbtbpre}melvepublic limit 1");
	//必填项
	...
	$ip=egetip();
	$sql=$Elves->query("insert into {$dbtbpre}melvefeedback(bid,saytime,ip,filepath,filename,userid,username,haveread".$addf.") values('$bid','$saytime','$ip','$filepath','$filename','$userid','$username',0".$addval.");");
	$fid=$Elves->lastid();
	//更新附件
	...
	if($sql)
	{
		$reurl=DoingReturnUrl("../tool/feedback/?bid=$bid",$add['elvefrom']);
...
function AddError($add){
	...
	$sql=$Elves->query("insert into {$dbtbpre}melvedownerror(id,errortext,errorip,errortime,email,classid,cid) values($id,'".addslashes($errortext)."','$ip','$errortime','".addslashes($email)."',$classid,'$cid');");
	if($sql)
	{
