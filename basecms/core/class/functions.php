<?php
define('InElvesCMSHfun',TRUE);
//-------------- 公共区 ----------------------

//返回后台风格
function elveReturnAdminStyle(){
	global $public_r;
...
function AdminReturnClassLink($classid){
	global $class_r,$editor,$fun_r,$elvecheck;
	$addcheck='';
	if($elvecheck)
	{
		$addcheck='&elvecheck=1';
	}
	if($editor==1)
	{
		$addurl='../';
...
function postpage($num,$line,$page_line,$start,$page,$form){
	...
	if($page<>0)
	{
		$toppage='<a href="#elve" onclick="javascript:GotoPostPage(0,0);">'.$fun_r['adminstartpage'].'</a>&nbsp;';
		$pagepr=$page-1;
		$prepage='<a href="#elve" onclick="javascript:GotoPostPage('.$pagepr.',0);">'.$fun_r['adminpripage'].'</a>';
	}
	//下一页
	if($page!=$totalpage-1)
	{
		$pagenex=$page+1;
		$nextpage='&nbsp;<a href="#elve" onclick="javascript:GotoPostPage('.$pagenex.',0);">'.$fun_r['adminnextpage'].'</a>';
		$lastpage='&nbsp;<a href="#elve" onclick="javascript:GotoPostPage('.($totalpage-1).',0);">'.$fun_r['adminlastpage'].'</a>';
	}
	$starti=$page-$snum<0?0:$page-$snum;
	$no=0;
	for($i=$starti;$i<$totalpage&&$no<$page_line;$i++)
	{
		...
		{
			$is_1='<a href="#elve" onclick="javascript:GotoPostPage('.$i.',0);">';
			$is_2="</a>";
		}
		$pagenum=$i+1;
		$returnstr.="&nbsp;".$is_1.$pagenum.$is_2;
...
function GetModTable($mid){
	...
	$r=$Elves->fetch1("select tid,tbname from {$dbtbpre}melvemod where mid='$mid'");
	return $r;
}

//建立专题目录
function CreateZtPath($ztpath){
	$createpath=elve_PATH.$ztpath;
...
function CreateClassPath($classpath){
	$createpath=elve_PATH.$classpath;
	$mk=DoMkdir($createpath);
	$createfilepath=elve_PATH.'d/file/'.$classpath;//建立附件目录
    $mk1=DoMkdir($createfilepath);
}

//建立标题分类目录
function CreateInfoTypePath($tpath){
	$createpath=elve_PATH.$tpath;
    $mk=DoMkdir($createpath);
}

//删除栏目缓存文件
function DelListmelve(){
	$file=elve_PATH."core/data/fc/Listmelve.php";
	DelFiletext($file);
	$file1=elve_PATH."core/data/fc/ListClass0.php";
	DelFiletext($file1);
	$file2=elve_PATH."core/data/fc/ListClass1.php";
	DelFiletext($file2);
}

//删除模板临时缓存文件
function DelOneTempTmpfile($classid){
	$file=elve_PATH.'core/data/tmp/dt_temp'.$classid.'.php';
	if(file_exists($file))
	{
		DelFiletext($file);
	}
...
function RepPhpAspJspcode($string){
	...
	if(!$public_r[candocode]){
		//$string=str_replace("<?xml","[!--elve.xml--]",$string);
		...
		//$string=str_replace("[!--elve.xml--]","<?xml",$string);
	}
	return $string;
}
...
//替换文件前缀
function RepFilenameQz($qz,$elve=0){
	if(empty($elve))
	{
		$qz=str_replace("/","",$qz);
		$qz=str_replace("\\","",$qz);
...
function ReturnCheckDoRep(){
	global $Elves,$dbtbpre;
	//信息来源
	$befrom=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvebefrom");
	//作者
	$writer=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvewriter");
	//替换字符
	$words=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvewords");
	//内容关键字
	$key=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvekey");
...
//更新缓存
function ChangemelveData($userid,$username){
	//操作权限
...
//下载
function DownLoadFile($file,$filepath,$elve=0){
	if(empty($file))
	{
...
function CheckTempGroup($gid){
	...
	$r=$Elves->fetch1("select gid,gname from {$dbtbpre}melvetempgroup where gid='$gid'");
	if(empty($r['gid']))
	{
		printerror("ErrorUrl","");
	}
...
function ReplaceKey($newstext,$classid=0){
	...
	$sql=$Elves->query("select keyname,keyurl from {$dbtbpre}melvekey".$where);
...
function ReplaceWord($newstext){
	...
	if(empty($newstext))
	{return $newstext;}
	$sql=$Elves->query("select newword,oldword from {$dbtbpre}melvewords");
	while($r=$Elves->fetch($sql))
	{
...
function RenameListfile($classid,$lencord,$num,$type,$newtype,$classpath){
	...
	for($j=1;$j<=$page;$j++)
	{
		if($j==1)
		{
			$listfile=elve_PATH.$classpath."/index";
		}
		else
		{
			$listfile=elve_PATH.$classpath."/index_".$j;
		}
...
//单信息加入专题
function AddInfoToZt($ztid,$zcid,$classid,$id,$newstime,$isgood=0,$elve=0){
	global $Elves,$dbtbpre,$class_r;
	if($elve==1)//检查是否增加过
	{
		$infor=$Elves->fetch1("select zid,ztid,cid from {$dbtbpre}melveztinfo where ztid='$ztid' and classid='$classid' and id='$id' limit 1");
		if($infor['ztid'])
		{
			if($infor['cid']!=$zcid)
			{
				$Elves->query("update {$dbtbpre}melveztinfo set cid='$zcid',newstime='$newstime' where zid='$infor[zid]' limit 1");
			}
		}
		else
		{
			$mid=$class_r[$classid]['modid'];
			$Elves->query("insert into {$dbtbpre}melveztinfo(ztid,cid,classid,id,newstime,mid,isgood) values('$ztid','$zcid','$classid','$id','$newstime','$mid','$isgood');");
		}
	}
	else
	{
...
//多信息加入专题
function AddMoreInfoToZt($ztid,$zcid,$tbname,$where,$elve=0){
	global $Elves,$dbtbpre,$class_r;
	if(empty($where))
	{
		return '';
	}
	$sql=$Elves->query("select id,classid,newstime from {$dbtbpre}elve_".$tbname.($elve==0?'':'_index')." where ".$where);
	while($r=$Elves->fetch($sql))
	{
		$zinfor=$Elves->fetch1("select zid,ztid,cid from {$dbtbpre}melveztinfo where ztid='$ztid' and classid='$r[classid]' and id='$r[id]' limit 1");
		if($zinfor['ztid'])
		{
			if($zinfor['cid']!=$zcid)
			{
				$Elves->query("update {$dbtbpre}melveztinfo set cid='$zcid' where zid='$zinfor[zid]' limit 1");
			}
		}
		else
		{
			$mid=$class_r[$r[classid]]['modid'];
			$Elves->query("insert into {$dbtbpre}melveztinfo(ztid,cid,classid,id,newstime,mid,isgood) values('$ztid','$zcid','$r[classid]','$r[id]','$r[newstime]','$mid','0');");
		}
	}
}
...
function InsertZtInfo($ztids,$zcids,$oldztids,$oldzcids,$classid,$id,$newstime){
	...
	if($zcids)
	{
		...
		for($i=0;$i<$count;$i++)
		{
			...
			{
				$zcr=$Elves->fetch1("select ztid from {$dbtbpre}melvezttype where cid='$cid' limit 1");
				if(!$zcr['ztid'])
				{
					continue;
				}
	...
	if($oldztids)
	{
		...
		for($di=0;$di<$dcount;$di++)
		{
			...
			$Elves->query("delete from {$dbtbpre}melveztinfo where ztid='$dztid' and classid='$classid' and id='$id'");
		}
	}
}
...
function DelZtInfo($where){
	...
	$Elves->query("delete from {$dbtbpre}melveztinfo where ".$where);
}

//信息送审
function InfoInsertToWorkflow($id,$classid,$wfid,$userid,$username){
	global $Elves,$dbtbpre,$class_r;
	$wfitemr=$Elves->fetch1("select tid,tno,groupid,userclass,username,tstatus from {$dbtbpre}melveworkflowitem where wfid='$wfid' order by tno limit 1");
	//状态更新
	$Elves->query("insert into {$dbtbpre}melvewfinfo(id,classid,wfid,tid,groupid,userclass,username,checknum,tstatus,checktno) values('$id','$classid','$wfid','$wfitemr[tid]','$wfitemr[groupid]','$wfitemr[userclass]','$wfitemr[username]',1,'$wfitemr[tstatus]',0);");
	//日志
...
function InfoUpdateToWorkflow($id,$classid,$wfid,$userid,$username){
	global $Elves,$dbtbpre,$class_r;
	$wfinfor=$Elves->fetch1("select checknum,wfid,tid,checktno from {$dbtbpre}melvewfinfo where id='$id' and classid='$classid' limit 1");
	...
	if($wfinfor[tid])
	{
		$ywfitemr=$Elves->fetch1("select tno from {$dbtbpre}melveworkflowitem where tid='$wfinfor[tid]'");
		$wfitemr=$Elves->fetch1("select tid,tno,groupid,userclass,username,tstatus from {$dbtbpre}melveworkflowitem where wfid='$wfinfor[wfid]' and tno>$ywfitemr[tno] order by tno limit 1");
	}
	else
	{
		$wfitemr=$Elves->fetch1("select tid,tno,groupid,userclass,username,tstatus from {$dbtbpre}melveworkflowitem where wfid='$wfinfor[wfid]' order by tno limit 1");
	}
	//状态更新
	$Elves->query("update {$dbtbpre}melvewfinfo set tid='$wfitemr[tid]',groupid='$wfitemr[groupid]',userclass='$wfitemr[userclass]',username='$wfitemr[username]',checknum=checknum+1,tstatus='$wfitemr[tstatus]',checktno='0' where id='$id' and classid='$classid' limit 1");
	//日志
...
function InsertWfLog($classid,$id,$wfid,$tid,$username,$checktext,$checknum,$checktype){
	global $Elves,$dbtbpre,$class_r,$lur;
	$checktime=time();
	$checktext=RepPostStr($checktext);
	$Elves->query("insert into {$dbtbpre}melvewfinfolog(id,classid,wfid,tid,username,checktime,checktext,checknum,checktype) values('$id','$classid','$wfid','$tid','$username','$checktime','$checktext','$checknum','$checktype');");
}
...
function eInsertTags($tags,$classid,$id,$newstime){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$r=$Elves->fetch1("select tagid from {$dbtbpre}melvetags where tagname='$tagname' limit 1");
		if($r[tagid])
		{
			$datar=$Elves->fetch1("select tagid,classid,newstime from {$dbtbpre}melvetagsdata where tagid='$r[tagid]' and id='$id' and mid='$mid' limit 1");
			if($datar[tagid])
			{
				if($datar[classid]!=$classid||$datar[newstime]!=$newstime)
				{
					$Elves->query("update {$dbtbpre}melvetagsdata set classid='$classid',newstime='$newstime' where tagid='$r[tagid]' and id='$id' and mid='$mid' limit 1");
				}
			}
			else
			{
				$Elves->query("update {$dbtbpre}melvetags set num=num+1 where tagid='$r[tagid]'");
				$Elves->query("insert into {$dbtbpre}melvetagsdata(tagid,classid,id,newstime,mid) values('$r[tagid]','$classid','$id','$newstime','$mid');");
			}
		}
		else
		{
			$Elves->query("insert into {$dbtbpre}melvetags(tagname,num,isgood,cid) values('$tagname',1,0,0);");
			$tagid=$Elves->lastid();
			$Elves->query("insert into {$dbtbpre}melvetagsdata(tagid,classid,id,newstime,mid) values('$tagid','$classid','$id','$newstime','$mid');");
		}
	}
}
...
function eReturnInfoTags($classid,$id,$mid){
	...
	$tags='';
	$dh='';
	$sql=$Elves->query("select tagid from {$dbtbpre}melvetagsdata where id='$id' and mid='$mid' order by tagid");
	while($r=$Elves->fetch($sql))
	{
		$tr=$Elves->fetch1("select tagname from {$dbtbpre}melvetags where tagid='$r[tagid]'");
		$tags.=$dh.$tr[tagname];
		$dh=',';
	}
	return $tags;
...
function MoveCheckInfoData($tbname,$checked,$stb,$where){
	...
	if(empty($checked))
	{
		$ytbname=$dbtbpre.'elve_'.$tbname.'_check';
		$ydatatbname=$dbtbpre.'elve_'.$tbname.'_check_data';
		$ntbname=$dbtbpre.'elve_'.$tbname;
		$ndatatbname=$dbtbpre.'elve_'.$tbname.'_data_'.$stb;
	}
	else
	{
		$ytbname=$dbtbpre.'elve_'.$tbname;
		$ydatatbname=$dbtbpre.'elve_'.$tbname.'_data_'.$stb;
		$ntbname=$dbtbpre.'elve_'.$tbname.'_check';
		$ndatatbname=$dbtbpre.'elve_'.$tbname.'_check_data';
	}
	$Elves->query("replace into ".$ntbname." select * from ".$ytbname." where ".$where);
	$Elves->query("replace into ".$ndatatbname." select * from ".$ydatatbname." where ".$where);
	//删除原表
	$Elves->query("delete from ".$ytbname." where ".$where);
	$Elves->query("delete from ".$ydatatbname." where ".$where);
}
...
function UpdateAllDataTbField($tbname,$update,$where,$upcheck=1,$updoc=1){
	global $Elves,$dbtbpre;
	//已审核
	$tbr=$Elves->fetch1("select datatbs from {$dbtbpre}melvetable where tbname='$tbname' limit 1");
	if($tbr['datatbs'])
	{
		...
		for($i=1;$i<$count-1;$i++)
		{
			$Elves->query("update {$dbtbpre}elve_".$tbname."_data_".$dtbr[$i]." set ".$update.$where);
		}
	}
	//未审核
	if($upcheck==1)
	{
		$Elves->query("update {$dbtbpre}elve_".$tbname."_check_data set ".$update.$where);
	}
	//归档
	if($updoc==1)
	{
		$Elves->query("update {$dbtbpre}elve_".$tbname."_doc_data set ".$update.$where);
	}
}
...
function DelAllDataTbInfo($tbname,$where,$delcheck=1,$deldoc=1){
	...
	if($tbr['datatbs'])
	{
		...
		for($i=1;$i<$count-1;$i++)
		{
			$Elves->query("delete from {$dbtbpre}elve_".$tbname."_data_".$dtbr[$i]." where ".$where);
		}
	}
	//未审核
	if($delcheck==1)
	{
		$Elves->query("delete from {$dbtbpre}elve_".$tbname."_check_data where ".$where);
	}
	//归档
	if($deldoc==1)
	{
		$Elves->query("delete from {$dbtbpre}elve_".$tbname."_doc_data where ".$where);
	}
}
...
function DelFileOtherTable($where,$tb='other'){
	...
	//删除附件
	$filesql=$Elves->query("select filename,path,modtype,fpath from {$dbtbpre}melvefile_{$tb} where ".$where);
	while($filer=$Elves->fetch($filesql))
	{
		DoDelFile($filer);
	}
	$Elves->query("delete from {$dbtbpre}melvefile_{$tb} where ".$where);
}
...
function DelFileAllTable($where){
	...
	if($public_r['filedatatbs'])
	{
		...
		{
			//删除附件
			$filesql=$Elves->query("select filename,path,classid,fpath from {$dbtbpre}melvefile_".$dtbr[$i]." where ".$where);
			while($filer=$Elves->fetch($filesql))
			{
				DoDelFile($filer);
			}
			$Elves->query("delete from {$dbtbpre}melvefile_".$dtbr[$i]." where ".$where);
		}
	}
}
...
function DelPlAllTable($where){
	...
	if($public_r['pldatatbs'])
	{
		...
		for($i=1;$i<$count;$i++)
		{
			$Elves->query("delete from {$dbtbpre}melvepl_".$pldtbr[$i]." where ".$where);
		}
	}
}
...
function UpdateTheFile($id,$checkpass,$classid,$fstb=1){
	...
	$sql=$Elves->query("update {$dbtbpre}melvefile_{$fstb} set pubid='$pubid',classid='$classid',id='$id',cjid=0 where cjid='$checkpass'");
}
...
function UpdateTheFileEdit($classid,$id,$fstb=1){
	global $Elves,$dbtbpre;
	$pubid=ReturnInfoPubid($classid,$id);
	$sql=$Elves->query("update {$dbtbpre}melvefile_{$fstb} set pubid='$pubid',cjid=0 where id='$id' and classid='$classid'");
}
...
function GetInfoTranFstb($classid,$id,$fstb){
	...
	if($id)
	{
		...
		$index_r=$Elves->fetch1("select id,classid,checked from {$dbtbpre}elve_".$class_r[$classid]['tbname']."_index where id='$id' limit 1");
		if(!$index_r['id'])
		{
			return $public_r['filedeftb'];
		}
		...
		$infor=$Elves->fetch1("select fstb from ".$infotb." where id='$id' limit 1");
		$fstb=$infor['fstb'];
	}
	elseif($fstb)
	{
...
function UpdateTheIspic($classid,$id,$checked){
	global $Elves,$dbtbpre,$class_r;
	$infotb=empty($checked)?$dbtbpre.'elve_'.$class_r[$classid][tbname].'_check':$dbtbpre.'elve_'.$class_r[$classid][tbname];
	$r=$Elves->fetch1("select titlepic,ispic from ".$infotb." where id='$id' limit 1");
	$ispic=$r['titlepic']?1:0;
	if($ispic<>$r['ispic'])
	{
		$Elves->query("update ".$infotb." set ispic='$ispic' where id='$id'");
	}
}
...
function GetFpicToTpic($classid,$id,$num=1,$getfirsttitlespic=0,$swidth=0,$sheight=0,$fstb=1){
	...
	$num=(int)$num;
	$num=$num-1;
	$picr=$Elves->fetch1("select fileid,filename,path,id,classid,no,fpath from {$dbtbpre}melvefile_{$fstb} where pubid='$pubid' and type=1 order by fileid limit $num,1");
	$firsttitlepic="";
	if($picr['fileid'])
	{
...
function UpdateImgNexturl($classid,$id,$checked=1){
	global $Elves,$dbtbpre,$class_r,$public_r,$emod_r;
	$mid=$class_r[$classid][modid];
	...
	$infotbname=$checked?$dbtbpre.'elve_'.$tbname:$dbtbpre.'elve_'.$tbname.'_check';
	//分页字段
	$tbdataf=strstr($emod_r[$mid]['tbdataf'],','.$pf.',')?1:0;
	if($tbdataf)
	{
		$r=$Elves->fetch1("select id,classid,titleurl,groupid,newspath,filename,stb from ".$infotbname." where id='$id'");
		$infodatatbname=$checked?$dbtbpre.'elve_'.$tbname.'_data_'.$r[stb]:$dbtbpre.'elve_'.$tbname.'_check_data';
		$finfor=$Elves->fetch1("select ".$pf." from ".$infodatatbname." where id='$id'");
		$r[$pf]=$finfor[$pf];
	}
	else
	{
		$r=$Elves->fetch1("select id,classid,titleurl,groupid,newspath,filename,".$pf." from ".$infotbname." where id='$id'");
	}
	//存文本
	if($stf&&$stf==$pf)
	{
	...
	if($tbdataf)
	{
		$Elves->query("update ".$infodatatbname." set ".$pf."='$newstext' where id='$id'");
	}
	else
	{
		$Elves->query("update ".$infotbname." set ".$pf."='$newstext' where id='$id'");
	}
}
...
function GetKeyid($keyboard,$classid,$id,$link_num){
	global $Elves,$public_r,$class_r,$fun_r,$dbtbpre,$eyh_r,$etable_r;
	if($keyboard)
	{
		if(empty($link_num))
		{
		...
		{
			$mr=$Elves->fetch1("select sonclass from {$dbtbpre}melvemod where mid='".$class_r[$classid][modid]."'");
		...
		$keyid="";
		$first=0;
		$key_sql=$Elves->query("select id from {$dbtbpre}elve_".$class_r[$classid][tbname]." where ".$yhadd.$add.$where." and id<>$id order by newstime desc limit $link_num");
		while($link_r=$Elves->fetch($key_sql))
		{
			if(empty($first))
			{
				$dh="";
...
function DelInfoSaveTxtfile($mid,$tbname,$where){
	global $Elves,$dbtbpre,$public_r,$class_r,$emod_r;
	if(empty($where))
	{
		return '';
	}
	...
	{
		//已审核
		$txtsql=$Elves->query("select ".$savetxtf." from {$dbtbpre}elve_".$tbname." where ".$where);
		while($txtr=$Elves->fetch($txtsql))
		{
		...
		//未审核
		$txtsql=$Elves->query("select ".$savetxtf." from {$dbtbpre}elve_".$tbname."_check where ".$where);
		while($txtr=$Elves->fetch($txtsql))
		{
			$newstextfile=$txtr[$savetxtf];
			//$txtr[$savetxtf]=GetTxtFieldText($txtr[$savetxtf]);
		...
		//归档
		$txtsql=$Elves->query("select ".$savetxtf." from {$dbtbpre}elve_".$tbname."_doc where ".$where);
		while($txtr=$Elves->fetch($txtsql))
		{
			$newstextfile=$txtr[$savetxtf];
			//$txtr[$savetxtf]=GetTxtFieldText($txtr[$savetxtf]);
...
function DelSingleInfoOtherData($classid,$id,$r,$delfile=0,$delpl=0){
	...
	$pubid=ReturnInfoPubid($classid,$id);
	//删除其它表记录
	$Elves->query("delete from {$dbtbpre}melvewfinfo where id='$id' and classid='$classid'");
	$Elves->query("delete from {$dbtbpre}melvewfinfolog where id='$id' and classid='$classid'");
	$Elves->query("delete from {$dbtbpre}melveinfovote where pubid='$pubid'");
	$Elves->query("delete from {$dbtbpre}melvediggips where id='$id' and classid='$classid'");
	$Elves->query("delete from {$dbtbpre}melveztinfo where id='$id' and classid='$classid'");
	if($delfile==0)
	{
...
function DelMoreInfoOtherData($classid,$delfile=0,$delpl=0){
	global $Elves,$dbtbpre,$public_r,$class_r,$emod_r;
	//删除其它表记录
	$Elves->query("delete from {$dbtbpre}melvewfinfo where classid='$classid'");
	$Elves->query("delete from {$dbtbpre}melvewfinfolog where classid='$classid'");
	$Elves->query("delete from {$dbtbpre}melveinfovote where classid='$classid'");
	$Elves->query("delete from {$dbtbpre}melvediggips where classid='$classid'");
	$Elves->query("delete from {$dbtbpre}melveztinfo where classid='$classid'");
	//附件
	if($delfile==0)
	{
...
function UpdateSingleInfoOtherData($classid,$id,$to_classid,$r,$updatefile=0,$updatepl=0){
	...
	$pubid=ReturnInfoPubid($classid,$id);
	//更新其它表记录
	$Elves->query("update {$dbtbpre}melvewfinfo set classid='$to_classid' where id='$id' and classid='$classid'");
	$Elves->query("update {$dbtbpre}melvewfinfolog set classid='$to_classid' where id='$id' and classid='$classid'");
	$Elves->query("update {$dbtbpre}melveinfovote set classid='$to_classid' where pubid='$pubid'");
	$Elves->query("update {$dbtbpre}melvediggips set classid='$to_classid' where id='$id' and classid='$classid'");
	$Elves->query("update {$dbtbpre}melveztinfo set classid='$to_classid' where id='$id' and classid='$classid'");
	//附件
	if($updatefile==0)
	{
		$Elves->query("update {$dbtbpre}melvefile_".$r['fstb']." set classid='$to_classid' where pubid='$pubid'");
	}
	//评论
	if($updatepl==0)
	{
		$Elves->query("update {$dbtbpre}melvepl_".$r['restb']." set classid='$to_classid' where pubid='$pubid'");
	}
}
...
function UpdateMoreInfoOtherData($classid,$to_classid,$updatefile=0,$updatepl=0){
	global $Elves,$dbtbpre,$public_r,$class_r,$emod_r;
	//更新其它表记录
	$Elves->query("update {$dbtbpre}melvewfinfo set classid='$to_classid' where classid='$classid'");
	$Elves->query("update {$dbtbpre}melvewfinfolog set classid='$to_classid' where classid='$classid'");
	$Elves->query("update {$dbtbpre}melveinfovote set classid='$to_classid' where classid='$classid'");
	$Elves->query("update {$dbtbpre}melvediggips set classid='$to_classid' where classid='$classid'");
	$Elves->query("update {$dbtbpre}melveztinfo set classid='$to_classid' where classid='$classid'");
	//附件
	if($updatefile==0)
	{
		if($public_r['filedatatbs'])
		{
			...
			for($i=1;$i<$count-1;$i++)
			{
				$Elves->query("update {$dbtbpre}melvefile_".$dtbr[$i]." set classid='$to_classid' where classid='$classid'");
			}
		}
	}
	//评论
	if($updatepl==0)
	{
		if($public_r['pldatatbs'])
		{
			...
			for($i=1;$i<$count;$i++)
			{
				$Elves->query("update {$dbtbpre}melvepl_".$pldtbr[$i]." set classid='$to_classid' where classid='$classid'");
			}
		}
	}
}
...
function DelNewsTheFile($id,$classid,$fstb='1',$delpl=0,$restb='1'){
	...
	$pubid=ReturnInfoPubid($classid,$id);
	$i=0;
	$sql=$Elves->query("select classid,filename,path,fpath from {$dbtbpre}melvefile_{$fstb} where pubid='$pubid'");
	while($r=$Elves->fetch($sql))
	{
		$i=1;
		DoDelFile($r);
	...
	if($i)
	{
		$Elves->query("delete from {$dbtbpre}melvefile_{$fstb} where pubid='$pubid'");
	}
	//删除评论
	if($delpl==0)
	{
		$Elves->query("delete from {$dbtbpre}melvepl_{$restb} where pubid='$pubid'");
	}
}
...
function DelNewsFile($filename,$newspath,$classid,$newstext,$groupid=0){
	...
	$r=explode("[!--Elvesnews.page--]",$newstext);
	for($i=1;$i<=count($r);$i++)
	{
		if(strstr($filename,'/'))
		{
			DelPath(elve_PATH.$iclasspath.$mynewspath.ReturnInfoSPath($filename));
		}
		else
		{
			if($i==1)
			{
				$file=elve_PATH.$iclasspath.$mynewspath.$filename.$filetype;
			}
			else
			{
				$file=elve_PATH.$iclasspath.$mynewspath.$filename."_".$i.$filetype;
			}
			DelFiletext($file);
		}
	}
...
function DelZtcFile($cid){
	global $Elves,$dbtbpre,$class_zr;
	$cr=$Elves->fetch1("select ztid,islist,maxnum,tnum,ttype from {$dbtbpre}melvezttype where cid='$cid'");
	if(!$cr['ztid'])
	{
		return '';
	}
	...
	//数量
	$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}melveztinfo where cid='$cid'");
...
function RepImg($text,$copyflash){
	...
	$exp2="[/--copyimg--]";
	//去掉图片链接
	if($elve_config['sets']['saveurlimgclearurl']==1)
	{
...
function CopyImg($text,$copyimg,$copyflash,$classid,$qz,$username,$theid,$cjid,$mark,$fstb=1){
	global $Elves,$public_r,$cjnewsurl,$navtheid,$dbtbpre;
	if(empty($text))
	{return "";}
	$fstb=(int)$fstb;
	if($copyimg)
...
function GetMySmallImg($classid,$no,$insertfile,$filepath,$yname,$maxwidth,$maxheight,$name,$id,$cjid,$userid,$username,$modtype=0,$fstb=1){
	global $Elves,$dbtbpre,$public_r,$efileftp_fr;
	if(empty($yname))
	{
		return "";
    }
	...
	$filer=ResizeImage($yname,$name,$maxwidth,$maxheight,$public_r['spickill'],$filepath);
	if($filer['file'])
	{
...
//投票组合
function ReturnVote($votename,$votenum,$delvid,$vid,$melve=0){
	global $Elves,$dbtbpre;
	$f_exp="::::::";
	$r_exp="\r\n";
	$returnstr="";
...
//显示无限级栏目[增加栏目时]
function ShowClass_AddClass($adminclass,$obclassid,$bclassid,$exp,$modid,$melve=0,$addminfocid=''){
	global $Elves,$dbtbpre,$public_r;
	if(empty($bclassid))
	{
		$bclassid=0;
		...
		if($melve==2)
		{
			$modr=$Elves->fetch1("select sonclass from {$dbtbpre}melvemod where mid='$modid'");
			$addminfocid=$modr['sonclass'];
		}
    }
	else
	{$exp="&nbsp;&nbsp;".$exp;}
	$sql=$Elves->query("select classid,classname,bclassid,islast,openadd,modid,sonclass from {$dbtbpre}melveclass where bclassid='$bclassid' and wburl='' order by myorder,classid");
	$returnstr="";
	while($r=$Elves->fetch($sql))
	{
		//投稿显示
		...
		if($r[islast])
		{
			if(empty($melve)||$melve==2||$melve==3||$melve==4)
			{
		...
		if(empty($r[islast]))
		{
			$returnstr.=ShowClass_AddClass($adminclass,$obclassid,$r[classid],$exp,$modid,$melve,$addminfocid);
		}
	}
	return $returnstr;
}
...
//替换字符
function RepInfoZZ($text,$exp,$melve=0){
...
function GetListtempMid($tempid){
	global $Elves;
	$r=$Elves->fetch1("select modid from ".GetTemptb("melvelisttemp")." where tempid='$tempid'");
	return $r[modid];
}

//替换模板JS地址
function RepTemplateJsUrl($temp,$classid,$melve=0){
	...
	if(!empty($classid))
	{
		$path=$melve==1?'[!--news.url--]d/js/class/zt[!--self.classid--]_':'[!--news.url--]d/js/class/class[!--self.classid--]_';
...
function GetListTemp($tempid){
	global $Elves;
	$r=$Elves->fetch1("select temptext,subnews,listvar,rownum,showdate,modid,subtitle,docode from ".GetTemptb("melvelisttemp")." where tempid='$tempid'");
...
function GetClassTemp($tempid){
	global $Elves;
	$r=$Elves->fetch1("select temptext from ".GetTemptb("melveclasstemp")." where tempid='$tempid'");
	return $r['temptext'];
}
...
function GetClassText($classid){
	global $Elves,$dbtbpre;
	$r=$Elves->fetch1("select classtext from {$dbtbpre}melveclassadd where classid='$classid'");
	return $r['classtext'];
}
...
function GetZtText($ztid){
	global $Elves,$dbtbpre;
	$r=$Elves->fetch1("select classtext from {$dbtbpre}melveztadd where ztid='$ztid'");
	return $r['classtext'];
}
...
function GetZtcText($cid){
	global $Elves,$dbtbpre;
	$r=$Elves->fetch1("select classtext from {$dbtbpre}melvezttypeadd where cid='$cid'");
	return $r['classtext'];
}
...
function GetIndextemp(){
	...
	{
		$r=$Elves->fetch1("select temptext from {$dbtbpre}melveindexpage where tempid='".$public_r['indexpageid']."'");
		return $r['temptext'];
	}
	$r=$Elves->fetch1("select indextemp from ".GetTemptb("melvepubtemp")." limit 1");
	return $r['indextemp'];
}

//取得内容模板
function GetNewsTemp($newstempid){
	global $Elves,$public_r;
	$r=$Elves->fetch1("select temptext,showdate from ".GetTemptb("melvenewstemp")." where tempid='$newstempid'");
...
function GetTheJstemp($tempid){
	global $Elves;
	$r=$Elves->fetch1("select temptext,showdate,modid,subnews,subtitle from ".GetTemptb("melvejstemp")." where tempid='$tempid'");
	return $r;
}
...
function ReplaceTempvar($temp){
	...
	if(empty($temp))
	{return $temp;}
	$sql=$Elves->query("select myvar,varvalue from ".GetTemptb("melvetempvar")." where isclose=0 order by myorder desc,varid");
	while($r=$Elves->fetch($sql))
	{
...
//栏目页替换公共标记
function Class_ReplaceSvars($temp,$url,$classid,$title,$key,$des,$classimg,$add,$melve=0){
	global $public_r,$class_r,$class_zr;
...
function Info_ReplaceSvars($temp,$url,$classid,$title,$key,$des){
	...
	$temp=str_replace('[!--pagekey--]','<?=$elve_gr[keyboard]?>',$temp);
	$temp=str_replace('[!--pagedes--]','<?=$grpagetitle?>',$temp);
	$temp=str_replace('[!--self.classid--]','<?=$elve_gr[classid]?>',$temp);
	$bclassid=$class_r[$classid]['bclassid'];
...
function AddCheckClassLevel($classid,$groupid,$classpath){
	...
	$include1=$include;
	$include.='core/class/CheckClassLevel.php';
	$addlevel="<?php
...
function ReClassBdInfo($classid){
	global $Elves,$dbtbpre;
	$cr=$Elves->fetch1("select classid,bdinfoid from {$dbtbpre}melveclass where classid='$classid'");
	...
	$classfile=elve_PATH.ReturnSaveClassPath($classid,1);
...
function DoSpReFile($r,$spid=0){
	...
	if($spid)
	{
		$r=$Elves->fetch1("select varname,refile,spfile,spfileline,spfilesub from {$dbtbpre}melvesp where spid='$spid' limit 1");
	}
	if(!$r['refile'])
	{
		return '';
	...
	ob_end_clean();
	$filename=elve_PATH.$r['spfile'];
	WriteFiletext($filename,$string);
}

//标签替换
function NewsBq($classid,$indextext,$melve=0,$doing=0){
	global $Elves,$dbtbpre,$public_r,$emod_r,$class_r,$class_zr,$fun_r,$navclassid,$navinfor,$class_tr,$level_r,$etable_r;
	...
	if($melve==0)//生成大栏目
	{
		...
		$cr=$Elves->fetch1("select classpagekey,intro,classimg,cgroupid".$cf." from {$dbtbpre}melveclass where classid='$classid'");
		if(!empty($cf))
		{
		...
		$onclick="<script src=".$public_r[newsurl]."core/public/onclick/?melve=doclass&classid=$classid></script>";
		$truefile=elve_PATH.ReturnSaveClassPath($classid,1);
		$file=elve_PATH.'core/data/tmp/class'.$classid.'.php';
	...
	elseif($melve==3)//专题
	{
		...
		$cr=$Elves->fetch1("select ztpagekey,intro,ztimg".$cf." from {$dbtbpre}melvezt where ztid='$classid'");
		if(!empty($cf))
		{
		...
		$pagedes=ehtmlspecialchars($cr['intro']);
		$classimg=$cr['ztimg'];
		$onclick="<script src=".$public_r[newsurl]."core/public/onclick/?melve=dozt&ztid=$classid></script>";
		$truefile=elve_PATH.ReturnSaveZtPath($classid,1);
		$file=elve_PATH.'core/data/tmp/zt'.$classid.'.php';
		$indextext=str_replace("[!--newsnav--]",$url,$indextext);//位置导航
		$indextext=Class_ReplaceSvars($indextext,$url,$classid,$pagetitle,$pagekey,$pagedes,$classimg,$add,1);
	}
	elseif($melve==4)//专题子类
	{
		$cr=$Elves->fetch1("select ztid,cname,ttype from {$dbtbpre}melvezttype where cid='$classid'");
		$GLOBALS['navclassid']=$classid;
		$GLOBALS['navinfor']['elvebid']=$cr['ztid'];
		...
		$onclick="<script src=".$public_r[newsurl]."core/public/onclick/?melve=dozt&ztid=$cr[ztid]></script>";
		$truefile=elve_PATH.ReturnSaveZtPath($cr['ztid'],0).'/type'.$classid.$cr['ttype'];
		$file=elve_PATH.'core/data/tmp/ztc'.$classid.'.php';
		$indextext=str_replace("[!--newsnav--]",$url,$indextext);//位置导航
		$indextext=Class_ReplaceSvars($indextext,$url,$classid,$pagetitle,$pagekey,$pagedes,$classimg,$add,1);
	}
	elseif($melve==1)//生成首页文件
	{
		$pr=$Elves->fetch1("select sitekey,siteintro,indexpagedt from {$dbtbpre}melvepublic limit 1");
		if($pr['indexpagedt'])
		{
			return '';
		}
...
function InfoNewsBq($classid,$indextext){
	...
	$file=elve_PATH.'core/data/tmp/temp'.$classid.'.php';
...
//标签替换3
function GetInfoNewsBq($classid,$newstemp_r,$elve_gr,$docheckrep){
	global $Elves,$dbtbpre,$public_r,$emod_r,$class_r,$class_zr,$fun_r,$navclassid,$navinfor,$class_tr,$level_r,$etable_r;
	if(!defined('ElvesCMSAdmin'))
	{
		$_GET['reallinfotime']=0;
	...
	if($_GET['reallinfotime'])
	{
		$file=elve_PATH.'core/data/tmp/tempnews'.$newstemp_r['tempid'].'_all.php';
	}
	else
	{
		$file=elve_PATH.'core/data/tmp/tempnews'.$newstemp_r['tempid'].'.php';
	}
	//变量处理
	$grurl=ReturnClassLink($elve_gr['classid']);//导航
	$grpagetitle=ehtmlspecialchars($elve_gr['title']);
	$grbclassid=$class_r[$elve_gr['classid']]['bclassid'];
	$grtitleurl=sys_ReturnBqTitleLink($elve_gr);
	$grclassurl=sys_ReturnBqClassname($elve_gr,9);
	...
	//替换标签
	$newstemp_r['temptext']=DoRepelveLoopBq($newstemp_r['temptext']);
	$newstemp_r['temptext']=RepBq($newstemp_r['temptext']);
	//替换变量
	$indextext=GetHtmlRepVar($newstemp_r,$elve_gr['classid']);
	//写文件
	WriteFiletext($file,AddCheckViewTempCode().$indextext);
	//读取文件内容
    ob_start();
...
//标签替换4
function DtNewsBq($classid,$indextext,$elve=0){
	global $Elves,$dbtbpre,$public_r,$emod_r,$class_r,$class_zr,$fun_r,$navclassid,$navinfor,$class_tr,$level_r,$etable_r;
	$cachetime=$elve==1?$public_r['dtncachetime']:$public_r['dtcachetime'];
	$file=elve_PATH.'core/data/tmp/dt_temp'.$classid.'.php';
...
function RepBq($indextext){
	global $Elves,$dbtbpre;
	$sql=$Elves->query("select bq,funname from {$dbtbpre}melvebq where isclose=0 order by bqid");
	while($r=$Elves->fetch($sql))
	{
...
//替换灵动标签
function DoRepelveLoopBq($temp){
	$yzz="/\[e:loop={(.+?)}\](.+?)\[\/e:loop\]/is";
	$xzz="<?php
\$bqno=0;
\$elve_bq_sql=sys_ReturnelveLoopBq(\\1);
if(\$elve_bq_sql){
while(\$bqr=\$Elves->fetch(\$elve_bq_sql)){
\$bqsr=sys_ReturnelveLoopStext(\$bqr);
\$bqno++;
?>\\2<?php
}
}
?>";
	...
	$temp=DoRepelveIndexLoopBq($temp);
	return $temp;
}

//替换索引灵动标签
function DoRepelveIndexLoopBq($temp){
	$yzz="/\[e:indexloop={(.+?)}\](.+?)\[\/e:indexloop\]/is";
	$xzz="<?php
\$bqno=0;
\$elve_bq_sql=sys_ReturnelveIndexLoopBq(\\1);
if(\$elve_bq_sql){
while(\$indexbqr=\$Elves->fetch(\$elve_bq_sql)){
if(empty(\$class_r[\$indexbqr['classid']]['tbname'])){continue;}
\$bqr=\$Elves->fetch1(\"select * from {\$dbtbpre}elve_\".\$class_r[\$indexbqr['classid']]['tbname'].\" where id='\$indexbqr[id]'\");
\$bqsr=sys_ReturnelveLoopStext(\$bqr);
\$bqno++;
?>\\2<?php
}
...
//生成信息列表
function ListHtml($classid,$fields,$melve=0,$userlistr=""){
	global $Elves,$dbtbpre,$emod_r,$public_r,$class_r,$class_zr,$fun_r,$class_tr,$level_r,$etable_r;
	//不生成栏目
	if(($melve==0||$melve==3)&&($class_r[$classid]['listdt']||$class_r[$classid]['wburl']||strstr($public_r['nreclass'],','.$classid.',')))
	{
		return '';
	}
	$GLOBALS['navclassid']=$classid;
	...
	if($melve==0)//子栏目列表
	{
		...
		$selfclassid=$classid;
		$domelve=0;
		$cr=$Elves->fetch1("select classpagekey,intro,classimg,cgroupid,repagenum,bdinfoid from {$dbtbpre}melveclass where classid='$classid'");
		//绑定信息
		...
		$query="select ".ReturnSqlListF($mid)." from {$dbtbpre}elve_".$class_r[$classid][tbname]." where ".$yhadd."classid='$classid' order by ".ReturnSetTopSql('list').$addorder.$limit;
		$totalquery="select count(*) as total from {$dbtbpre}elve_".$class_r[$classid][tbname]." where ".$yhadd."classid='$classid'";//统计
	...
	elseif($melve==5)//标题分类列表
	{
		...
		$selfclassid=$classid;
		$domelve=1;
		$cr=$Elves->fetch1("select tnum,listtempid,maxnum,reorder,timg,intro,pagekey,listdt,repagenum from {$dbtbpre}melveinfotype where typeid='$classid'");
		//页面
		...
		$query="select ".ReturnSqlListF($mid)." from {$dbtbpre}elve_".$tbname." where ".$yhadd."ttid='$classid' order by ".$addorder.$limit;
		$totalquery="select count(*) as total from {$dbtbpre}elve_".$tbname." where ".$yhadd."ttid='$classid'";//统计
	...
	elseif($melve==3)//大栏目列表
	{
		...
		$selfclassid=$classid;
		$domelve=0;
		$cr=$Elves->fetch1("select classpagekey,intro,classimg,cgroupid,repagenum from {$dbtbpre}melveclass where classid='$classid'");
		$mid=$class_r[$classid][modid];
		//权限
		if($cr['cgroupid'])
		{
		...
		$query="select ".ReturnSqlListF($mid)." from {$dbtbpre}elve_".$class_r[$classid][tbname]." where ".$yhadd."(".$whereclass.") order by ".ReturnSetTopSql('list').$addorder.$limit;
		$totalquery="select count(*) as total from {$dbtbpre}elve_".$class_r[$classid][tbname]." where ".$yhadd."(".$whereclass.")";//统计
		$doclasspath=ReturnSaveClassPath($classid,0);
		$dopath=elve_PATH.$doclasspath."/";
		if(empty($class_r[$classid][classurl]))
		{
	...
	$listtemp=Class_ReplaceSvars($listtemp,$url,$selfclassid,$pagetitle,$pagekey,$pagedes,$classimg,$add,$domelve);
...
//生成索引类信息列表
function ListHtmlIndex($classid,$fields,$melve=0,$userlistr=""){
	global $Elves,$dbtbpre,$emod_r,$public_r,$class_r,$class_zr,$fun_r,$class_tr,$level_r,$etable_r;
	$GLOBALS['navclassid']=$classid;
	$dofile="index";
	$classlevel='';
	...
	if($melve==0)//专题列表
	{
		$selfclassid=$classid;
		$domelve=1;
		$cr=$Elves->fetch1("select ztpagekey,intro,ztimg,classtempid from {$dbtbpre}melvezt where ztid='$classid'");
		//页面
		$pagetitle=ehtmlspecialchars($class_zr[$classid][ztname]);
		$pagekey=ehtmlspecialchars($cr['ztpagekey']);
		$pagedes=ehtmlspecialchars($cr['intro']);
		...
		$query="select ztid,cid,classid,id,isgood from {$dbtbpre}melveztinfo where ".$yhadd."ztid='$classid' order by ".$addorder.$limit;
		$totalquery="select count(*) as total from {$dbtbpre}melveztinfo where ".$yhadd."ztid='$classid'";//统计
	...
	elseif($melve==1)//专题子类列表
	{
		$selfclassid=$classid;
		$domelve=1;
		$cr=$Elves->fetch1("select ztid,cname,islist,listtempid,maxnum,tnum,reorder,ttype from {$dbtbpre}melvezttype where cid='$classid'");
		$GLOBALS['navinfor']['elvebid']=$cr['ztid'];
		//页面
		$pagetitle=ehtmlspecialchars($cr['cname']);
		$pagekey=ehtmlspecialchars($cr['cname']);
		...
		$query="select ztid,cid,classid,id,isgood from {$dbtbpre}melveztinfo where ".$yhadd."cid='$classid' order by ".$addorder.$limit;
		$totalquery="select count(*) as total from {$dbtbpre}melveztinfo where ".$yhadd."cid='$classid'";//统计
		...
		$classname=$cr['cname'];
		$lencord=$cr['tnum'];//记录数
		$onclick="<script src='".$public_r[newsurl]."core/public/onclick/?melve=dozt&ztid=$ztid'></script>";
		//模板
		$listtempid=$cr['listtempid'];
	}
	elseif($melve==4)//按sql语句生成列表
	...
	while($k=$Elves->fetch($sql))
	{
		...
		$infor=$Elves->fetch1("select * from {$dbtbpre}elve_".$class_r[$k[classid]][tbname]." where id='$k[id]' limit 1");
		if(empty($infor['id']))
		{
			continue;
		}
...
function DoGetHtml($classid,$id){
	global $Elves,$class_r,$public_r,$dbtbpre;
	...
	$r=$Elves->fetch1("select * from {$dbtbpre}elve_".$tbname." where id='$id'");
...
function GetHtmlRepVar($tempr,$classid){
	global $public_r,$class_r,$class_zr,$fun_r,$Elves,$dbtbpre,$emod_r,$class_tr,$level_r,$etable_r;
	$mid=$class_r[$classid]['modid'];
	$tbname=$class_r[$classid][tbname];
	...
	$newstempstr=str_replace('[!--pagekey--]','<?=$elve_gr[keyboard]?>',$newstempstr);
	$newstempstr=str_replace('[!--pagedes--]','<?=$grpagetitle?>',$newstempstr);
	$newstempstr=str_replace('[!--self.classid--]','<?=$elve_gr[classid]?>',$newstempstr);
	...
	for($i=1;$i<$fcount;$i++)
	{
		...
		{
			$value='ReturnDownSoftHtml($elve_gr)';
		}
		elseif($f=='onlinepath')//观看地址
		{
			$value='ReturnOnlinepathHtml($elve_gr)';
		}
		elseif($f=='morepic')//图片集
		{
			$value='ReturnMorepicpathHtml($elve_gr)';
		}
		...
		{
			$value='empty('.$value.')?$public_r[newsurl].\'core/data/images/notimg.gif\':'.$value;
		}
		elseif($f=='title')//标题
		{
		}
	...
	//固定变量
	$newstempstr=str_replace('[!--id--]','<?=$elve_gr[id]?>',$newstempstr);
	$newstempstr=str_replace('[!--classid--]','<?=$elve_gr[classid]?>',$newstempstr);
	$newstempstr=str_replace('[!--class.name--]','<?=$class_r[$elve_gr[classid]][classname]?>',$newstempstr);
	$newstempstr=str_replace('[!--ttid--]','<?=$elve_gr[ttid]?>',$newstempstr);
	$newstempstr=str_replace('[!--tt.name--]','<?=$class_tr[$elve_gr[ttid]][tname]?>',$newstempstr);
	$newstempstr=str_replace('[!--onclick--]','<?=$elve_gr[onclick]?>',$newstempstr);
	$newstempstr=str_replace('[!--userfen--]','<?=$elve_gr[userfen]?>',$newstempstr);
	$newstempstr=str_replace('[!--username--]','<?=$elve_gr[username]?>',$newstempstr);
	//带链接的用户名
	$newstempstr=str_replace('[!--linkusername--]','<?=$elve_gr[ismember]==1&&$elve_gr[userid]?\'<a href="\'.$public_r[newsurl].\'core/space/?userid=\'.$elve_gr[userid].\'" target=_blank>\'.$elve_gr[username].\'</a>\':$elve_gr[username]?>',$newstempstr);
	$newstempstr=str_replace('[!--userid--]','<?=$elve_gr[userid]?>',$newstempstr);
	//相关链接
	$keyboardtext='<?=GetKeyboard($elve_gr[keyboard],$elve_gr[keyid],$elve_gr[classid],$elve_gr[id],$class_r[$elve_gr[classid]][link_num])?>';
	$newstempstr=str_replace('[!--other.link--]',$keyboardtext,$newstempstr);
	$newstempstr=str_replace('[!--plnum--]','<?=$elve_gr[plnum]?>',$newstempstr);
	$newstempstr=str_replace('[!--totaldown--]','<?=$elve_gr[totaldown]?>',$newstempstr);
	$newstempstr=str_replace('[!--keyboard--]','<?=$elve_gr[keyboard]?>',$newstempstr);
	//链接
	$newstempstr=str_replace('[!--titleurl--]','<?=$grtitleurl?>',$newstempstr);
	//点击
	$onclick='<?=\'<script src="\'.$public_r[newsurl].\'core/public/onclick/?melve=donews&classid=\'.$elve_gr[classid].\'&id=\'.$elve_gr[id].\'"></script>\'?>';
	...
	{
	$infonext='<?php
	$next_r=$Elves->fetch1("select isurl,titleurl,classid,id,title from {$dbtbpre}elve_".$class_r[$elve_gr[classid]][tbname]." where id>$elve_gr[id] and classid=\'$elve_gr[classid]\' order by id limit 1");
	if(empty($next_r[id]))
	...
	{
	$infopre='<?php
	$next_r=$Elves->fetch1("select isurl,titleurl,classid,id,title from {$dbtbpre}elve_".$class_r[$elve_gr[classid]][tbname]." where id<$elve_gr[id] and classid=\'$elve_gr[classid]\' order by id desc limit 1");
	if(empty($next_r[id]))
	{$infonext="<a href=\'".$grclassurl."\'>'.$fun_r['HaveNoNextLink'].'</a>";}
	else
	{
	...
	{
		$newstempstr=str_replace('[!--info.vote--]','<?=sys_GetInfoVote($elve_gr[classid],$elve_gr[id])?>',$newstempstr);
	}
	return $newstempstr;
}

//生成内容文件
function GetHtml($classid,$id,$add,$elve=0,$doall=0){
	global $public_r,$class_r,$class_zr,$fun_r,$Elves,$dbtbpre,$emod_r,$class_tr,$level_r,$etable_r;
	$mid=$class_r[$classid]['modid'];
	$tbname=$class_r[$classid][tbname];
	if(InfoIsInTable($tbname))//内部表
	...
	if($elve==0)//主表
	{
		$add=$Elves->fetch1("select ".ReturnSqlTextF($mid,1)." from {$dbtbpre}elve_".$tbname." where id='$id' limit 1");
	}
	$add['id']=$id;
	$add['classid']=$classid;
	if($add['isurl'])
	...
	//副表
	$addr=$Elves->fetch1("select ".ReturnSqlFtextF($mid)." from {$dbtbpre}elve_".$tbname."_data_".$add[stb]." where id='$add[id]' limit 1");
	$add=array_merge($add,$addr);
	//路径
	$iclasspath=ReturnSaveInfoPath($add[classid],$add[id]);
	$doclasspath=elve_PATH.$iclasspath;
	...
	$newstemp_r=$Elves->fetch1("select temptext,showdate from ".GetTemptb("melvenewstemp")." where tempid='$add[newstempid]' limit 1");
	...
	if($add[groupid]||$class_r[$add[classid]]['cgtoinfo'])
	{
		...
		$include1=$include;
		$include.='core/class/CheckLevel.php';
		$filetype='.php';
		$addlevel="<?php
		define('Elvescms','wm_chief');
	...
	if($pf&&strstr($add[$pf],$expage))//有分页
	{
		...
		for($j=1;$j<=$thispagenum;$j++)
		{
			...
			else
			{
				...
				$ti_r=explode('[/!--Elvesnews.page--]',$n_r[$k]);
				if(count($ti_r)>=2)
				{
					$ptitle=$ti_r[0];
					$n_r[$k]=$ti_r[1];
	...
	{
		$Elves->query("update {$dbtbpre}elve_".$class_r[$add[classid]][tbname]."_index set havehtml=1 where id='$add[id]' limit 1");
		$Elves->query("update {$dbtbpre}elve_".$class_r[$add[classid]][tbname]." set havehtml=1 where id='$add[id]' limit 1");
	}
}
...
function GetKeyboard($keyboard,$keyid,$classid,$id,$link_num){
	global $Elves,$public_r,$class_r,$fun_r,$dbtbpre;
	if($keyid&&$link_num)
	{
		$add="id in (".$keyid.")";
		$tr=$Elves->fetch1("select otherlinktemp,otherlinktempsub,otherlinktempdate from ".GetTemptb("melvepubtemp")." limit 1");//取得相关链接模板
		$temp_r=explode("[!--Elvesnews.listtemp--]",$tr[otherlinktemp]);
		$key_sql=$Elves->query("select id,newstime,title,isurl,titleurl,classid,titlepic from {$dbtbpre}elve_".$class_r[$classid][tbname]." where ".$add." order by newstime desc limit $link_num");
		while($link_r=$Elves->fetch($key_sql))
		{
...
function RepOtherTemp($temptext,$r,$tr){
	...
	if(empty($r[titlepic]))
	{
		$titlepic=$public_r[newsurl]."core/data/images/notimg.gif";
	}
	else
	{
		$titlepic=$r[titlepic];
...
//替换下载在线地址模板
function RepDownOnlinePathTemp($add,$downsofttemp,$pj,$showdown_r,$elve){
	global $public_r,$level_r,$fun_r;
	if($elve==0)//下载
	{
		$downurl=$public_r[newsurl]."core/DownSys/DownSoft/?classid=$add[classid]&id=$add[id]&pathid=$pj";
	}
	else//在线
	{
		$downurl=$public_r[newsurl]."core/DownSys/play/?classid=$add[classid]&id=$add[id]&pathid=$pj";
	}
...
function ReturnMorepicpathHtml($add){
	...
	for($pj=0;$pj<count($path_r);$pj++)
    {
		...
		$picpath.="<td align=center><a href='".$public_r[newsurl]."core/ViewImg/index.html?url=".$showdown_r[1]."' target=_blank><img src='".$showdown_r[0]."'".$width.$height." border=0>".$name."</a></td>";
		//分割
        if($p%$line==0)
		{
			$picpath.='</tr>';
...
//生成js
function GetNewsJs($classid,$line,$sub,$showdate,$melve=0,$tempr){
	global $Elves,$public_r,$class_r,$class_tr,$emod_r,$etable_r,$dbtbpre,$eyh_r;
	if(empty($line))
	{
		$line=10;
	}
	...
	//栏目
	if($melve==0||$melve==1||$melve==2||$melve==9||$melve==12||$melve==15)
	{
	...
	elseif($melve==25||$melve==26||$melve==27||$melve==28||$melve==29||$melve==30)//标题分类
	{
		$where="ttid='$classid'";
		$mid=$class_tr[$classid][mid];
		$tbname=$emod_r[$mid][tbname];
	...
	$allpath=elve_PATH.'d/js/js/';
	$ttpath=elve_PATH.'d/js/class/tt'.$classid.'_';
	$classpath=elve_PATH.'d/js/class/class'.$classid.'_';
	$query='';
	$qand=' and ';
	if($melve==0)//栏目最新
	{
	...
	$query='select '.ReturnSqlListF($mid).' from '.$dbtbpre.'elve_'.$tbname.$query.' order by '.ReturnSetTopSql('js').$order.' desc limit '.$line;
	$sql=$Elves->query($query);
	//取得js模板
	$tempr[temptext]=str_replace('[!--news.url--]',$public_r[newsurl],$tempr[temptext]);
	$temp_r=explode("[!--Elvesnews.listtemp--]",$tempr[temptext]);
	$no=1;
	while($r=$Elves->fetch($sql))
	{
		$r[oldtitle]=$r[title];
...
function ReUserjs($jsr,$addpath){
	...
	$temp_r=explode("[!--Elvesnews.listtemp--]",$jstemptext[temptext]);
	$query=$jsr[jssql];
...
//刷新信息列表
function ReListHtml($classid,$melve=0){
	...
	$r=$Elves->fetch1("select classtempid,islist from {$dbtbpre}melveclass where classid='$classid'");
...
function GetPageTemp($tempid){
	global $Elves;
	$r=$Elves->fetch1("select temptext from ".GetTemptb("melvepagetemp")." where tempid='$tempid'");
	return $r['temptext'];
}
...
function GetSearch($mid=0){
	...
	//取得模板
	$tr=$Elves->fetch1("select searchtemp,searchjstemp,searchjstemp1 from ".GetTemptb("melvepubtemp")." limit 1");
	//返回栏目搜索
	...
	//$options="<script src=".$public_r[newsurl]."core/data/fc/searchclass.js></script>";
...
function GetPlTempPage($pltempid=0){
	global $Elves,$public_r,$fun_r,$dbtbpre;
	$pl_t_filename=elve_PATH.'core/data/template/pltemp.txt';
	...
	$pagekey=$pagetitle;
	$pagedes=$pagetitle;
	$pr=$Elves->fetch1("select plf from {$dbtbpre}melvepl_set limit 1");
	//回车字段
	$tobrf=',';
	$plfsql=$Elves->query("select f from {$dbtbpre}melveplf where ftype='VARCHAR' or ftype='TEXT' or ftype='MEDIUMTEXT' or ftype='LONGTEXT'");
	while($plfr=$Elves->fetch($plfsql))
	{
		$tobrf.=$plfr[f].',';
	}
	$pr['pltobrf']=$tobrf;
	...
	$ptsql=$Elves->query("select tempid,temptext from ".GetTemptb("melvepltemp").$where);
	while($ptr=$Elves->fetch($ptsql))
	{
		$plfiletemp=$yplfiletemp;
		$pl_filename=elve_PATH.'core/data/filecache/template/pl'.$ptr[tempid].'.php';
		$pltemp=$ptr['temptext'];
		//头部变量
		...
		//登录
		$pltemp=str_replace("[!--key.url--]",$public_r[newsurl]."core/ShowKey/?v=pl",$pltemp);
		$pltemp=str_replace("[!--lusername--]","<?=\$lusername?>",$pltemp);
		$pltemp=str_replace("[!--lpassword--]","<?=\$lpassword?>",$pltemp);
		//列表变量
		$listtemp_r=explode("[!--Elvesnews.listtemp--]",$pltemp);
		$plfiletemp=str_replace("<!--Elves.listtemp.top-->",$listtemp_r[0],$plfiletemp);
		$plfiletemp=str_replace("<!--Elves.listtemp.footer-->",$listtemp_r[2],$plfiletemp);
		//列表中间
		...
		$plfiletemp=str_replace("<!--Elves.listtemp.center-->",$listtemp_center,$plfiletemp);
		WriteFiletext($pl_filename,$plfiletemp);
	}
}

//替换评论字段
function ReplacePlListVars($temp,$r,$pr,$elve=0){
	$fr=explode(',',$pr['plf']);
	$count=count($fr)-1;
	for($i=1;$i<$count;$i++)
	{
...
function GetPlJsPage(){
	...
	$pr['pltobrf']=$tobrf;
	//取得评论JS模板
	$pl_r=$Elves->fetch1("select pljstemp from ".GetTemptb("melvepubtemp")." limit 1");
	...
	$listtemp_r=explode("[!--Elvesnews.listtemp--]",$pljstemp);
	$pltemp=str_replace("<!--Elves.listtemp.top-->",$listtemp_r[0],$pltemp);
	$pltemp=str_replace("<!--Elves.listtemp.footer-->",$listtemp_r[2],$pltemp);
	//列表中间
	$listtemp_center=str_replace("[!--plid--]","<?=\$r[plid]?>",$listtemp_r[1]);
	$listtemp_center=str_replace("[!--pltext--]","<?=\$saytext?>",$listtemp_center);
	$listtemp_center=str_replace("[!--pltime--]","<?=\$saytime?>",$listtemp_center);
	...
	$pltemp=str_replace("<!--Elves.listtemp.center-->",$listtemp_center,$pltemp);
...
function ReGbooktemp(){
	...
	//取得留言页面模板
	$pr=$Elves->fetch1("select gbooktemp from ".GetTemptb("melvepubtemp")." limit 1");
	$url="<?=\$url?>";
	$pagetitle="<?=\$bname?>";
	...
	$listtemp_r=explode("[!--Elvesnews.listtemp--]",$pr['gbooktemp']);
	$gbtemp=str_replace("<!--Elves.listtemp.top-->",$listtemp_r[0],$gbtemp);
	$gbtemp=str_replace("<!--Elves.listtemp.footer-->",$listtemp_r[2],$gbtemp);
	//---列表中间
	//处理回复
	$restart="
<?
...
?>";
	...
	$gbtemp=str_replace("<!--Elves.listtemp.center-->",$listtemp_center,$gbtemp);
	WriteFiletext($file,$gbtemp);
}

//更新控制面板模板
function ReCptemp(){
	global $Elves,$public_r,$dbtbpre,$fun_r;
	$pr=$Elves->fetch1("select cptemp from ".GetTemptb("melvepubtemp")." limit 1");
	$url="<?=\$url?>";
	$pagetitle="<?=defined('Elvescms')?\$public_diyr[pagetitle]:'".$fun_r['membercp']."'?>";
	$temptext=ReplaceSvars($pr['cptemp'],$url,0,$pagetitle,$pagetitle,$pagetitle,$add,1);
	//生成头尾文件
	$r=explode("[!--Elvesnews.template--]",$temptext);
...
function ReLoginIframe(){
	...
	$pr=$Elves->fetch1("select loginiframe,loginjstemp from ".GetTemptb("melvepubtemp")." limit 1");
	//框架登陆状态调用
...
//返回投票模板
function ReturnVoteTemp($tempid,$melve=0){
	global $Elves;
	$r=$Elves->fetch1("select temptext from ".GetTemptb("melvevotetemp")." where tempid='$tempid'");
	if($melve)
	{
...
function RepVoteTempAllvar($temptext,$r){
	global $public_r;
	$action=$public_r['newsurl']."core/melve/index.php";
	$temptext=str_replace("[!--vote.action--]",$action,$temptext);
	$temptext=str_replace("[!--title--]",$r[title],$temptext);
	$viewurl=$public_r[newsurl]."core/tool/vote/?voteid=".$r[voteid];
...
function GetPrintPage($printtempid=0){
	global $Elves,$dbtbpre,$fun_r,$public_r;
	$file=elve_PATH.'core/data/template/printtemp.txt';
	$string=ReadFiletext($file);
	$url="<?=\$url?>";
	$pagetitle="<?=ehtmlspecialchars(\$r[title])?> ".$fun_r['PrintPage'];
	//取得评论页面模板
	$where=$printtempid?" where tempid='$printtempid'":'';
	$ptsql=$Elves->query("select tempid,temptext,showdate,modid from ".GetTemptb("melveprinttemp").$where);
	while($ptr=$Elves->fetch($ptsql))
	{
		$ptr[temptext]=ReplaceSvars($ptr[temptext],$url,0,$pagetitle,$pagetitle,$pagetitle,$add,1);
		$printtemp=RepPrintTempV($ptr);
		$printtemp=str_replace("<!--Elves.print-->",$printtemp,$string);
		$truefile=elve_PATH.'core/data/filecache/template/print'.$ptr[tempid].'.php';
...
function RepPrintTempV($tr){
	global $Elves,$dbtbpre,$fun_r,$public_r,$emod_r;
...
function GetDownloadPage(){
	global $Elves,$public_r,$dbtbpre,$fun_r;
	$pr=$Elves->fetch1("select downpagetemp from ".GetTemptb("melvepubtemp")." limit 1");
...
function ReSchAlltemp(){
	...
	//取得页面模板
	$pr=$Elves->fetch1("select schalltemp,schallsubnum,schalldate from ".GetTemptb("melvepubtemp")." limit 1");
	$url="<?=\$url?>";
	$pagetitle=$fun_r['SearchAllNav'];
	$pr['schalltemp']=ReplaceSvars($pr['schalltemp'],$url,0,$pagetitle,$pagetitle,$pagetitle,$add,1);
	$temp=str_replace("<!--Elves.listtemp.subnum-->",$pr['schallsubnum'],$temp);
	$temp=str_replace("<!--Elves.listtemp.formatdate-->",$pr['schalldate'],$temp);
	...
	$listtemp_r=explode("[!--Elvesnews.listtemp--]",$pr['schalltemp']);
	$temp=str_replace("<!--Elves.listtemp.top-->",$listtemp_r[0],$temp);
	$temp=str_replace("<!--Elves.listtemp.footer-->",$listtemp_r[2],$temp);
	//---列表中间
	...
	$temp=str_replace("<!--Elves.listtemp.center-->",$listtemp_center,$temp);
...
function ReturnLeftLevel($groupid){
	...
	{return "";}
	$groupid=(int)$groupid;
	$r=$Elves->fetch1("select * from {$dbtbpre}melvegroup where groupid='$groupid'");
	return $r;
}

//返回操作权限
function CheckLevel($userid,$username,$classid,$melve){
	global $Elves,$dbtbpre;
	$userid=(int)$userid;
	$r=$Elves->fetch1("select groupid,adminclass from {$dbtbpre}melveuser where userid='$userid' limit 1");
	//操作信息
	if($melve=="news")
	{
		//操作所有栏目权限
		$gr=$Elves->fetch1("select doall,doselfinfo,doaddinfo,doeditinfo,dodelinfo,docheckinfo,dogoodinfo,dodocinfo,domoveinfo,domustcheck,docheckedit from {$dbtbpre}melvegroup where groupid='$r[groupid]'");
		if(empty($gr[doall]))
		{
	...
	{
		//用户组
		$gr=$Elves->fetch1("select * from {$dbtbpre}melvegroup where groupid='$r[groupid]'");
		$melve="do".$melve;
		if(empty($gr[$melve]))
		{
...
//验证设置权限
function CheckDoLevel($lur,$groupid,$userclass,$username,$elve=0){
	$ret=0;
...
function CheckAndUsernamesLevel($level,$id,$userid,$username,$groupid){
	...
	if($level=='dozt')//专题
	{
		$getquery="select ztid,usernames from {$dbtbpre}melvezt where ztid='$id'";
		$id_field='ztid';
	...
	$getr=$Elves->fetch1($getquery);
	if(!$getr[$id_field])
	{
		printerror('ErrorUrl','history.go(-1)');
	}
	$gr=$Elves->fetch1("select groupid,".$level." from {$dbtbpre}melvegroup where groupid='$groupid'");
	if(!$gr['groupid'])
	{
		printerror('NotLevel','history.go(-1)');
	}
...
function is_login($uid=0,$uname='',$urnd=''){
	...
	{
		$cdbdata=getcvar('elvedodbdata',1)?1:0;
...
function is_login_ebak($userid,$username,$rnd){
	...
	$userid=(int)$userid;
	$username=RepPostVar($username);
	$dodbdata=getcvar('elvedodbdata',1);
	if(!$userid||!$username)
	{
		printerror("NotLogin","index.php");
	}
	if($dodbdata!="Elvescms")
	{
		printerror("NotLogin","index.php");
	}
	$rnd=RepPostVar($rnd);
...
function DoECookieRnd($userid,$username,$rnd,$userkey,$dbdata,$groupid,$adminstyle,$truelogintime){
	...
	esetcookie("loginelveckpass",$elveckpass,0,1);
...
function DoChECookieRnd($userid,$username,$rnd,$userkey,$dbdata,$groupid,$adminstyle,$truelogintime){
	...
	$elveckpass=md5(md5($rnd.$elve_config['esafe']['ecookiernd']).'-'.$ip.'-'.$otherinfo.'-'.$userid.'-'.$username.'-'.$dbdata.$rnd.$groupid.'-'.$adminstyle.$sessval);
	if($elveckpass<>getcvar('loginelveckpass',1))
	{
		printerror("NotLogin","index.php");
	}
	if(empty($elve_config['esafe']['ckhloginfile']))
...
function DelECookieRnd(){
	esetcookie("loginelveckpass",'',0,1);
}
...
function DoEDelFileRnd($userid){
	$path=elve_PATH.'core/data/adminlogin/';
	$hand=@opendir($path);
...
function DoECreatAndAuthRnd($userid,$username,$rnd,$userkey,$dbdata,$groupid,$adminstyle,$truelogintime,$ip,$sessval){
	...
	$andauth=md5(md5($rnd.'-'.$username.'-Elvescms!check.andauth'.$truelogintime.'-'.$elve_config['esafe']['ecookiernd'].$userkey).$sessval.'-'.$ip.'-'.$userid.$rnd.'-'.$adminstyle.'-'.$groupid.$username.'-'.$dbdata);
	DoEDelAndAuthRnd($userid);
	$Elves->query("replace into {$dbtbpre}melveuserloginck(userid,andauth) values('$userid','$andauth');");
}

function DoECheckAndAuthRnd($userid,$username,$rnd,$userkey,$dbdata,$groupid,$adminstyle,$truelogintime){
	global $Elves,$dbtbpre,$elve_config;
	$anduser_r=$Elves->fetch1("select andauth from {$dbtbpre}melveuserloginck where userid='$userid'");
	...
	//$otherinfo=DoECkOtherInfo();
	$sessval=ReESessionRnd();
	$ckandauth=md5(md5($rnd.'-'.$username.'-Elvescms!check.andauth'.$truelogintime.'-'.$elve_config['esafe']['ecookiernd'].$userkey).$sessval.'-'.$ip.'-'.$userid.$rnd.'-'.$adminstyle.'-'.$groupid.$username.'-'.$dbdata);
...
function DoEDelAndAuthRnd($userid){
	global $Elves,$dbtbpre;
	$Elves->query("delete from {$dbtbpre}melveuserloginck where userid='$userid'");
}

//写入操作日志
function insert_dolog($doing,$pubid=0){
	global $Elves,$melve,$mgxe,$logininid,$loginin,$elve_config,$dbtbpre;
	if($elve_config['esafe']['thedolog'])
	{
		return "";
	}
	if(empty($doing))
	...
	if(empty($melve))
	{$melve=$mgxe;}
	$melve=RepPostVar($melve);
	$pubid=RepPostVar($pubid);
	$loginin=RepPostVar($loginin);
	$sql=$Elves->query("insert into {$dbtbpre}melvedolog(username,logip,logtime,melve,doing,pubid) values('$loginin','$logip','$logtime','$melve','$doing','$pubid');");
}

//返回安全提问问题
function ReturnHLoginQuestionStr($userid,$username,$question,$answer){
	$pass=md5(md5('-#20Elves27#-'.$question.'-Elvescms-'.$userid.'-www.webelves.org-'.$answer.'-wm-').'-dg2002-'.$answer.'-wm_chief-'.$userid.'-wangmeng-');
	return $pass;
}
...
function FtpMkdir($ftpid,$pr,$mod){
	...
	for($i=0;$i<count($pr);$i++)
	{
		if(stristr($pr[$i],elve_PATH))
		{
			$pr[$i]=FtpRTruePath($r[ftppath],str_replace(elve_PATH,'',$pr[$i]));
		}
		if(!$e->fChdir($pr[$i]))
		{
			$e->fMkdir($pr[$i]);
...
function ReturnFtpInfo($ftpid){
	global $Elves,$dbtbpre;
	$r=$Elves->fetch1("select * from {$dbtbpre}melvepublic limit 1");
	return $r;
}

//初使化发布任务
function AddPostUrlData($postdata,$userid,$username){
	global $Elves,$fun_r,$dbtbpre;
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$r[1]=(int)$r[1];
		$sql=$Elves->query("insert into {$dbtbpre}melvepostdata(rnd,postdata,ispath) values('$rnd','$r[0]',$r[1]);");
    }
	$line=(int)$_POST['line'];
	if($line==0)
	{
	...
	echo $fun_r[AddPostDataSuccess]."<script>self.location.href='melve.php?melve=PostUrlData&start=0&line=$line&rnd=$rnd';</script>";
	exit();
}

//远程发布
function PostUrlData($start,$rnd,$userid,$username){
	global $Elves,$fun_r,$dbtbpre,$incftp;
	$rnd=RepPostVar($rnd);
	if(empty($rnd))
	...
	if(empty($incftp))
	{
		@include(elve_PATH."core/class/ftp.php");
	}
	...
	$start=(int)$start;
	$b=0;
	$sql=$Elves->query("select postid,postdata,ispath from {$dbtbpre}melvepostdata where rnd='$rnd' and postid>$start order by postid limit ".$line);
	while($r=$Elves->fetch($sql))
	{
		$b=1;
		$newstart=$r[postid];
		...
		if($r[ispath])
		{
			...
			{
				$e->fTranFile(FtpRTruePath($pr[ftppath],$fr[$i]),elve_PATH.$fr[$i]);
			}
		}
		//目录
		else
		{
			$e->ftp_copy(elve_PATH.$r[postdata],FtpRTruePath($pr[ftppath],$r[postdata]));
		}
	}
	$e->fExit();
	if(empty($b))
	{
		$sql=$Elves->query("delete from {$dbtbpre}melvepostdata where rnd='$rnd'");
		//操作日志
		insert_dolog("");
		printerror("PostDataSuccess","PostUrlData.php");
	}
	echo $fun_r[OnePostDataSuccess]."<script>self.location.href='melve.php?melve=PostUrlData&start=$newstart&line=$line&rnd=$rnd';</script>";
	exit();
}

//测试FTP
function CheckFtpConnect($ftphost,$ftpport,$ftpusername,$ftppassword,$ftppath,$ftpssl=0,$pasv=0,$tranmode=0,$timeout=0){
	if(!defined('InElvesCMSFtp'))
	{
		include(elve_PATH.'core/class/ftp.php');
	}
	$eftp=new ElvesCMSFTP();
...
//复制表
function CopyelveTb($otb,$tb){
	global $Elves;
	$usql=$Elves->query("SET SQL_QUOTE_SHOW_CREATE=1;");//设置引号
	$r=$Elves->fetch1("SHOW CREATE TABLE `$otb`;");//数据表结构
...
function SetCreateTable($sql,$dbcharset) {
	...
	return preg_replace("/^\s*(CREATE TABLE\s+.+\s+\(.+?\)).*$/isU", "\\1", $sql).
		($elve_config['db']['dbver']>='4.1'&&$dbcharset ? " ENGINE=$type DEFAULT CHARSET=$dbcharset" : " TYPE=$type");
}

//组合存文本
function TogSaveTxtF($elve=0){
	global $Elves,$dbtbpre;
	$savesql=$Elves->query("select f,tbname from {$dbtbpre}melvef where savetxt=1");
	$savef=',';
	while($saver=$Elves->fetch($savesql))
	{
		$savef.=$saver[tbname].'.'.$saver[f].',';
	}
	$Elves->query("update {$dbtbpre}melvepublic set savetxtf='$savef' limit 1");
	if($elve==0)
	{
		GetConfig();
	}
...
function ReturnMFileF($enter,$tbname,$tid,$fform="file"){
	...
	$f=",";
	$sql=$Elves->query("select f from ".$tbname." where fform='$fform'".$a);
	while($r=$Elves->fetch($sql))
	{
...
function ChGetFname($mid,$f){
	global $Elves,$dbtbpre,$emod_r;
	$r=$Elves->fetch1("select fname from {$dbtbpre}melvef where f='$f' and tid='".$emod_r[$mid]['tid']."' limit 1");
...
function ChIsOnlyAddF($mid,$id,$f,$value,$isq=0){
	...
	if(strstr($emod_r[$mid]['onlyf'],','.$f.','))
	{
		...
		//已审核
		$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}elve_".$emod_r[$mid]['tbname']." where ".$f."='".addslashes($value)."'".$and." limit 1");
		//未审核
		if(empty($num))
		{
			$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}elve_".$emod_r[$mid]['tbname']."_check where ".$f."='".addslashes($value)."'".$and." limit 1");
		}
		if($num)
		{
...
function SameDataAddF($id,$classid,$mid,$f,$value){
	global $Elves,$public_r,$dbtbpre,$emod_r,$emod_pubr;
	if(strstr($emod_pubr['linkfields'],','.$emod_r[$mid]['tbname'].'.'.$f.'|'))
	{
		$index_r=$Elves->fetch1("select checked from {$dbtbpre}elve_".$emod_r[$mid]['tbname']."_index where id='$id' limit 1");
		//返回表
		$infotb=empty($index_r['checked'])?$dbtbpre.'elve_'.$emod_r[$mid]['tbname'].'_check':$dbtbpre.'elve_'.$emod_r[$mid]['tbname'];
		$value=addslashes($value);
		$r=$Elves->fetch1("select ".$f." from ".$infotb." where id='$id' limit 1");
		if($r[$f]<>$value)
		{
			...
			if($ltbname&&$lf)
			{
				$Elves->query("update {$dbtbpre}elve_".$ltbname." set ".$lf."='$value' where ".$lf."='$r[$f]'");
			}
		}
	}
}
...
//处理多值字段
function DoFieldMoreValue($f,$add,$elve=0){
	$rvarname=$f.'_1';
...
//返回多值字段数据
function ReturnMoreValueAddF($add,$r,$mid,$f,$elve=0){
	...
	if(strstr($emod_r[$mid]['morevaluef'],'|'.$f.','))
	{
		...
		{
			$val=DoFieldMoreValue($f,$add,$elve);
		}
		else
		{
			$val='';
...
function ReturnAddF($add,$modid,$userid,$username,$do=0,$rdata=0,$ch=0){
	global $Elves,$public_r,$dbtbpre,$emod_r;
	if($do==0||$do==1)
	{
		//导入gd处理文件
	...
	if(empty($do))//增加
	{
		...
		for($i=1;$i<$count;$i++)
		{
			...
			if($f=="newstext")
			{
				...
				//自动分页
				if($add[autopage]&&!strstr($value,"[!--Elvesnews.page--]"))
				{
...
function SaveMorepicFile($varname,$msavepic,$i,$picurl,$picname,$classid,$id,$add,$modtype=0,$fstb=1){
	global $public_r,$Elves,$loginin,$dbtbpre,$elve_config;
...
function GetClassNavCache($line,$navfh){
	...
	$navs='';
	$fh='';
	$sql=$Elves->query("select classid,classname,wburl,listdt,classurl,classpath from {$dbtbpre}melveclass where bclassid=0 and showclass=0 order by myorder,classid".$limit);
	while($r=$Elves->fetch($sql))
	{
...
function GetPubCache(){
	...
	//扩展变量
	$pvstring='';
	$pvsql=$Elves->query("select myvar,varvalue from {$dbtbpre}melvepubvar where tocache=1");
	while($pvr=$Elves->fetch($pvsql))
	{
		$pvstring.=",'add_".$pvr['myvar']."'=>'".addslashes($pvr['varvalue'])."'";
	}
	//公共变量
	$r=$Elves->fetch1("select * from {$dbtbpre}melvepublic limit 1");
	$tr=$Elves->fetch1("select downsofttemp,onlinemovietemp,listpagetemp from ".GetTemptb("melvepubtemp")." limit 1");
	$fsr=$Elves->fetch1("select purl from {$dbtbpre}melvepostserver where ptype=1 limit 1");
	$plr=$Elves->fetch1("select * from {$dbtbpre}melvepl_set limit 1");
	$memberconnectnum=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvemember_connect_app where isclose=0");
...
function GetModCache(){
	global $Elves,$dbtbpre;
	//数据表
	$tablesql=$Elves->query("select tbname,deftb,yhid,mid,intb from {$dbtbpre}melvetable");
	while($tabler=$Elves->fetch($tablesql))
	{
...
";
	...
	//系统模型
	$alllinkfields='|';//关联同步
	$modsql=$Elves->query("select * from {$dbtbpre}melvemod");
	while($mr=$Elves->fetch($modsql))
	{
		...
		$fsql=$Elves->query("select * from {$dbtbpre}melvef where tid='$mr[tid]'");
		...
		//表数据
		$tr=$Elves->fetch1("select * from {$dbtbpre}melvetable where tid='$mr[tid]'");
		//字符
...
function GetMemberLevel(){
	global $Elves,$dbtbpre;
	$file=elve_PATH."core/data/dbcache/MemberLevel.php";
	$sql=$Elves->query("select * from {$dbtbpre}melvemembergroup order by groupid");
	while($r=$Elves->fetch($sql))
	{
...
function GetYh(){
	global $Elves,$dbtbpre;
	$sql=$Elves->query("select * from {$dbtbpre}melveyh");
	while($r=$Elves->fetch($sql))
	{
...
function GetClass(){
	global $Elves,$dbtbpre;
	$fileqz=elve_PATH.'core/data/dbcache/';
	$filename=$fileqz.'class.php';
	$line=250;//每个文件存放栏目数
	$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}melveclass");
	$sql=$Elves->query("select * from {$dbtbpre}melveclass");
	$no=0;
	$p=0;
	$l="";
	$mod="";
...
";
		if($no%$line==0||($num%$line<>0&&$num==$no))
		{
			$p++;
			$file="class".$p.".php";
			$include.="require(elve_PATH.'core/data/dbcache/".$file."');\r\n";
			$classes="<?php
".$classes."?>";
	...
	//-----专题缓存
	$zsql=$Elves->query("select * from {$dbtbpre}melvezt");
	$zt="";
	$zfile=$fileqz."ztclass.php";
	while($zr=$Elves->fetch($zsql))
	{
...
".$zt.GetTitleTypeCache()."?>";
	...
	$include.="require(elve_PATH.'core/data/dbcache/ztclass.php');\r\n";
	$include="<?php
".AddCheckViewCode()."
\$class_r=array();
\$class_zr=array();
...
?>";
	...
	{
		$mid=$er[$i];
		$usql=$Elves->query("update {$dbtbpre}melvemod set sonclass='".$mod[$mid]."' where mid='$mid'");
    }
}
...
function GetTitleTypeCache(){
	global $Elves,$dbtbpre;
	$sql=$Elves->query("select typeid,tname,mid,yhid,tpath,tid,tbname,listdt,ttype from {$dbtbpre}melveinfotype");
	while($r=$Elves->fetch($sql))
	{
...
function GetSearchAllTb(){
	global $Elves,$dbtbpre;
	$file=elve_PATH."core/data/dbcache/SearchAllTb.php";
	$sql=$Elves->query("select tbname,titlefield,smalltextfield from {$dbtbpre}melvesearchall_load");
	while($r=$Elves->fetch($sql))
	{
