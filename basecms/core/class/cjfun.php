...
function EditCjNews($add,$newstext,$userid,$username){
	...
	$record="<!--record-->";
	$field="<!--field--->";
	$cr=$Elves->fetch1("select newsclassid,tbname from {$dbtbpre}melveinfoclass where classid='$add[classid]'");
	$r=$Elves->fetch1("select cj from {$dbtbpre}melvemod where mid='".$class_r[$cr[newsclassid]][modid]."'");
	...
	$sql=$Elves->query("update {$dbtbpre}elve_infotmp_".$cr[tbname]." set keyboard='".eaddslashes2($add[keyboard])."',newstime='$add[newstime]'".$update." where id='$add[id]'");
	if($sql)
	{
		//操作日志
...
function DelCjNews($classid,$id,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"cj");
	$cr=$Elves->fetch1("select newsclassid,tbname from {$dbtbpre}melveinfoclass where classid='$classid'");
	$r=$Elves->fetch1("select title from {$dbtbpre}elve_infotmp_".$cr[tbname]." where id='$id'");
	$sql=$Elves->query("delete from {$dbtbpre}elve_infotmp_".$cr[tbname]." where id='$id'");
	if($sql)
	{
		//操作日志
...
function DelCjNews_all($classid,$id,$userid,$username){
	...
	if(!$count)
	{printerror("NotDelCjNewsid","history.go(-1)");}
	$cr=$Elves->fetch1("select newsclassid,tbname,classname from {$dbtbpre}melveinfoclass where classid='$classid'");
	...
	$sql=$Elves->query("delete from {$dbtbpre}elve_infotmp_".$cr[tbname]." where ".$add);
	if($sql)
	{
		//操作日志
...
function CjNewsIn($classid,$id,$checked,$uptime,$userid,$username){
	global $class_r,$Elves,$public_r,$dbtbpre,$emod_r;
	$checked=(int)$checked;
	$classid=(int)$classid;
	if(empty($classid))
	{
	...
	$cr=$Elves->fetch1("select * from {$dbtbpre}melveinfoclass where classid='$classid'");
	//副表
	$cra=$Elves->fetch1("select * from {$dbtbpre}elve_infoclass_".$cr[tbname]." where classid='$classid'");
	//组合两数组
	...
	$record="<!--record-->";
	$field="<!--field--->";
	$mr=$Elves->fetch1("select cj from {$dbtbpre}melvemod where mid='".$class_r[$cr[newsclassid]][modid]."'");
	...
	$sql=$Elves->query("select * from {$dbtbpre}elve_infotmp_".$cr[tbname]." where ".$a." and checked=0 order by id desc");
	$todaytime=time();
	$filetime=$todaytime;
	while($r=$Elves->fetch($sql))
	{
		...
		//索引表
		$havehtml=0;
		$indexsql=$Elves->query("insert into {$dbtbpre}elve_".$class_r[$cr[newsclassid]][tbname]."_index(classid,checked,newstime,truetime,lastdotime,havehtml) values('$cr[newsclassid]','$checked','$r[newstime]','$r[truetime]','$r[truetime]','$havehtml');");
		$id=$Elves->lastid();
		$infotbr=ReturnInfoTbname($class_r[$cr[newsclassid]][tbname],$checked,$stb);
		//主表
		$isurl=$r['titleurl']?1:0;
		$isql=$Elves->query("insert into ".$infotbr['tbname']."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard,newstime".$ifield.") values('$id','$cr[newsclassid]',0,0,0,0,'$newspath','$filename','$r[userid]','$r[username]',0,0,'$ispic',0,'$isqf',0,'$isurl','$r[truetime]','$r[truetime]','$havehtml',0,0,'$r[titlefont]','$r[titleurl]','$stb','$public_r[filedeftb]','$public_r[pldeftb]','$r[keyboard]','$r[newstime]'".$ivalue.");");
		//副表
		$fisql=$Elves->query("insert into ".$infotbr['datatbname']."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags".$dataifield.") values('$id','$cr[newsclassid]','$keyid',1,'$newstempid',0,0,''".$dataivalue.");");
		//更新栏目信息数
	...
	{
		$del=$Elves->query("delete from {$dbtbpre}elve_infotmp_".$cr[tbname]." where ".$a);
	}
	else
	{
		$del=$Elves->query("update {$dbtbpre}elve_infotmp_".$cr[tbname]." set checked=1,keyboard=''".$updatefield." where ".$a);
	}
	//操作日志
...
function CjNewsIn_all($classid,$checked,$uptime,$start,$userid,$username){
	global $class_r,$Elves,$public_r,$dbtbpre,$fun_r,$emod_r;
	$checked=(int)$checked;
	$classid=(int)$classid;
	$start=(int)$start;
	if(empty($classid))
	...
	$cjr=explode($record,$mr[cj]);
	$ccount=count($cjr);
	$sql=$Elves->query("select * from {$dbtbpre}elve_infotmp_".$cr[tbname]." where classid='$classid' and checked=0 and id>$start order by id limit ".$cr[insertnum]);
	$todaytime=time();
	$filetime=$todaytime;
	$b=0;
	while($r=$Elves->fetch($sql))
	...
	if(empty($b))
	{
		...
		if($cr['delloadinfo'])//删除
		{
			$del=$Elves->query("delete from {$dbtbpre}elve_infotmp_".$cr[tbname]." where classid='$classid'");
		}
		else
		{
			$del=$Elves->query("update {$dbtbpre}elve_infotmp_".$cr[tbname]." set checked=1,keyboard=''".$updatefield." where classid='$classid'");
		}
		if($fm)
		{
	...
	echo "<b>$cr[classname]</b>&nbsp;&nbsp;".$fun_r['OneCjLoadDbSuccess']."(ID:<font color=red><b>".$newstart."</b></font>)<script>self.location.href='elvecj.php?melve=CjNewsIn_all&checked=$checked&uptime=$uptime&classid=$classid&start=$newstart&fm=$fm&from=$_GET[from]';</script>";
	exit();
}
...
//返回字符正则
function GetInfoStr($text,$exp,$melve=0){
	$e1="[mgxe-".$exp."]";
	$e2="[/mgxe-".$exp."]";
	$rep="[!--".$exp."--]";
	$mode="*";
...
//地址
function eCheckCjUrl($url,$elve=0){
	if(!strstr($url,'://'))
	{
		if($elve)
		{
...
//编码转换
function doCjUtfAndGbk($str,$mgxe=0){
	//正常编码
	if($mgxe==0)
	{
		return $str;
	}
	if($mgxe==1)//UTF8->GB2312
	{
		$str=DoIconvVal("UTF8","GB2312",$str);
	}
	elseif($mgxe==2)//UTF8->BIG5
	{
		$str=DoIconvVal("UTF8","BIG5",$str);
	}
	elseif($mgxe==3)//BIG5->GB2312
	{
		$str=DoIconvVal("BIG5","GB2312",$str);
	}
	elseif($mgxe==4)//GB2312->BIG5
	{
		$str=DoIconvVal("GB2312","BIG5",$str);
	}
	elseif($mgxe==5)//UNICODE->GB2312
	{
		$str=DoIconvVal("UNICODE","GB2312",$str);
	}
	elseif($mgxe==6)//UNICODE->BIG5
	{
		$str=DoIconvVal("UNICODE","BIG5",$str);
	}
	elseif($mgxe==7)//GB2312->UTF8
	{
		$str=DoIconvVal("GB2312","UTF8",$str);
	}
	elseif($mgxe==8)//BIG5->UTF8
	{
		$str=DoIconvVal("BIG5","UTF8",$str);
	}
	elseif($mgxe==9)//UNICODE->UTF8
	{
...
function EchoUrl($text,$exp,$exp1,$dr,$url,$classid,$num,$checkrnd){
	...
	for($i=1;$i<count($r)&&$i<=$num;$i++)
	{
		...
		{
			$dbnum=$Elves->gettotal("select count(*) as total from {$dbtbpre}elve_infotmp_".$dr[tbname]." where oldurl='$dourl' limit 1");
			if($dbnum)
			{continue;}
		}
		//图片链接
		...
		//将地址写入数据库
		$sql=$Elves->query("insert into {$dbtbpre}melvelinktmp(newsurl,checkrnd,titlepic) values('$dourl','$checkrnd','$titlepic');");
		echo $dourl."<br>";
	}
}
...
function PageEchoUrl($classid,$cr,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		{
			$dbnum=$Elves->gettotal("select count(*) as total from {$dbtbpre}elve_infotmp_".$cr[tbname]." where oldurl='$dourl' limit 1");
			if($dbnum)
			{continue;}
		}
		//将地址写入数据库
		$sql=$Elves->query("insert into {$dbtbpre}melvelinktmp(newsurl,checkrnd,titlepic) values('$dourl','$checkrnd','');");
		echo $dourl."<br>";
	}
	echo $fun_r['GetUrlOver']."<script>self.location.href='elvecj.php?melve=GetNewsInfo&classid=$classid&start=0&checkrnd=$checkrnd';</script>";
	exit();
}
...
function CJUrl($classid,$start,$checkrnd,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"cj");
	$r=$Elves->fetch1("select endday,num,zz_smallurl,zz_newsurl,httpurl,infourl,newsclassid,relistnum,zz_titlepicl,z_titlepicl,qz_titlepicl,save_titlepicl,tbname,recjtheurl,enpagecode,pagerepad,oldpagerep,newpagerep,keeptime,infourlispage from {$dbtbpre}melveinfoclass where classid='$classid'");
	...
	//echo $fun_r['GetOneListUrl']."<script>self.location.href='elvecj.php?melve=CjUrl&classid=$classid&start=$newstart&checkrnd=$checkrnd';</script>";
	echo"<meta http-equiv=\"refresh\" content=\"".$r['keeptime'].";url=elvecj.php?melve=CjUrl&classid=$classid&start=$newstart&checkrnd=$checkrnd\">".$fun_r['GetOneListUrl'];
	exit();
}
...
function ViewCjList($classid,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"cj");
	$r=$Elves->fetch1("select endday,num,zz_smallurl,zz_newsurl,httpurl,infourl,newsclassid,relistnum,zz_titlepicl,z_titlepicl,qz_titlepicl,save_titlepicl,infourlispage from {$dbtbpre}melveinfoclass where classid='$classid'");
	if(empty($r[newsclassid]))
	{printerror("NotCjid","history.go(-1)");}
	//直接内容页列表
	if($r[infourlispage])
	...
	for($i=0;$i<$j;$i++)
	{
		...
		$data.="<tr><td><a href='".$url_r[$i]."' target=_blank>".$url_r[$i]."</a></td><td align=center>[<a href='elvecj.php?melve=ViewCjUrl&classid=$classid&listpage=".$dourl."' target=_blank>".$fun_r['view']."</a>]</td></tr>";
    }
...
function PageViewCjList($classid,$cr,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$data.="<tr><td><a href='".$url_r[$i]."' target=_blank>".$url_r[$i]."</a></td><td align=center>[<a href='elvecj.php?melve=ViewGetNewsInfo&classid=$classid&newspage=".$dourl."' target=_blank>".$fun_r['view']."</a>]</td></tr>";
    }
	$data="<p align=center><b>".$fun_r['CjListUrl']."</b></p><table width='96%' border=1 align=center cellpadding=3 cellspacing=0>
  <tr><td width=70% align=center><b>URL</b></td><td align=center><b>VIEW</b></td></tr>".$data."</table>";
	echo $data;
...
function ViewCjUrl($classid,$listpage,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"cj");
	$r=$Elves->fetch1("select endday,num,zz_smallurl,zz_newsurl,httpurl,infourl,newsclassid,relistnum,zz_titlepicl,z_titlepicl,qz_titlepicl,save_titlepicl,tbname,recjtheurl,enpagecode,pagerepad,oldpagerep,newpagerep from {$dbtbpre}melveinfoclass where classid='$classid'");
	if(empty($r[newsclassid]))
	{printerror("NotCjid","history.go(-1)");}
	//导入编码文件
	if($r['enpagecode'])
...
function ViewEchoUrl($text,$exp,$exp1,$dr,$url,$classid,$num,$checkrnd){
	...
	for($i=1;$i<count($r)&&$i<=$num;$i++)
	{
		...
		echo "<tr><td><a href='".$dourl."' target=_blank>".$dourl."</a>".$a."</td><td align=center>[<a href='elvecj.php?melve=ViewGetNewsInfo&classid=$classid&newspage=".urlencode($dourl)."' target=_blank>".$fun_r['view']."</a>]</td></tr>";
	}
}
...
function GetMoreCjPagetext($self,$newstextzz,$smallpagezz,$pagezz,$pagetype,$firsttext,$the_r){
	...
	{return "";}
	//是否保留分页
	$addpagetag=empty($the_r['doaddtextpage'])?'[!--Elvesnews.page--]':'';
	//取得地址前缀
...
function GetNewsInfo($classid,$checkrnd,$start,$userid,$username){
	global $Elves,$class_r,$fun_r,$dbtbpre;
	$userid=(int)$userid;
	$classid=(int)$classid;
	$start=(int)$start;
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"cj");
	$r=$Elves->fetch1("select * from {$dbtbpre}melveinfoclass where classid='$classid'");
	//副表
	$ra=$Elves->fetch1("select * from {$dbtbpre}elve_infoclass_".$r[tbname]." where classid='$classid'");
	//组合两数组
	$r=TogTwoArray($r,$ra);
	//导入编码文件
	if($r['enpagecode'])
	...
	$record="<!--record-->";
	$field="<!--field--->";
	$mr=$Elves->fetch1("select cj,tid,tbname from {$dbtbpre}melvemod where mid='".$class_r[$r[newsclassid]][modid]."'");
	$cjr=explode($record,$mr[cj]);
	$count=count($cjr);
	if(empty($start))
	{$start=0;}
	$b=0;
	$sql=$Elves->query("select linkid,newsurl,titlepic from {$dbtbpre}melvelinktmp where checkrnd='$checkrnd' and linkid>".$start." order by linkid limit ".$r[renum]);
	//采集时间
	$newstime=date("Y-m-d H:i:s");
    $truetime=time();
	$tmptime=$newstime;
	while($nr=$Elves->fetch($sql))
	{
		$b=1;
		$newstart=$nr[linkid];
		...
		{
			$tmpnum=$Elves->gettotal("select count(*) as total from {$dbtbpre}elve_infotmp_".$r[tbname]." where oldurl='$nr[newsurl]' limit 1");
			if($tmpnum)
			{continue;}
		}
		//取得远程页面，三次取不到值的将不取值
		...
		for($c=0;$c<$count-1;$c++)
		{
			...
			if($dofield=="morepic"&&$r[$var])
			{
				...
				//替换回车
				$text=ReplaceFc($text);
				$down=explode("[!Elvescms!]",$text);
				//小图
	            $zztext=RepInfoZZ($down[0],"elvespicurl",0);
	            $strtext=GetInfoStr($down[0],"elvespicurl",1);
	            $text1=stripSlashes(preg_replace($zztext,$strtext,$info));
				//大图
				if($down[1])
				{
					$zztext=RepInfoZZ($down[1],"elvebpicurl",0);
	                $strtext=GetInfoStr($down[1],"elvebpicurl",1);
	                $text2=stripSlashes(preg_replace($zztext,$strtext,$info));
				}
				//名称
				if($down[2])
				{
					$zztext=RepInfoZZ($down[2],"elvepicname",0);
	                $strtext=GetInfoStr($down[2],"elvepicname",1);
	                $text3=stripSlashes(preg_replace($zztext,$strtext,$info));
				}
				$zzvalue=GetCjMorepicpath($text1,$text2,$text3,"elvespicurl","elvebpicurl","elvepicname",$r[$var2]);
				$ifield.=",".$dofield;
			    $ivalue.=",'".addslashes($zzvalue)."'";
				continue;
			}
			...
			if($dofield=="downpath"&&$r[$var])
			{
				...
				$down=explode("[!Elvescms!]",$text);
				//地址
	            $zztext=RepInfoZZ($down[0],"elvedownpathurl",0);
	            $strtext=GetInfoStr($down[0],"elvedownpathurl",1);
	            $text1=stripSlashes(preg_replace($zztext,$strtext,$info));
				//名称
				if($down[1])
				{
					$zztext=RepInfoZZ($down[1],"elvedownpathname",0);
	                $strtext=GetInfoStr($down[1],"elvedownpathname",1);
	                $text2=stripSlashes(preg_replace($zztext,$strtext,$info));
				}
				$zzvalue=GetCjDownpath($text1,$text2,"elvedownpathurl","elvedownpathname",$r[$var2],0);
				$ifield.=",".$dofield;
			    $ivalue.=",'".addslashes($zzvalue)."'";
				continue;
			}
			...
			if($dofield=="onlinepath"&&$r[$var])
			{
				...
				$down=explode("[!Elvescms!]",$text);
				//地址
	            $zztext=RepInfoZZ($down[0],"elveonlinepathurl",0);
	            $strtext=GetInfoStr($down[0],"elveonlinepathurl",1);
	            $text1=stripSlashes(preg_replace($zztext,$strtext,$info));
				//名称
				if($down[1])
				{
					$zztext=RepInfoZZ($down[1],"elveonlinepathname",0);
	                $strtext=GetInfoStr($down[1],"elveonlinepathname",1);
	                $text2=stripSlashes(preg_replace($zztext,$strtext,$info));
				}
				$zzvalue=GetCjDownpath($text1,$text2,"elveonlinepathurl","elveonlinepathname",$r[$var2],1);
				$ifield.=",".$dofield;
			    $ivalue.=",'".addslashes($zzvalue)."'";
				continue;
			}
			...
			if($dofield=="title")
			{
				...
				{
					//入库比较
					$onum=$Elves->gettotal("select count(*) as total from {$dbtbpre}elve_".$class_r[$r[newsclassid]][tbname]." where title='".addslashes($zzvalue)."' and classid='$r[newsclassid]' limit 1");
					if($onum)
					{
						$next=1;
						break;
					...
					/*采集库比较
					$onum=$Elves->num("select id from {$dbtbpre}elve_infotmp_".$r[tbname]." where title='".addslashes($zzvalue)."' limit 1");
					if($onum)
					{
						$next=1;
						break;
				...
				if($r[titlelen])//标题相似
				{
					...
					$onum=$Elves->gettotal("select count(*) as total from {$dbtbpre}elve_".$class_r[$r[newsclassid]][tbname]." where title like '%$c_title%' and classid='$r[newsclassid]' limit 1");
					if($onum)
					{
						$next=1;
						break;
					...
					/*采集比较
					$onum=$Elves->num("select id from {$dbtbpre}elve_infotmp_".$r[tbname]." where title like '%$c_title%' limit 1");
					if($onum)
					{
						$next=1;
						break;
		...
		{continue;}
		//入临时库
		$isql=$Elves->query("insert into {$dbtbpre}elve_infotmp_".$r[tbname]."(classid,oldurl,checked,keyboard,newstime,truetime,tmptime,userid,username".$ifield.") values($classid,'$nr[newsurl]',0,'".addslashes($keyboard)."','$newstime',$truetime,'$tmptime',$userid,'".addslashes($username)."'".$ivalue.");");
    }
	//采集结束
	if(empty($b))
	{
		//删除记录
		$del=$Elves->query("delete from {$dbtbpre}melvelinktmp where checkrnd='$checkrnd'");
		//更新最后时间
		$ucjsql=$Elves->query("update {$dbtbpre}melveinfoclass set lasttime='".time()."' where classid='$classid'");
		//自动入库
		if($r['justloadin'])
		{
			echo $fun_r['cjLoadInInfos']."<script>self.location.href='elvecj.php?melve=CjNewsIn_all&classid=$classid&checked=$r[justloadcheck]&fm=1';</script>";
			exit();
		}
		//单个节点
	...
	//echo"(ID:<font color=red><b>".$newstart."</b></font>)<script>self.location.href='elvecj.php?melve=GetNewsInfo&checkrnd=$checkrnd&classid=$classid&start=$newstart';</script>";
	echo"<meta http-equiv=\"refresh\" content=\"".$r['keeptime'].";url=elvecj.php?melve=GetNewsInfo&checkrnd=$checkrnd&classid=$classid&start=$newstart\">".$r[classname]." (ID:<font color=red><b>".$newstart."</b></font>)";
	exit();
}
...
function ViewGetNewsInfo($classid,$newspage,$userid,$username){
	...
	$record="<!--record-->";
	$field="<!--field--->";
	$mr=$Elves->fetch1("select cj from {$dbtbpre}melvemod where mid='".$class_r[$r[newsclassid]][modid]."'");
	$cjr=explode($record,$mr[cj]);
	$count=count($cjr);
	//采集时间
	$newstime=date("Y-m-d H:i:s");
	...
	if(empty($r[recjtheurl]))
	{
		$tmpnum=$Elves->gettotal("select count(*) as total from {$dbtbpre}elve_infotmp_".$r[tbname]." where oldurl='$newspage' limit 1");
		if($tmpnum)
		{
			echo $fun_r['ReCj'];
			exit();
...
function LoadOutCj($classid,$userid,$username){
	...
	//副表
	$infocr=$Elves->fetch1("select * from {$dbtbpre}elve_infoclass_".$cr[tbname]." where classid='$classid'");
	...
	$cjstr=ElvesCMS_VERSION.$recordexp.$mainstr.$recordexp.$datafield.$recordexp.$datastr;
	$file=$cr['tbname'].time().".cj";
	$filepath=elve_PATH."core/data/tmp/cj/".$file;
...
function LoadOutCjDataField($tid,$tbname){
	...
	$field='';
	$dh='';
	$fsql=$Elves->query("select f from {$dbtbpre}melvef where tid='$tid' and iscj=1");
...
function LoadInCj($add,$file,$file_name,$file_type,$file_size,$userid,$username){
	...
	$cr=$Elves->fetch1("select classid,islast,tid,tbname from {$dbtbpre}melveclass where classid='$classid' and islast=1");
	if(!$cr['classid'])
	{
	...
	$path=elve_PATH.'core/data/tmp/cj/uploadcj'.time().'.cj';
	//上传文件
	...
	$cjr=$Elves->fetch1("select classname from {$dbtbpre}melveinfoclass where classid='$infoclassid'");
	//操作日志
...
function LoadInCjInsertMainstr($classid,$tid,$tbname,$mainstr,$fieldexp,$sfieldexp){
	...
	$Elves->query("insert into {$dbtbpre}melveinfoclass(classid,bclassid,newsclassid,tid,tbname,".$mainfield.") values(NULL,'0','$classid','$tid','$tbname'".$mainvalues.");");
	$lastid=$Elves->lastid();
	return $lastid;
}
...
function LoadInCjInsertDatastr($classid,$tid,$tbname,$infoclassid,$dataf,$datastr,$fieldexp,$sfieldexp){
	...
	if(empty($dataf))
	{
		$Elves->query("insert into {$dbtbpre}elve_infoclass_".$tbname."(classid) values('$infoclassid');");
		return '';
	}
	//返回表字段
	...
	$Elves->query("insert into {$dbtbpre}elve_infoclass_".$tbname."(classid".$datafield.") values('$infoclassid'".$datavalues.");");
}
?>