...
function EditGbooktemp($temptext,$userid,$username){
	...
	$gid=(int)$_POST['gid'];
	$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set gbooktemp='".eaddslashes2($temptext)."' limit 1");
	//备份模板
...
function EditCptemp($temptext,$userid,$username){
	...
	$temptext=RepPhpAspJspcode($temptext);
	$gid=(int)$_POST['gid'];
	$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set cptemp='".eaddslashes2($temptext)."' limit 1");
	//备份模板
...
function EditLoginIframe($temptext,$userid,$username){
	...
	$temptext=RepPhpAspJspcode($temptext);
	$gid=(int)$_POST['gid'];
	$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set loginiframe='".eaddslashes2($temptext)."' limit 1");
	//备份模板
...
function EditLoginJstemp($temptext,$userid,$username){
	...
	$temptext=RepPhpAspJspcode($temptext);
	$gid=(int)$_POST['gid'];
	$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set loginjstemp='".eaddslashes2($temptext)."' limit 1");
	//备份模板
...
function EditSchallTemp($temptext,$sub,$formatdate,$userid,$username){
	...
	$gid=(int)$_POST['gid'];
	$sub=(int)$sub;
	$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set schalltemp='".eaddslashes2($temptext)."',schallsubnum='$sub',schalldate='".eaddslashes($formatdate)."' limit 1");
	//备份模板
...
function AddBq($add,$bqsay,$userid,$username){
	...
	//标签重复
	$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvebq where bq='$add[bq]' limit 1");
	if($num)
	...
	$sql=$Elves->query("insert into {$dbtbpre}melvebq(bqname,bqsay,funname,bq,issys,bqgs,isclose,classid,myorder) values('".$add[bqname]."','".eaddslashes2($bqsay)."','$add[funname]','$add[bq]',0,'".eaddslashes2($add[bqgs])."',$add[isclose],$classid,'$myorder');");
	$bqid=$Elves->lastid();
	if($sql)
	{
		//操作日志
		insert_dolog("bqid=".$bqid."<br>bqname=".$add[bqname]);
		printerror("AddBqSuccess","template/AddBq.php?melve=AddBq");
	}
	else
	{printerror("DbError","history.go(-1)");}
}
...
function EditBq($add,$bqsay,$userid,$username){
	...
	CheckLevel($userid,$username,$classid,"bq");
	//标签重复
	$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvebq where bq='$add[bq]' and bqid<>'$add[bqid]' limit 1");
	if($num)
	{printerror("ReBq","history.go(-1)");}
	//函数是否存在
	if(!function_exists($add[funname]))
	...
	$add[isclose]=(int)$add[isclose];
	$myorder=(int)$add[myorder];
	$sql=$Elves->query("update {$dbtbpre}melvebq set bqname='$add[bqname]',bqsay='".eaddslashes2($bqsay)."',funname='$add[funname]',bq='$add[bq]',bqgs='".eaddslashes2($add[bqgs])."',isclose=$add[isclose],classid=$classid,myorder='$myorder' where bqid='$add[bqid]'");
	if($sql)
	{
		//操作日志
...
function DelBq($bqid,$cid,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"bq");
	$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvebq where bqid='$bqid' and issys=0");
	if(empty($num))
	{printerror("NotDelSysBq","history.go(-1)");}
	$r=$Elves->fetch1("select bqname from {$dbtbpre}melvebq where bqid='$bqid'");
	$sql=$Elves->query("delete from {$dbtbpre}melvebq where bqid='$bqid'");
	if($sql)
	{
		//操作日志
...
function EditSearchTemp($tempname,$temptext,$userid,$username){
	...
	$gid=(int)$_POST['gid'];
	$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set ".$f."='".eaddslashes2($temptext)."'");
	//备份模板
...
function EditOtherLinkTemp($tempname,$temptext,$userid,$username){
	...
	$gid=(int)$_POST['gid'];
	$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set ".$f."='".eaddslashes2($temptext)."',otherlinktempsub=$otherlinktempsub,otherlinktempdate='".eaddslashes($otherlinktempdate)."'");
	//备份模板
...
function EditOtherPubTemp($tempname,$temptext,$userid,$username){
	...
	$gid=(int)$_POST['gid'];
	$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set ".$tempname."='".eaddslashes2($temptext)."'");
	//备份模板
	AddEBakTemp($temptype,$gid,1,'',$temptext,0,0,'',0,0,'',0,0,0,$userid,$username);
	if($gid==$public_r['deftempid']||(!$public_r['deftempid']&&($gid==1||$gid==0)))
	{
...
function EditIndextemp($temptext,$userid,$username){
	...
	$temptext=RepPhpAspJspcode($temptext);
	$gid=(int)$_POST['gid'];
	$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set indextemp='".eaddslashes2($temptext)."'");
	//备份模板
...
function PreviewIndexpage($tempid,$userid,$username){
	...
	$tempr=$Elves->fetch1("select tempid,temptext from {$dbtbpre}melveindexpage where tempid='$tempid'");
	if(!$tempr['tempid'])
	{
		printerror('ErrorUrl','');
	}
	...
	$onclick='';
	$file=elve_PATH.'core/data/tmp/indexpage'.$tempid.'.php';
...
function LoadTempInClass($path,$start,$userid,$username){
	...
	$b=0;
	$sql=$Elves->query("select classid,classtempid,islist from {$dbtbpre}melveclass where islast=0 and islist<>1 and classid>$start order by classid limit ".$public_r[loadtempnum]);
	while($r=$Elves->fetch($sql))
	{
		...
		if(file_exists($file))
		{
			...
			if($r[islist]==2)
			{
				$usql=$Elves->query("update {$dbtbpre}melveclassadd set classtext='".$data."' where classid='$r[classid]'");
			}
			else
			{
				$usql=$Elves->query("update {$dbtbpre}melveclasstemp set temptext='".$data."' where tempid='$r[classtempid]'");
			}
	...
	echo $fun_r['LoadOneTempSuccess']."(ID:<font color=red><b>".$newstart."</b></font>)<script>self.location.href='elvetemp.php?melve=LoadTempInClass&start=$newstart';</script>";
	exit();
}
...
function ChangeClassListtemp($classid,$listtempid,$userid,$username){
	...
	$sql=$Elves->query("update {$dbtbpre}melveclass set listtempid=$listtempid where ".$where);
	GetClass();
	if($sql)
	{
		//操作日志
...
function LoadOutBq($add,$userid,$username){
	...
	//验证权限
    CheckLevel($userid,$username,$classid,"bq");
	$r=$Elves->fetch1("select bqid,bqname,bqsay,funname,bq,bqgs from {$dbtbpre}melvebq where bqid=$bqid");
	if(!$r[bqid])
	{
		printerror("NotThisBqid","history.go(-1)");
	}
	$add['funvalue']=ClearAddsData($add['funvalue']);
	$field="<!--#Elvescms.bq-webelves.org#--!>";
	...
	$filepath=elve_PATH.'core/data/tmp/temp/'.$filename;
...
function LoadInBq($add,$file,$file_name,$file_type,$file_size,$userid,$username){
	...
	$field="<!--#Elvescms.bq-webelves.org#--!>";
	$path=elve_PATH.'core/data/tmp/temp/uploadbq'.time().'.bq';
	//上传文件
	...
	$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvebq where bq='$r[3]' or funname='$r[2]' limit 1");
	if($num)
	{
		printerror("ReLoadInBq","history.go(-1)");
	}
	$sql=$Elves->query("insert into {$dbtbpre}melvebq(bqname,bqsay,funname,bq,issys,bqgs,isclose,classid,myorder) values('".addslashes($r[0])."','".addslashes(addslashes($r[1]))."','".addslashes($r[2])."','".addslashes($r[3])."',0,'".addslashes(addslashes($r[4]))."',0,$classid,0);");
	$bqid=$Elves->lastid();
	//操作日志
...
function DoRepTemp($add,$userid,$username){
	...
	if($add['indextemp']||$add['cptemp']||$add['sformtemp']||$add['otherlinktemp']||$add['gbooktemp']||$add['loginiframe']||$add['pljstemp']||$add['schalltemp']||$add['loginjstemp']||$add['downpagetemp'])
	{
		...
		$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set id=1".$set." limit 1");
	}
	//修改栏目封面模板
	if($add['classtemp'])
	{
		$Elves->query("update ".GetDoTemptb("melveclasstemp",$gid)." set temptext=REPLACE(temptext,'".$oldword."','".$newword."')");
    }
	//修改标签模板
	if($add['bqtemp'])
	{
		$Elves->query("update ".GetDoTemptb("melvebqtemp",$gid)." set temptext=REPLACE(temptext,'".$oldword."','".$newword."'),listvar=REPLACE(listvar,'".$oldword."','".$newword."')");
    }
	//修改列表模板
	if($add['listtemp'])
	{
		$Elves->query("update ".GetDoTemptb("melvelisttemp",$gid)." set temptext=REPLACE(temptext,'".$oldword."','".$newword."'),listvar=REPLACE(listvar,'".$oldword."','".$newword."')");
    }
	//修改内容模板
	if($add['newstemp'])
	{
		$Elves->query("update ".GetDoTemptb("melvenewstemp",$gid)." set temptext=REPLACE(temptext,'".$oldword."','".$newword."')");
	}
	//修改搜索模板
	if($add['searchtemp'])
	{
		$Elves->query("update ".GetDoTemptb("melvesearchtemp",$gid)." set temptext=REPLACE(temptext,'".$oldword."','".$newword."'),listvar=REPLACE(listvar,'".$oldword."','".$newword."')");
	}
	//修改自定义页面
	if($add['userpage'])
	{
		$Elves->query("update {$dbtbpre}melvepage set pagetext=REPLACE(pagetext,'".$oldword."','".$newword."')");
	}
	//修改自定义页面模板
	if($add['pagetemp'])
	{
		$Elves->query("update ".GetDoTemptb("melvepagetemp",$gid)." set temptext=REPLACE(temptext,'".$oldword."','".$newword."')");
    }
	//评论列表模板
	if($add['pltemp'])
	{
		$Elves->query("update ".GetDoTemptb("melvepltemp",$gid)." set temptext=REPLACE(temptext,'".$oldword."','".$newword."')");
    }
	//打印模板
	if($add['printtemp'])
	{
		$Elves->query("update ".GetDoTemptb("melveprinttemp",$gid)." set temptext=REPLACE(temptext,'".$oldword."','".$newword."')");
    }
	//模板变量
	if($add['tempvar'])
	{
		$Elves->query("update ".GetDoTemptb("melvetempvar",$gid)." set varvalue=REPLACE(varvalue,'".$oldword."','".$newword."')");
	}
	//修改JS模板
	if($add['jstemp'])
	{
		$Elves->query("update ".GetDoTemptb("melvejstemp",$gid)." set temptext=REPLACE(temptext,'".$oldword."','".$newword."')");
    }
	//修改投票模板
	if($add['votetemp'])
	{
		$Elves->query("update ".GetDoTemptb("melvevotetemp",$gid)." set temptext=REPLACE(temptext,'".$oldword."','".$newword."')");
    }
	//反馈表单模板
	if($add['feedbackbtemp'])
	{
		$Elves->query("update {$dbtbpre}melvefeedbackclass set btemp=REPLACE(btemp,'".$oldword."','".$newword."')");
	}
	//操作日志
	insert_dolog("gid=$gid");
...
function EditTempGroup($add,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		$usql=$Elves->query("update {$dbtbpre}melvetempgroup set gname='".$gname[$i]."' where gid='".$gid[$i]."'");
	}
	//操作日志
	insert_dolog("");
...
function DefTtempGroup($add,$userid,$username){
	...
	$usql=$Elves->query("update {$dbtbpre}melvetempgroup set isdefault=0");
	$sql=$Elves->query("update {$dbtbpre}melvetempgroup set isdefault=1 where gid=$gid");
	$upsql=$Elves->query("update {$dbtbpre}melvepublic set deftempid=$gid limit 1");
	if($usql&&$sql&&$upsql)
	{
		GetConfig();
		//操作日志
...
function ReturnTemptbList(){
	$templist="melvebqtemp,melvejstemp,melvelisttemp,melvenewstemp,melvepubtemp,melvesearchtemp,melvetempvar,melvevotetemp,melveclasstemp,melvepltemp,melveprinttemp,melvepagetemp";
	return $templist;
}
...
function DelTempTb($gid){
	...
	$sql=$Elves->query("DROP TABLE IF EXISTS ".$droptb.";");
	return $sql;
}
...
function DelTempGroup($add,$userid,$username){
	...
	$r=$Elves->fetch1("select gid,gname,isdefault from {$dbtbpre}melvetempgroup where gid=$gid");
	if(!$r['gid'])
	{
		printerror("EmptyDelTempGroup","");
	}
	$sql=$Elves->query("delete from {$dbtbpre}melvetempgroup where gid=$gid");
	if($r['isdefault'])
	{
		$upsql=$Elves->query("update {$dbtbpre}melvepublic set deftempid=0 limit 1");
		GetConfig();
	}
	DelTempTb($gid);
	//删除备份记录
	$Elves->query("delete from {$dbtbpre}melvetempbak where gid='$gid'");
	if($sql)
	{
		//操作日志
		insert_dolog("gid=$gid&gname=$r[gname]");
...
function LoadTempGroup($add,$userid,$username){
	...
	$file="e".time().".temp";
	$filepath=elve_PATH.'core/data/tmp/temp/'.$file;
...
function LoadInTempChangeChar($tempchar,$text){
	global $elve_config;
	@include_once(elve_PATH.'core/class/ElvesCMS_version.php');
	if($tempchar=='GB2312')//简体GB2312
	{
		...
		if($elve_config['sets']['pagechar']=='utf-8')
		{
			if(ElvesCMS_CHARVER=='TC-UTF-8')//繁体
			{
		...
		elseif($elve_config['sets']['pagechar']=='big5')
		{
			$text=DoIconvVal("GB2312","BIG5",$text,1);
	...
	{
		if($elve_config['sets']['pagechar']=='utf-8'&&ElvesCMS_CHARVER=='UTF-8')
		{
			return $text;
		}
		if($elve_config['sets']['pagechar']=='gb2312')
		...
		elseif($elve_config['sets']['pagechar']=='utf-8')//繁体
		{
			$text=DoIconvVal("UTF8","GB2312",$text,1);
			$text=DoIconvVal("GB2312","BIG5",$text);
			$text=DoIconvVal("BIG5","UTF8",$text);
	...
	{
		if($elve_config['sets']['pagechar']=='big5')
		{
			return $text;
		}
		if($elve_config['sets']['pagechar']=='gb2312')
		...
		elseif($elve_config['sets']['pagechar']=='utf-8')
		{
			if(ElvesCMS_CHARVER=='UTF-8')//简体
			{
				$text=DoIconvVal("BIG5","GB2312",$text,1);
	...
	{
		if($elve_config['sets']['pagechar']=='utf-8'&&ElvesCMS_CHARVER=='TC-UTF-8')
		{
			return $text;
		}
		if($elve_config['sets']['pagechar']=='gb2312')
		...
		elseif($elve_config['sets']['pagechar']=='utf-8')//简体
		{
			$text=DoIconvVal("UTF8","BIG5",$text,1);
			$text=DoIconvVal("BIG5","GB2312",$text);
			$text=DoIconvVal("GB2312","UTF8",$text);
...
function LoadInTGReptext_pubvar($text){
	//shop
	$text=str_replace('/melve/?melve=AddBuycar','/ShopSys/doaction.php?melve=AddBuycar',$text);
	$text=str_replace('/melve?melve=AddBuycar','/ShopSys/doaction.php?melve=AddBuycar',$text);
	//pl
	$text=str_replace('/melve/?melve=DoForPl','/pl/doaction.php?melve=DoForPl',$text);
	$text=str_replace('/melve?melve=DoForPl','/pl/doaction.php?melve=DoForPl',$text);
	//member
	$text=str_replace('/melve/?melve=exit','/member/doaction.php?melve=exit',$text);
	$text=str_replace('/melve?melve=exit','/member/doaction.php?melve=exit',$text);
	//report
...
function LoadInTGReptext_othervar($text,$type='pl'){
	if($type=='pl')
	{
		$text=str_replace('/melve/index.php','/pl/doaction.php',$text);
	}
	elseif($type=='member')
	{
		$text=str_replace('/melve/index.php','/member/doaction.php',$text);
	}
	elseif($type=='shop')
	{
		$text=str_replace('/melve/index.php','/ShopSys/doaction.php',$text);
	}
	return $text;
}
...
function LoadTGAddVer(){
	@include_once(elve_PATH.'core/class/ElvesCMS_version.php');
	$ver=' ,-- '.ElvesCMS_VERSION.','.ElvesCMS_CHARVER;
	return $ver;
}
...
function LoadInTempGroup($add,$file,$file_name,$file_type,$file_size,$userid,$username){
	...
	//上传文件
	$path=elve_PATH.'core/data/tmp/temp/uploadtg'.time().make_password(10).'.temp';
	$cp=@move_uploaded_file($file,$path);
	DoChmodFile($path);
	$data=ReadFiletext($path);
	DelFiletext($path);
	...
	if(empty($gid))//新建模板组
	{
		$sql=$Elves->query("insert into {$dbtbpre}melvetempgroup(gname,isdefault) values('".addslashes($gname)."',0);");
		$gid=$Elves->lastid();
		$gname=$pr[0];
		$en="_".$gid;
...
function LoadTGBqtemp($gid,$en,$pageexp,$record,$field){
	global $Elves,$dbtbpre;
	$tb=$dbtbpre."melvebqtemp".$en;
	$sql=$Elves->query("select * from ".$tb." order by tempid");
	$classid=0;
	while($r=$Elves->fetch($sql))
	{
...
function LoadInTGBqtemp($gid,$en,$record,$field,$text){
	...
	for($i=0;$i<$count-1;$i++)
	{
		...
		$sql=$Elves->query("insert into ".$tb."(tempid,tempname,modid,temptext,showdate,listvar,subnews,rownum,classid,docode) values('$r[0]','".addslashes($r[1])."','$r[2]','".addslashes(addslashes($r[3]))."','".addslashes($r[4])."','".addslashes(addslashes($r[5]))."','$r[6]','$r[7]','$r[8]','$r[9]');");
	}
}
...
function LoadInTGJstemp($gid,$en,$record,$field,$text){
	...
	for($i=0;$i<$count-1;$i++)
	{
		...
		$sql=$Elves->query("insert into ".$tb."(tempid,tempname,temptext,classid,isdefault,showdate,modid,subnews,subtitle) values('$r[0]','".addslashes($r[1])."','".addslashes(addslashes($r[2]))."','$r[3]','$r[4]','".addslashes($r[5])."','$r[6]','$r[7]','$r[8]');");
	}
}
...
function LoadInTGListtemp($gid,$en,$record,$field,$text){
	...
	{
		$r=explode($field,$rr[$i]);
		$sql=$Elves->query("insert into ".$tb."(tempid,tempname,temptext,subnews,isdefault,listvar,rownum,modid,showdate,subtitle,classid,docode) values('$r[0]','".addslashes($r[1])."','".addslashes(addslashes($r[2]))."','$r[3]','$r[4]','".addslashes(addslashes($r[5]))."','$r[6]','$r[7]','".addslashes($r[8])."','$r[9]','$r[10]','$r[11]');");
	}
}
...
function LoadInTGNewstemp($gid,$en,$record,$field,$text){
	...
	{
		$r=explode($field,$rr[$i]);
		$sql=$Elves->query("insert into ".$tb."(tempid,tempname,isdefault,temptext,showdate,modid,classid) values('$r[0]','".addslashes($r[1])."','$r[2]','".addslashes(addslashes($r[3]))."','".addslashes($r[4])."','$r[5]','$r[6]');");
	}
}
...
function LoadTGPubtemp($gid,$en,$pageexp,$record,$field){
	global $Elves,$dbtbpre;
	$tb=$dbtbpre."melvepubtemp".$en;
	$r=$Elves->fetch1("select * from ".$tb." limit 1");
...
function LoadInTGPubtemp($gid,$en,$record,$field,$text,$isold=0){
	...
	if($isold==1&&$r[2])
	{
		...
		$Elves->query("insert into ".$pltb."(tempid,tempname,temptext,isdefault) values(NULL,'".addslashes($pltempname)."','".addslashes(addslashes($r[2]))."',1);");
	}
	//6.0以下版本
	if(($isold==1||$isold==2)&&$r[8])
	{
		$printtb=$dbtbpre."melveprinttemp".$en;
		$printtempname=$fun_r['PrintTempname'];
		$Elves->query("insert into ".$printtb."(tempid,tempname,temptext,isdefault,showdate,modid) values(NULL,'".addslashes($printtempname)."','".addslashes(addslashes($r[8]))."',1,'Y-m-d H:i:s',1);");
	}
}
...
function LoadTGTempvar($gid,$en,$pageexp,$record,$field){
	global $Elves,$dbtbpre;
	$tb=$dbtbpre."melvetempvar".$en;
	$sql=$Elves->query("select * from ".$tb." order by varid");
	$classid=0;
	while($r=$Elves->fetch($sql))
	{
...
function LoadInTGTempvar($gid,$en,$record,$field,$text){
	...
	for($i=0;$i<$count-1;$i++)
	{
		...
		$sql=$Elves->query("insert into ".$tb."(varid,myvar,varname,varvalue,classid,isclose,myorder) values('$r[0]','".addslashes($r[1])."','".addslashes($r[2])."','".addslashes(addslashes($r[3]))."','$r[4]','$r[5]','$r[6]');");
	}
}
...
function LoadInTGVotetemp($gid,$en,$record,$field,$text){
	...
	{
		$r=explode($field,$rr[$i]);
		$sql=$Elves->query("insert into ".$tb."(tempid,tempname,temptext) values('$r[0]','".addslashes($r[1])."','".addslashes(addslashes($r[2]))."');");
	}
}
...
function LoadInTGClasstemp($gid,$en,$record,$field,$text){
	...
	{
		$r=explode($field,$rr[$i]);
		$sql=$Elves->query("insert into ".$tb."(tempid,tempname,temptext,classid) values('$r[0]','".addslashes($r[1])."','".addslashes(addslashes($r[2]))."','$r[3]');");
	}
}
...
function LoadInTGPltemp($gid,$en,$record,$field,$text){
	...
	for($i=0;$i<$count-1;$i++)
	{
		...
		$sql=$Elves->query("insert into ".$tb."(tempid,tempname,temptext,isdefault) values('$r[0]','".addslashes($r[1])."','".addslashes(addslashes($r[2]))."','$r[3]');");
	}
}
...
function LoadInTGPrinttemp($gid,$en,$record,$field,$text){
	...
	{
		$r=explode($field,$rr[$i]);
		$sql=$Elves->query("insert into ".$tb."(tempid,tempname,temptext,isdefault,showdate,modid) values('$r[0]','".addslashes($r[1])."','".addslashes(addslashes($r[2]))."','$r[3]','".addslashes($r[4])."','$r[5]');");
	}
}
...
function DelEBakTemp($temptype,$gid,$tempid){
	global $Elves,$dbtbpre;
	$pr=$Elves->fetch1("select baktempnum from {$dbtbpre}melvepublic limit 1");
	if(!$pr['baktempnum'])
	{
		return $pr;
	}
	$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvetempbak where temptype='$temptype' and gid='$gid' and tempid='$tempid'");
	if($num+1>$pr['baktempnum'])
	{
		$limitnum=$num+1-$pr['baktempnum'];
		$ids='';
		$dh='';
		$sql=$Elves->query("select bid from {$dbtbpre}melvetempbak where temptype='$temptype' and gid='$gid' and tempid='$tempid' order by bid limit ".$limitnum);
		while($r=$Elves->fetch($sql))
		{
			$ids.=$dh.$r['bid'];
			$dh=',';
		}
		$Elves->query("delete from {$dbtbpre}melvetempbak where bid in ($ids)");
	}
	return $pr;
}
...
function DelEbakTempAll($temptype,$gid,$tempid){
	...
	$Elves->query("delete from {$dbtbpre}melvetempbak where temptype='$temptype' and gid='$gid' and tempid='$tempid'");
}
...
function AddEBakTemp($temptype,$gid,$tempid,$tempname,$temptext,$subnews,$isdefault,$listvar,$rownum,$modid,$showdate,$subtitle,$classid,$docode,$userid,$username){
	...
	$docode=(int)$docode;
	$baktime=time();
	$Elves->query("insert into {$dbtbpre}melvetempbak(tempid,tempname,temptext,subnews,isdefault,listvar,rownum,modid,showdate,subtitle,classid,docode,baktime,temptype,gid,lastuser) values('$tempid','".eaddslashes($tempname)."','".eaddslashes2($temptext)."','$subnews','$isdefault','".eaddslashes2($listvar)."','$rownum','$modid','".eaddslashes($showdate)."','$subtitle','$classid','$docode','$baktime','$temptype','$gid','$username');");
}
...
function ReEBakTemp($add,$userid,$username){
	...
	$r=$Elves->fetch1("select * from {$dbtbpre}melvetempbak where bid='$bid'");
	if(!$r['bid'])
	{
		printerror("NotBakTemp","history.go(-1)");
	}
	...
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvebqtemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".addslashes($r[temptext])."',modid='$r[modid]',showdate='".StripAddsData($r[showdate])."',listvar='".addslashes($r[listvar])."',subnews='$r[subnews]',rownum='$r[rownum]',classid='$r[classid]',docode='$r[docode]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='classtemp')//封面模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melveclasstemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".addslashes($r[temptext])."',classid='$r[classid]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='jstemp')//JS模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvejstemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".addslashes($r[temptext])."',classid='$r[classid]',showdate='".StripAddsData($r[showdate])."',modid='$r[modid]',subnews='$r[subnews]',subtitle='$r[subtitle]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='listtemp')//列表模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvelisttemp",$gid)." set subnews='$r[subnews]',tempname='".StripAddsData($r[tempname])."',temptext='".addslashes($r[temptext])."',listvar='".addslashes($r[listvar])."',rownum='$r[rownum]',modid='$r[modid]',showdate='".StripAddsData($r[showdate])."',subtitle='$r[subtitle]',classid='$r[classid]',docode='$r[docode]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='newstemp')//内容模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvenewstemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".addslashes($r[temptext])."',showdate='".StripAddsData($r[showdate])."',modid='$r[modid]',classid='$r[classid]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='pltemp')//评论模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepltemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".addslashes($r[temptext])."' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='printtemp')//打印模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melveprinttemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".addslashes($r[temptext])."',showdate='".StripAddsData($r[showdate])."',modid='$r[modid]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='searchtemp')//搜索模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvesearchtemp",$gid)." set subnews='$r[subnews]',tempname='".StripAddsData($r[tempname])."',temptext='".addslashes($r[temptext])."',listvar='".addslashes($r[listvar])."',rownum='$r[rownum]',modid='$r[modid]',showdate='".StripAddsData($r[showdate])."',subtitle='$r[subtitle]',classid='$r[classid]',docode='$r[docode]' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='tempvar')//公共模板变量
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvetempvar",$gid)." set myvar='".StripAddsData($r[tempname])."',varname='".StripAddsData($r[listvar])."',varvalue='".addslashes($r[temptext])."',classid='$r[classid]',isclose='$r[docode]',myorder='$r[subnews]' where varid='$r[tempid]'");
	}
	elseif($r['temptype']=='votetemp')//投票模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvevotetemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".addslashes($r[temptext])."' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='pagetemp')//自定义页面模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepagetemp",$gid)." set tempname='".StripAddsData($r[tempname])."',temptext='".addslashes($r[temptext])."' where tempid='$r[tempid]'");
	}
	elseif($r['temptype']=='indexpage')//首页方案模板
	{
		$sql=$Elves->query("update {$dbtbpre}melveindexpage set tempname='".StripAddsData($r[tempname])."',temptext='".addslashes($r[temptext])."' where tempid='$r[tempid]'");
	}
	//公共模板
	elseif($r['temptype']=='pubindextemp')//首页模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set indextemp='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubcptemp')//控制面板模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set cptemp='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubsearchtemp')//高级搜索表单模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set searchtemp='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubsearchjstemp')//搜索JS模板[横向]
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set searchjstemp='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubsearchjstemp1')//搜索JS模板[纵向]
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set searchjstemp1='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubotherlinktemp')//相关链接模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set otherlinktemp='".addslashes($r[temptext])."',otherlinktempsub='$r[subtitle]',otherlinktempdate='".StripAddsData($r[showdate])."' limit 1");
	}
	elseif($r['temptype']=='pubdownsofttemp')//下载地址模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set downsofttemp='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubonlinemovietemp')//在线播放地址模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set onlinemovietemp='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='publistpagetemp')//列表分页模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set listpagetemp='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubpljstemp')//评论JS调用模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set pljstemp='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubdownpagetemp')//最终下载页模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set downpagetemp='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubgbooktemp')//留言板模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set gbooktemp='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='publoginiframe')//登陆状态模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set loginiframe='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='publoginjstemp')//JS调用登陆状态模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set loginjstemp='".addslashes($r[temptext])."' limit 1");
	}
	elseif($r['temptype']=='pubschalltemp')//全站搜索模板
	{
		$sql=$Elves->query("update ".GetDoTemptb("melvepubtemp",$gid)." set schalltemp='".addslashes($r[temptext])."',schallsubnum='$r[subnews]',schalldate='".StripAddsData($r[showdate])."' limit 1");
	}
	if($sql)
	{
		//操作日志
