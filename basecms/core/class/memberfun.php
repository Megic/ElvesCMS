...
function AddMemberF($add,$userid,$username){
	...
	//字段是否重复
	$s=$Elves->query("SHOW FIELDS FROM {$dbtbpre}melvememberadd");
	$b=0;
	...
	$s=$Elves->query("SHOW FIELDS FROM {$dbtbpre}melvemember");
	$b=0;
	while($r=$Elves->fetch($s))
	{
		if($r[Field]==$add[f])
	...
	//新增字段
	$asql=$Elves->query("alter table {$dbtbpre}melvememberadd add ".$field);
	//替换代码
	...
	$sql=$Elves->query("insert into {$dbtbpre}melvememberf(f,fname,fform,fhtml,fzs,myorder,ftype,flen,fvalue,fformsize) values('$add[f]','$add[fname]','$add[fform]','".eaddslashes2($fhtml)."','".eaddslashes($add[fzs])."',$add[myorder],'$add[ftype]','$add[flen]','".eaddslashes2($add[fvalue])."','$add[fformsize]');");
	$lastid=$Elves->lastid();
	if($asql&&$sql)
	{
		//操作日志
		insert_dolog("fid=".$lastid."<br>f=".$add[f]);
		printerror("AddFSuccess","member/AddMemberF.php?melve=AddMemberF");
	}
	else
	{
...
function EditMemberF($add,$userid,$username){
	...
	$field="`".$add[f]."` ".$type." NOT NULL".$def;
	$usql=$Elves->query("alter table {$dbtbpre}melvememberadd change `".$add[oldf]."` ".$field);
	//替换代码
	...
	//处理变量
	$add[myorder]=(int)$add[myorder];
	$sql=$Elves->query("update {$dbtbpre}melvememberf set f='$add[f]',fname='$add[fname]',fform='$add[fform]',fhtml='".eaddslashes2($fhtml)."',fzs='".eaddslashes($add[fzs])."',myorder=$add[myorder],ftype='$add[ftype]',flen='$add[flen]',fvalue='".eaddslashes2($add[fvalue])."',fformsize='$add[fformsize]' where fid=$fid");
	//更新表单
	$record="<!--record-->";
	$field="<!--field--->";
	...
	$slike=",".$add[oldf].",";
	$newslike=",".$add[f].",";
	$csql=$Elves->query("select fid,ftemp,enter,mustenter,filef,imgf,tobrf,viewenter,searchvar,canaddf,caneditf,checkboxf from {$dbtbpre}melvememberform where enter like '%$like%'");
	while($cr=$Elves->fetch($csql))
	{
		...
		$cusql=$Elves->query("update {$dbtbpre}melvememberform set enter=REPLACE(enter,'$like','$newlike')".$setf." where fid='$cr[fid]'");
		//生成表单页面
...
function DelMemberF($add,$userid,$username){
	...
	$r=$Elves->fetch1("select f from {$dbtbpre}melvememberf where fid=$fid");
	if(!$r[f]){
		printerror("EmptyFid","history.go(-1)");
	}
	$usql=$Elves->query("alter table {$dbtbpre}melvememberadd drop COLUMN `".$r[f]."`");
	$sql=$Elves->query("delete from {$dbtbpre}melvememberf where fid=$fid");
	//更新表单表
	$record="<!--record-->";
	$field="<!--field--->";
	...
	while($cr=$Elves->fetch($csql))
	{
		...
		$cusql=$Elves->query("update {$dbtbpre}melvememberform set enter='$enter'".$setf." where fid='$cr[fid]'");
	}
	if($usql&&$sql)
	{
...
function EditMemberFOrder($fid,$myorder,$userid,$username){
	...
	for($i=0;$i<count($myorder);$i++)
	{
		...
		$usql=$Elves->query("update {$dbtbpre}melvememberf set myorder=$newmyorder where fid='$fid[$i]'");
    }
...
function ChangeMemberForm($fid,$mtemp){
	...
	$sql=$Elves->query("select f,fhtml from {$dbtbpre}melvememberf");
...
function AddMemberForm($add,$userid,$username){
	...
	$filef=ReturnMFileF($enter,$dbtbpre."melvememberf",0,"file");
	$imgf=ReturnMFileF($enter,$dbtbpre."melvememberf",0,"img");
	$tobrf=ReturnMFileF($enter,$dbtbpre."melvememberf",0,"textarea");
	$checkboxf=ReturnMFileF($enter,$dbtbpre."melvememberf",0,"checkbox");
	//自动生成表单
	if($add[ftype])
	{
		$add[ftemp]=ReturnMemberFtemp($add['cname'],$add['center']);
	}
	$sql=$Elves->query("insert into {$dbtbpre}melvememberform(fname,ftemp,fzs,enter,mustenter,filef,imgf,tobrf,viewenter,searchvar,canaddf,caneditf,checkboxf) values('$add[fname]','".eaddslashes2($add[ftemp])."','".addslashes($add[fzs])."','$enter','$mustenter','$filef','$imgf','$tobrf','$viewenter','$searchvar','$canaddf','$caneditf','$checkboxf');");
	$fid=$Elves->lastid();
	//生成表单页面
	...
	if($sql)
	{
		...
		printerror("AddMemberFormSuccess","member/AddMemberForm.php?melve=AddMemberForm");
	}
	else
	{printerror("DbError","history.go(-1)");}
}
...
function EditMemberForm($add,$userid,$username){
	...
	$sql=$Elves->query("update {$dbtbpre}melvememberform set fname='$add[fname]',ftemp='".eaddslashes2($add[ftemp])."',fzs='".addslashes($add[fzs])."',enter='$enter',mustenter='$mustenter',filef='$filef',imgf='$imgf',tobrf='$tobrf',viewenter='$viewenter',searchvar='$searchvar',canaddf='$canaddf',caneditf='$caneditf',checkboxf='$checkboxf' where fid=$fid");
	//生成表单页面
	ChangeMemberForm($fid,$add[ftemp]);
	if($sql)
	{
...
function DelMemberForm($add,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"memberf");
	$r=$Elves->fetch1("select fid,fname from {$dbtbpre}melvememberform where fid=$fid;");
	if(!$r['fid'])
	{
		printerror("EmptyMemberFormId","history.go(-1)");
	}
	$sql=$Elves->query("delete from {$dbtbpre}melvememberform where fid=$fid;");
	//删除表单文件
	$file="../data/html/memberform".$fid.".php";
	DelFiletext($file);
	if($sql)
...
function AddMemberGroup($add,$userid,$username){
	...
	$sql=$Elves->query("insert into {$dbtbpre}melvemembergroup(groupname,level,checked,favanum,daydown,msglen,msgnum,canreg,formid,regchecked,spacestyleid,dayaddinfo,infochecked,plchecked) values('$add[groupname]',$add[level],$add[checked],$add[favanum],$add[daydown],$msglen,$msgnum,$canreg,$formid,$regchecked,$spacestyleid,'$add[dayaddinfo]','$add[infochecked]','$add[plchecked]');");
	$groupid=$Elves->lastid();
	//更新缓存
	GetMemberLevel();
	if($sql)
	{
		...
		printerror("AddMemberGroupSuccess","member/AddMemberGroup.php?melve=AddMemberGroup");
	}
	else
	{printerror("DbError","history.go(-1)");}
}
...
function EditMemberGroup($add,$userid,$username){
	...
	$add[infochecked]=(int)$add[infochecked];
	$add[plchecked]=(int)$add[plchecked];
	$sql=$Elves->query("update {$dbtbpre}melvemembergroup set groupname='$add[groupname]',level=$add[level],checked=$add[checked],favanum=$add[favanum],daydown=$add[daydown],msglen=$msglen,msgnum=$msgnum,canreg=$canreg,formid=$formid,regchecked=$regchecked,spacestyleid=$spacestyleid,dayaddinfo='$add[dayaddinfo]',infochecked='$add[infochecked]',plchecked='$add[plchecked]' where groupid='$add[groupid]'");
	//更新缓存
	GetMemberLevel();
	if($sql)
	{
...
function DelMemberGroup($groupid,$userid,$username){
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"member");
	$r=$Elves->fetch1("select groupname from {$dbtbpre}melvemembergroup where groupid='$groupid'");
	$sql=$Elves->query("delete from {$dbtbpre}melvemembergroup where groupid='$groupid'");
	//更新缓存
	GetMemberLevel();
	if($sql)
	{
