...
define('InElvesCMSTfun',TRUE);
require_once(elve_PATH."core/class/userfun.php");
...
//返回sql语句
function sys_ReturnBqQuery($classid,$line,$melve=0,$do=0,$ewhere='',$eorder=''){
	global $Elves,$public_r,$class_r,$class_zr,$navclassid,$dbtbpre,$fun_r,$class_tr,$emod_r,$etable_r,$eyh_r;
	if($melve==24)//按sql查询
	{
	...
	if($melve==0||$melve==1||$melve==2||$melve==9||$melve==12||$melve==15)//栏目
	{
	...
	elseif($melve==6||$melve==7||$melve==8||$melve==11||$melve==14||$melve==17)//专题
	{
	...
	elseif($melve==9)//栏目评论排行
	{
		$query=' where ('.$where.')';
		$order='plnum';
		$yhvar='bqpl';
    }
	elseif($melve==12)//栏目头条
	{
		$query=' where ('.$where.') and firsttitle>0';
		$order='newstime';
		$yhvar='bqfirst';
    }
	elseif($melve==15)//栏目下载排行
	{
		$query=' where ('.$where.')';
		$order='totaldown';
		$yhvar='bqdown';
	...
	if(empty($tbname))
	{
		echo $fun_r['BqErrorCid']."=<b>".$classid."</b>".$fun_r['BqErrorNtb']."(".$fun_r['BqErrorDo']."=".$melve.")";
		return false;
	}
	//排序
	$addorder=empty($eorder)?$order.' desc':$eorder;
	$query='select '.ReturnSqlListF($mid).' from '.$dbtbpre.'elve_'.$tbname.$query.' order by '.ReturnSetTopSql('bq').$addorder.' limit '.$line;
	$sql=$Elves->query1($query);
	if(!$sql)
	{
...
function sys_ReturnBqTemp($tempid){
	...
	$r=$Elves->fetch1("select tempid,modid,temptext,showdate,listvar,subnews,rownum,docode from ".GetTemptb("melvebqtemp")." where tempid='$tempid'");
	if(empty($r[tempid]))
	{
...
//替换栏目名
function ReplaceelveinfoClassname($temp,$melve,$classid){
...
//带模板的标签
function sys_GetelveInfo($classid,$line,$strlen,$have_class=0,$melve=0,$tempid,$doing=0,$ewhere='',$eorder=''){
	global $Elves,$public_r;
	$sql=sys_ReturnBqQuery($classid,$line,$melve,$doing,$ewhere,$eorder);
	if(!$sql)
	{return "";}
	//取得模板
	...
	$docode=$tr[docode];
	//替换变量
	$listtemp=ReplaceelveinfoClassname($listtemp,$melve,$classid);
	if(empty($rownum))
	{$rownum=1;}
	//字段
...
//灵动标签：返回SQL内容函数
function sys_ReturnelveLoopBq($classid=0,$line=10,$melve=3,$doing=0,$ewhere='',$eorder=''){
	return sys_ReturnBqQuery($classid,$line,$melve,$doing,$ewhere,$eorder);
}

//灵动标签：返回特殊内容函数
function sys_ReturnelveLoopStext($r){
	global $class_r;
...
//返回相关链接操作类型
function sys_OtherLinkQuery($classid,$line,$melve,$doing){
	global $Elves,$public_r,$class_r,$class_zr,$navinfor,$dbtbpre,$eyh_r,$etable_r,$class_tr;
	if($melve==1)//按表
	{
		$tbname=$classid;
	}
	...
	$query="select * from {$dbtbpre}elve_".$tbname." where ".$yhadd.$and.$keys." order by newstime desc limit $line";
	$sql=$Elves->query1($query);
	if(!$sql)
	{
		echo"SQL Error: ".ReRepSqlTbpre($query);
...
//相关链接标签
function sys_GetOtherLinkInfo($tempid,$classid='',$line=0,$strlen=60,$have_class=0,$melve=0,$doing=0){
	global $Elves,$navinfor,$public_r;
	if(empty($navinfor['keyboard'])||(empty($melve)&&!$navinfor['keyid']))
	{
		return '';
	}
	$sql=sys_OtherLinkQuery($classid,$line,$melve,$doing);
	if(!$sql)
	{return "";}
	//取得模板
	$tr=sys_ReturnBqTemp($tempid);
...
//文字标签函数
function sys_GetClassNews($classid,$line,$strlen,$showdate=true,$melve=0,$have_class=0,$formatdate='(m-d)',$ewhere='',$eorder=''){
	global $Elves;
	$sql=sys_ReturnBqQuery($classid,$line,$melve,0,$ewhere,$eorder);
	if(!$sql)
	{return "";}
	$record=0;
	while($r=$Elves->fetch($sql))
...
//图文信息调用
function sys_GetClassNewsPic($classid,$line,$num,$width,$height,$showtitle=true,$strlen,$melve=0,$ewhere='',$eorder=''){
	global $Elves;
	$sql=sys_ReturnBqQuery($classid,$num,$melve,1,$ewhere,$eorder);
	if(!$sql)
	{return "";}
	//输出
	$i=0;
...
function sys_GetAd($adid){
	...
	$r=$Elves->fetch1("select * from {$dbtbpre}melvead where adid='$adid'");
	//到期
	...
	{
		$ad_url=$public_r[newsurl]."core/public/ClickAd?adid=".$adid;//广告链接
	}
	//----------------------文字广告
	if($r[t]==1)
	{
		...
		{
			$html="<script language=javascript src=".$public_r[newsurl]."d/js/acmsd/elve_dialog.js></script> 
...
<script language=javascript src=".$public_r[newsurl]."d/js/acmsd/elve_float_fullscreen.js></script>";
		}
		//上下浮动显示 - 右
		elseif($r[adtype]==5)
		{
...
<script language=javascript src=".$public_r[newsurl]."d/js/acmsd/elve_float_upanddown.js></script>";
		}
		//上下浮动显示 - 左
		elseif($r[adtype]==6)
		{
...
<script language=javascript src=".$public_r[newsurl]."d/js/acmsd/elve_float_upanddown_L.js></script>";
		}
		//全屏幕渐隐消失
		elseif($r[adtype]==7)
		{
...
<script language=javascript src=".$public_r[newsurl]."d/js/acmsd/elve_fullscreen.js></script>";
		}
		//可移动透明对话框
		elseif($r[adtype]==3)
		{
...
function sys_GetInfoVote($classid,$id){
	global $Elves,$public_r,$dbtbpre;
	$r=$Elves->fetch1("select * from {$dbtbpre}melveinfovote where id='$id' and classid='$classid' limit 1");
	if(empty($r[votetext]))
	{
		return '';
	}
...
//友情链接
function sys_GetSitelink($line,$num,$melve=0,$classid=0,$stats=0){
	global $Elves,$public_r,$dbtbpre;
	//图片
	if($melve==1)
	{$a=" and lpic<>''";}
	...
	$sql=$Elves->query("select * from {$dbtbpre}melvelink where checked=1".$a.$whereclass." order by myorder,lid limit ".$num);
	//输出
	$i=0;
	while($r=$Elves->fetch($sql))
	{
		...
		if(empty($stats))
		{
			$linkurl=$public_r[newsurl]."core/public/GotoSite/?lid=".$r[lid]."&url=".urlencode($r[lurl]);
		}
		else
		{
			$linkurl=$r[lurl];
...
//信息统计
function sys_TotalData($classid,$melve=0,$day=0,$totaltype=0){
	global $Elves,$class_r,$class_zr,$dbtbpre,$fun_r,$class_tr;
	if(empty($classid))
	{
		return "";
    }
	...
	if($melve==1)//统计标题分类
	{
		...
		$query="select ".$totalfield." from {$dbtbpre}elve_".$class_tr[$classid][tbname]." where ttid='$classid'".$and;
    }
	elseif($melve==2)//统计数据表
	{
		$query="select ".$totalfield." from {$dbtbpre}elve_".$classid.(empty($and)?'':' where '.substr($and,5));
    }
	else//统计栏目数据
	{
		...
		$query="select ".$totalfield." from {$dbtbpre}elve_".$class_r[$classid][tbname]." where ".$where.$and;
    }
...
//flash幻灯图片信息调用
function sys_FlashPixpic($classid,$line,$width,$height,$showtitle=true,$strlen,$melve=0,$sec=5,$ewhere='',$eorder=''){
	global $Elves,$public_r,$class_r,$class_zr;
	$sql=sys_ReturnBqQuery($classid,$line,$melve,1,$ewhere,$eorder);
	if(!$sql)
	{return "";}
	$i=0;
	while($r=$Elves->fetch($sql))
...
<!--
 ...
 var swfpath="<?=$public_r[newsurl]?>core/data/images/pixviewer.swf";
 var swfpatha="<?=$public_r[newsurl]?>core/data/images/pixviewer.swf";
...
//搜索关键字
function sys_ShowSearchKey($line,$num,$classid=0,$melve=0){
	global $Elves,$public_r,$dbtbpre;
	if($melve)
	{
		$order="searchid";
	...
	$sql=$Elves->query("select searchid,keyboard from {$dbtbpre}melvesearch where iskey=0".$add." order by ".$order." desc limit ".$num);
	$i=0;
	$returnkey="";
	while($r=$Elves->fetch($sql))
	{
		$i++;
		$keyurl=$public_r[newsurl]."core/search/result/?searchid=$r[searchid]";
		$br="";
		if($i%$line==0)
		{
			$br="<br>";
...
//带模板的标签显示-循环
function sys_GetelveInfoMore($classid,$line,$strlen,$have_class=0,$elve=0,$tr,$doing=0,$field,$cr,$dofirstinfo=0,$fsubtitle=0,$fsubnews=0,$fdoing=0,$ewhere='',$eorder=''){
	global $Elves,$public_r;
	//操作类型
	if($elve==0)//栏目最新
	{
...
//循环子栏目显示头条信息
function sys_ForSonclassDataFirstInfo($temptext,$cr,$elve=0,$subtitle=0,$subnews=0,$fdoing=0){
	global $Elves,$class_r,$public_r,$dbtbpre;
	if($elve==2||$elve==3||$elve==4)
	{
	...
	elseif($elve==2)//推荐信息
	{
		$r=$Elves->fetch1("select * from {$dbtbpre}elve_".$class_r[$cr[classid]][tbname]." where isgood>0 and (".$where.")".$add." order by newstime desc limit 1");
	}
	elseif($elve==3)//头条信息
	{
		$r=$Elves->fetch1("select * from {$dbtbpre}elve_".$class_r[$cr[classid]][tbname]." where firsttitle>0 and (".$where.")".$add." order by newstime desc limit 1");
	}
	elseif($elve==4)//最新信息
	{
		$r=$Elves->fetch1("select * from {$dbtbpre}elve_".$class_r[$cr[classid]][tbname]." where (".$where.")".$add." order by newstime desc limit 1");
	}
	else
	{
		return $temptext;
...
//循环子栏目数据
function sys_ForSonclassData($classid,$line,$strlen,$have_class=0,$melve=0,$tempid,$doing=0,$cline=0,$dofirstinfo=0,$fsubtitle=0,$fsubnews=0,$fdoing=0,$ewhere='',$eorder=''){
	global $Elves,$public_r,$class_r,$class_zr,$navclassid,$dbtbpre;
	//多栏目
	if(strstr($classid,","))
	{
	...
	$csql=$Elves->query("select classid".$addclassfield." from {$dbtbpre}melveclass where ".$where." and wburl='' order by myorder,classid".$limit);
	while($cr=$Elves->fetch($csql))
	{
		sys_GetelveInfoMore($cr[classid],$line,$strlen,$have_class,$melve,$tr,$doing,$ret_r,$cr,$dofirstinfo,$fsubtitle,$fsubnews,$fdoing,$ewhere,$eorder);
	}
}

//带模板的栏目导航标签
function sys_ShowClassByTemp($classid,$tempid,$show=0,$cline=0){
	global $navclassid,$Elves,$class_r,$public_r,$dbtbpre;
	//当前栏目
	if($classid=="selfinfo")
	{
		if(empty($navclassid))
	...
	$no=1;
	$changerow=1;
	$sql=$Elves->query("select classid,classname,islast,sonclass,tbname,intro,classimg,infos from {$dbtbpre}melveclass where bclassid='$classid' and showclass=0 order by myorder,classid".$limit);
	while($r=$Elves->fetch($sql))
	{
		//显示类别数据数
		if($show)
...
function sys_ForShowSonClass($classid,$tempid,$show=0,$cline=0){
	...
	$no=1;
	$sql=$Elves->query("select classid,classname,islast,sonclass,tbname,intro,classimg,infos from {$dbtbpre}melveclass where ".$where." and showclass=0 order by myorder,classid".$limit);
	while($r=$Elves->fetch($sql))
	{
		//显示栏目数据数
		if($show)
...
function sys_GetShowClassMore($bclassid,$bcr,$tr,$bno,$bnum,$show=0){
	...
	$bclassimg=$bcr[classimg]?$bcr[classimg]:$public_r[newsurl]."core/data/images/notimg.gif";//栏目图片
	...
	$no=1;
	$changerow=1;
	$sql=$Elves->query("select classid,classname,islast,sonclass,tbname,intro,classimg,infos from {$dbtbpre}melveclass where bclassid='$bclassid' and showclass=0 order by myorder,classid");
	while($r=$Elves->fetch($sql))
	{
		//显示栏目数据数
		if($show)
...
//替换栏目导航标签
function ReplaceShowClassVars($no,$listtemp,$r,$num,$elve=0,$subnews=0){
	...
	if(empty($r[classimg]))
	{
		$r[classimg]=$public_r[newsurl]."core/data/images/notimg.gif";
	}
...
function sys_ShowLyInfo($line,$tempid,$bid=0){
	...
	$no=1;
	$changerow=1;
	$sql=$Elves->query("select lyid,name,email,lytime,lytext,retext from {$dbtbpre}melvegbook where checked=0".$a." order by lyid desc limit ".$line);
	while($r=$Elves->fetch($sql))
	{
		//替换列表变量
...
function sys_ShowZtData($tempid,$zcid=0,$cline=0,$classid=0){
	...
	$no=1;
	$changerow=1;
	$sql=$Elves->query("select ztid,ztname,intro,ztimg from {$dbtbpre}melvezt where showzt=0".$a." order by myorder,ztid desc".$limit);
	while($r=$Elves->fetch($sql))
	{
		//替换列表变量
...
function sys_PhotoMorepage($tempid,$spicwidth=0,$spicheight=0){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$sdh.=$nbsp."<a href='#elve' onclick='GotoPhPage(".$j.");' title='".$picname."'><img src='".$smallpic."' alt='".$picname."' border=0".$gs."></a>";
		if($i==0)
		{
			$firstpic=$showpic;
		}
		...
		$listpage.=$nbsp."<a href='#elve' onclick='GotoPhPage(".$j.");' title='".$picname."'>".$j."</a>";
		$nbsp="&nbsp;";
	}
...
//评论调用
function sys_ShowPlInfo($line,$tempid,$classid=0,$id=0,$isgood=0,$melve=0){
	global $Elves,$dbtbpre,$class_r,$public_r;
	$a="";
	if($isgood)
	{
		$a.=" and isgood='$isgood'";
	...
	$no=1;
	$changerow=1;
	$sql=$Elves->query("select plid,userid,username,saytime,id,classid,zcnum,fdnum,saytext from {$dbtbpre}melvepl_".$public_r['pldeftb']." where checked=0".$a." order by ".$order." limit ".$line);
	while($r=$Elves->fetch($sql))
	{
		//替换列表变量
...
function ReplaceShowPlVars($no,$listtemp,$r,$formatdate,$subnews=0){
	global $public_r,$Elves,$dbtbpre,$class_r;
	//标题
	$infor=$Elves->fetch1("select isurl,titleurl,classid,id,title from {$dbtbpre}elve_".$class_r[$r[classid]][tbname]." where id='$r[id]' limit 1");
	...
	if($r['userid'])
	{
		$r['username']="<a href='".$public_r[newsurl]."core/space/?userid=$r[userid]' target='_blank'>$r[username]</a>";
	}
	if(empty($r['username']))
	{
		$r['username']='匿名';
...
function sys_ShowMemberInfo($userid=0,$fields=''){
	global $Elves,$dbtbpre,$public_r,$navinfor,$level_r;
	...
	$r=$Elves->fetch1("select ".$fields." from ".eReturnMemberTable()." u LEFT JOIN {$dbtbpre}melvememberadd ui ON u.".egetmf('userid')."=ui.userid where u.".egetmf('userid')."='$uid' limit 1");
...
//调用会员列表
function sys_ListMemberInfo($line=10,$elve=0,$groupid=0,$userids=0,$fields=''){
	global $Elves,$dbtbpre,$public_r,$navinfor,$level_r;
	if(!defined('InElvesCMSUser'))
	{
		include_once elve_PATH.'core/member/class/user.php';
	...
	$sql=$Elves->query("select ".$fields." from ".eReturnMemberTable()." u LEFT JOIN {$dbtbpre}melvememberadd ui ON u.".egetmf('userid')."=ui.userid where u.".egetmf('checked')."=1".$where." order by ".$order." limit ".$line);
	return $sql;
}

//显示TAGS
function sys_eShowTags($cid,$num=0,$line=0,$order='',$isgood='',$isgoodshow='',$showjg='',$shownum=0,$cs='',$vartype=''){
	global $Elves,$dbtbpre,$public_r,$navinfor;
	$str='';
	if(empty($showjg))
	{
		$showjg=' &nbsp; ';
	...
	if($cid=='selfinfo')
	{
		...
		for($i=0;$i<$count;$i++)
		{
			...
			{
				$tagsurl=$public_r[newsurl].'core/tags/?tagname='.urlencode($r[$i]).$cs;
			}
	...
	else
	{
		...
		$jg='';
		$snum='';
		$sql=$Elves->query("select tagid,tagname,num,isgood from {$dbtbpre}melvetags".$where." order by".$order.$limit);
		while($r=$Elves->fetch($sql))
		{
			if($shownum)
			{
			...
			{
				$tagsurl=$public_r[newsurl].'core/tags/?'.($vartype=='tagid'?'tagid='.$r[tagid]:'tagname='.urlencode($r[tagname])).$cs;
			}
...
//索引灵动标签：返回SQL内容函数
function sys_ReturnelveIndexLoopBq($id=0,$line=10,$melve=3,$classid='',$mid='',$ewhere=''){
	global $navclassid;
	if($melve==1)//专题最新
	{
		$type='zt';
	...
	elseif($melve==4)//专题子类最新
	{
		$type='ztc';
		$eorder='newstime desc';
		$selectf=',ztid,cid,isgood';
	}
	elseif($melve==5)//专题子类最早
	{
		$type='ztc';
		$eorder='newstime asc';
		$selectf=',ztid,cid,isgood';
	}
	elseif($melve==6)//专题子类推荐
	{
		$type='ztc';
		$eorder='newstime asc';
		$selectf=',ztid,cid,isgood';
	...
	elseif($melve==7)//TAGS最新
	{
		$type='sp';
		$eorder='newstime desc';
		$selectf='';
	}
	elseif($melve==8)//TAGS最早
	{
		$type='sp';
		$eorder='newstime asc';
		$selectf='';
	...
	elseif($melve==10)//碎片最早
	{
		$type='tags';
		$eorder='newstime asc';
		$selectf='';
	}
	elseif($melve==11)//SQL调用
	{
		$type='sql';
		$eorder='newstime asc';
		$selectf='';
...
function sys_ReturnTogQuery($type,$id,$line,$classid='',$mid='',$ewhere='',$eorder='',$selectf=''){
	global $Elves,$public_r,$class_r,$class_zr,$navclassid,$dbtbpre,$class_tr,$emod_r;
	if($type=='tags')//TAGS
	{
		$idf='tagid';
		$orderf='newstime desc';
		$table=$dbtbpre.'melvetagsdata';
	}
	elseif($type=='zt')//专题
	{
		$idf='ztid';
		$orderf='newstime desc';
		$table=$dbtbpre.'melveztinfo';
	}
...
function sys_eShowSpInfo($spvar,$line=10,$strlen=0){
	...
	$spr=$Elves->fetch1("select spid,spname,sppic,spsay,tempid,sptype from {$dbtbpre}melvesp where varname='$spvar' limit 1");
...
function sys_eShowSp1($spid,$spr,$line,$strlen){
	...
	$no=1;
	$changerow=1;
	$sql=$Elves->query("select sid,title,titlepic,bigpic,titleurl,smalltext,titlefont,newstime,titlepre,titlenext from {$dbtbpre}melvesp_1 where spid='$spid' order by newstime desc limit ".$line);
	while($r=$Elves->fetch($sql))
	{
		$r[oldtitle]=$r[title];
		//替换列表变量
...
function sys_eShowSp3($spid){
	...
	$r=$Elves->fetch1("select sptext from {$dbtbpre}melvesp_3 where spid='$spid' limit 1");
	echo $r['sptext'];
}

//调用生成缩图
function sys_ResizeImg($file,$width,$height,$docut=0,$target_filename='',$target_path='core/data/tmp/titlepic/'){
	...
	$filename=elve_PATH.substr($file,1);
	...
	if(!defined('InElvesCMSGd'))
	{
		include_once elve_PATH.'core/class/gd.php';
	}
	$filer=ResizeImage($filename,$newname,$width,$height,$docut,$newpath);
