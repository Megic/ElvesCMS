...
function ChangeInfoOtherLink($start,$classid,$from,$retype,$startday,$endday,$startid,$endid,$tbname,$userid,$username){
	...
	$b=0;
	$sql=$Elves->query("select id,classid,checked from {$dbtbpre}elve_".$tbname."_index where id>$start".$add1." order by id limit ".$public_r[infolinknum]);
	while($r=$Elves->fetch($sql))
	{
		...
		$infopr=$Elves->fetch1("select diyotherlink from {$dbtbpre}melveinfovote where pubid='$pubid' limit 1");
		...
		$infor=$Elves->fetch1("select stb,keyboard from ".$infotb." where id='$r[id]' limit 1");
		//返回表信息
		...
		$usql=$Elves->query("update ".$infodatatb." set keyid='$newkeyid' where id='$r[id]' limit 1");
	}
	if(empty($b))
	{
	    insert_dolog("");//操作日志
	...
	echo $fun_r[OneChangeInfoLinkSuccess]."(ID:<font color=red><b>".$new_start."</b></font>)<script>self.location.href='elvecom.php?melve=ChangeInfoOtherLink&tbname=$tbname&classid=$classid&start=$new_start&from=$from&retype=$retype&startday=$startday&endday=$endday&startid=$startid&endid=$endid';</script>";
	exit();
}
...
function AddUserpage($add,$userid,$username){
	...
	$gid=(int)$add['gid'];
	$sql=$Elves->query("insert into {$dbtbpre}melvepage(title,path,pagetext,classid,pagetitle,pagekeywords,pagedescription,tempid) values('$title','$path','".eaddslashes2($pagetext)."','$classid','".eaddslashes($pagetitle)."','".eaddslashes($pagekeywords)."','".eaddslashes($pagedescription)."','$tempid');");
	$id=$Elves->lastid();
	...
	if($sql)
	{
		...
		printerror("AddUserpageSuccess","template/AddPage.php?melve=AddUserpage&gid=$gid&ChangePagemod=$add[pagemod]");
	}
	else
	{
...
function EditUserpage($add,$userid,$username){
	...
	$tempid=(int)$add['tempid'];
	$gid=(int)$add['gid'];
	$sql=$Elves->query("update {$dbtbpre}melvepage set title='$title',path='$path',pagetext='".eaddslashes2($pagetext)."',classid='$classid',pagetitle='".eaddslashes($pagetitle)."',pagekeywords='".eaddslashes($pagekeywords)."',pagedescription='".eaddslashes($pagedescription)."',tempid='$tempid' where id='$id'");
	ReUserpage($id,$pagetext,$path,$title,$pagetitle,$pagekeywords,$pagedescription,$tempid);
	if($sql)
	{
		//操作日志
...
function DelUserpage($id,$cid,$userid,$username){
	...
	$gid=(int)$_GET['gid'];
	$r=$Elves->fetch1("select title,id,path from {$dbtbpre}melvepage where id='$id'");
	if(empty($r['id']))
	{
		printerror("NotDelUserpageid","history.go(-1)");
	}
	$sql=$Elves->query("delete from {$dbtbpre}melvepage where id='$id'");
	DelFiletext($r['path']);
	if($sql)
	{
		//操作日志
...
function DoReUserpage($add,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$ur=$Elves->fetch1("select id,path,pagetext,title,pagetitle,pagekeywords,pagedescription,tempid from {$dbtbpre}melvepage where id='".$id[$i]."'");
...
function DoRepNewstext($start,$oldword,$newword,$field,$classid,$tid,$tbname,$over,$dozz,$dotxt,$userid,$username){
	...
	$fr=$Elves->fetch1("select tid,savetxt,tbdataf from {$dbtbpre}melvef where tbname='$tbname' and f='$field' limit 1");
	//覆盖方式
	if($dotype==1)
	{
		...
		if($fr['tbdataf'])//副表
		{
			...
			if($tbr['datatbs'])
			{
				...
				for($i=1;$i<$count-1;$i++)
				{
					$Elves->query("update {$dbtbpre}elve_".$tbname."_data_".$dtbr[$i]." set ".$field."='$repnewword'".$add2);
				}
			}
			//未审核
			$Elves->query("update {$dbtbpre}elve_".$tbname."_check_data set ".$field."='$repnewword'".$add2);
		}
		else//主表
		{
			$Elves->query("update {$dbtbpre}elve_".$tbname." set ".$field."='$repnewword'".$add2);
			//未审核
			$Elves->query("update {$dbtbpre}elve_".$tbname."_check set ".$field."='$repnewword'".$add2);
		}
		//替换完毕
	...
	elseif($fr['savetxt'])
	{
		...
		$start=(int)$start;
		$b=0;
		$sql=$Elves->query("select id,checked from {$dbtbpre}elve_".$tbname."_index where id>$start".$add1." order by id limit ".$public_r[dorepnum]);
		while($r=$Elves->fetch($sql))
		{
			$b=1;
			$newstart=$r[id];
			...
			$infotb=ReturnInfoMainTbname($tbname,$r['checked']);
			//主表
			$infor=$Elves->fetch1("select isurl".$selectf." from ".$infotb." where id='$r[id]' limit 1");
			if($infor['isurl'])
			{
				continue;
			}
			...
			if($fr['tbdataf'])
			{
				//返回表信息
				$infodatatb=ReturnInfoDataTbname($tbname,$r['checked'],$infor['stb']);
				$finfor=$Elves->fetch1("select ".$field." from ".$infodatatb." where id='$r[id]' limit 1");
	...
	elseif($dozz==1)
	{
		...
		while($r=$Elves->fetch($sql))
		{
			...
			if($fr['tbdataf'])//副表
			{
				...
				$Elves->query("update ".$infodatatb." set ".$field."='".addslashes($newvalue)."' where id='$r[id]'");
			}
			else//主表
			{
				$newvalue=DoRepNewstextZz($oldword,$newword,stripSlashes($infor[$field]));//正则替换
				$Elves->query("update ".$infotb." set ".$field."='".addslashes($newvalue)."' where id='$r[id]'");
			}
		}
		//替换完毕
		if(empty($b))
	...
	else
	{
		...
		if($fr['tbdataf'])//副表
		{
			...
			if($tbr['datatbs'])
			{
				...
				for($i=1;$i<$count-1;$i++)
				{
					$Elves->query("update {$dbtbpre}elve_".$tbname."_data_".$dtbr[$i]." set ".$field."=REPLACE(".$field.",'$repoldword','$repnewword')".$add2);
				}
			}
			//未审核
			$Elves->query("update {$dbtbpre}elve_".$tbname."_check_data set ".$field."=REPLACE(".$field.",'$repoldword','$repnewword')".$add2);
		}
		else//主表
		{
			$Elves->query("update {$dbtbpre}elve_".$tbname." set ".$field."=REPLACE(".$field.",'$repoldword','$repnewword')".$add2);
			//未审核
			$Elves->query("update {$dbtbpre}elve_".$tbname."_check set ".$field."=REPLACE(".$field.",'$repoldword','$repnewword')".$add2);
		}
		//替换完毕
		insert_dolog("tbname=".$tbname."&field=".$field."<br>oldword=".$oldword."<br>newword=".$newword);//操作日志
		printerror("DoRepNewstextSuccess","db/RepNewstext.php?tid=$tid");
...
function DoRepNewstextZz($oldword,$newword,$text){
	$zztext=RepInfoZZ($oldword,"Elves-cms-wm.chief-mgxe",0);
...
function EchoRepFieldForm($tid,$tbname,$over,$dozz,$dotxt,$newstart,$fieldform,$classid,$oldword,$newword){
	...
	$dotype=(int)$_POST['dotype'];
	?>
	<?=$fun_r['RepOnmelvetextSuccess']?>(ID:<font color=red><b><?=$newstart?></b></font>)
	<form name="RepFieldForm" method="post" action="elvecom.php">
		<input type=hidden name="melve" value="DoRepNewstext">
...
//返回类别管理操作信息
function ReturnDoclassVar($elve){
	global $dbtbpre;
	//标签模板分类
	if($elve=="bqtemp")
	{
		$r['tbname']=$dbtbpre."melvebqtempclass";
		$r['mess']="Bqtemp";
		$r['thelevel']="template";
	...
	elseif($elve=="page")
	{
		$r['tbname']=$dbtbpre."melvepageclass";
		$r['mess']="Page";
		$r['thelevel']="userpage";
	...
	elseif($elve=="tempvar")
	{
		$r['tbname']=$dbtbpre."melvetempvarclass";
		$r['mess']="Tempvar";
		$r['thelevel']="tempvar";
		$r['returnpage']="template/TempvarClass.php";
	}
	//列表模板分类
	elseif($elve=="listtemp")
	{
		$r['tbname']=$dbtbpre."melvelisttempclass";
		$r['mess']="Listtemp";
		$r['thelevel']="template";
		$r['returnpage']="template/ListtempClass.php";
	}
	//内容模板分类
	elseif($elve=="newstemp")
	{
		$r['tbname']=$dbtbpre."melvenewstempclass";
		$r['mess']="Newstemp";
		$r['thelevel']="template";
		$r['returnpage']="template/NewstempClass.php";
	}
	//搜索模板分类
	elseif($elve=="searchtemp")
	{
		$r['tbname']=$dbtbpre."melvesearchtempclass";
		$r['mess']="Searchtemp";
		$r['thelevel']="template";
	...
	elseif($elve=="jstemp")
	{
		$r['tbname']=$dbtbpre."melvejstempclass";
		$r['mess']="Jstemp";
		$r['thelevel']="template";
	...
	elseif($elve=="link")
	{
		$r['tbname']=$dbtbpre."melvelinkclass";
		$r['mess']="Linkclass";
		$r['thelevel']="link";
		$r['returnpage']="tool/LinkClass.php";
	}
	//单页模板
	elseif($elve=="classtemp")
	{
		$r['tbname']=$dbtbpre."melveclasstempclass";
		$r['mess']="Classtemp";
		$r['thelevel']="template";
	...
	elseif($elve=="error")
	{
		$r['tbname']=$dbtbpre."melveerrorclass";
		$r['mess']="Error";
	...
	elseif($elve=="tags")
	{
		$r['tbname']=$dbtbpre."melvetagsclass";
		$r['mess']="Tags";
		$r['thelevel']="tags";
		$r['returnpage']="tags/TagsClass.php";
	}
	//用户自定义列表
	elseif($elve=="userlist")
	{
		$r['tbname']=$dbtbpre."melveuserlistclass";
		$r['mess']="Userlist";
		$r['thelevel']="userlist";
	...
	elseif($elve=="userjs")
	{
		$r['tbname']=$dbtbpre."melveuserjsclass";
		$r['mess']="Userjs";
		$r['thelevel']="userjs";
...
function AddThisClass($add,$userid,$username){
	...
	$sql=$Elves->query("insert into ".$thisr['tbname']."(classname) values('$add[classname]');");
	if($sql)
	{
		$lastid=$Elves->lastid();
		//操作日志
...
function EditThisClass($add,$userid,$username){
	...
	//验证权限
    CheckLevel($userid,$username,$classid,$thisr['thelevel']);
	$sql=$Elves->query("update ".$thisr['tbname']." set classname='$add[classname]' where classid='$classid'");
	if($sql)
	{
		//操作日志
...
function DelThisClass($classid,$doing,$userid,$username){
	...
	//验证权限
    CheckLevel($userid,$username,$classid,$thisr['thelevel']);
	$r=$Elves->fetch1("select classname from ".$thisr['tbname']." where classid=$classid");
	$sql=$Elves->query("delete from ".$thisr['tbname']." where classid=$classid");
	if($sql)
	{
		//操作日志
...
function RepDownLevel($add,$userid,$username){
	...
	$fr=$Elves->fetch1("select tid,savetxt,tbdataf from {$dbtbpre}melvef where tbname='$tbname' and f='$checkf' limit 1");
	if($fr['tbdataf'])//副表
	{
		$sfield=$field;
		$field='';
	...
	$update="";
	$b=0;
	$sql=$Elves->query("select id,stb".$field." from {$dbtbpre}elve_".$tbname." where id>$start".$wheresql." order by id limit ".$public_r['dorepdlevelnum']);
	while($r=$Elves->fetch($sql))
	{
		$b=1;
		$newstart=$r[id];
		...
		if($fr['tbdataf'])
		{
			$finfor=$Elves->fetch1("select id".$sfield." from {$dbtbpre}elve_".$tbname."_data_".$r[stb]." where id='$r[id]'");
		...
		if($fr['tbdataf'])
		{
			$Elves->query("update {$dbtbpre}elve_".$tbname."_data_".$r[stb]." set ".$update." where id='$r[id]'");
		}
		else
		{
			$Elves->query("update {$dbtbpre}elve_".$tbname." set ".$update." where id='$r[id]'");
		}
	}
	//替换完毕
	if(empty($b))
	{
		//未审核
		$sql=$Elves->query("select id,stb".$field." from {$dbtbpre}elve_".$tbname."_check".($wheresql?' where '.substr($wheresql,5):''));
		while($r=$Elves->fetch($sql))
		{
			//副表
			if($fr['tbdataf'])
			{
				$finfor=$Elves->fetch1("select id".$sfield." from {$dbtbpre}elve_".$tbname."_check_data where id='$r[id]' limit 1");
				$r['downpath']=$finfor['downpath'];
				$r['onlinepath']=$finfor['onlinepath'];
			}
			$update='';
			...
			if($fr['tbdataf'])
			{
				$Elves->query("update {$dbtbpre}elve_".$tbname."_check_data set ".$update." where id='$r[id]'");
			}
			else
			{
				$Elves->query("update {$dbtbpre}elve_".$tbname."_check set ".$update." where id='$r[id]'");
			}
		}
		//操作日志
...
function EchoRepDownLevelForm($add,$newstart){
	...
	<form name="RepDownLevelForm" method="post" action="elvecom.php">
		<input type=hidden name="melve" value="RepDownLevel">
		<input type=hidden name="start" value="<?=$newstart?>">
...
function ClearTmpFileData($add,$userid,$username){
	...
	CheckLevel($userid,$username,$classid,"changedata");
	//临时文件目录
	$tmppath=elve_PATH.'core/data/tmp';
	...
	//临时模板导出目录
	$temppath=elve_PATH.'core/data/tmp/temp';
	...
	//临时模型导出目录
	$modpath=elve_PATH.'core/data/tmp/mod';
	...
	//临时采集导出目录
	$cjpath=elve_PATH.'core/data/tmp/cj';
	$hand=@opendir($cjpath);
	while($file=@readdir($hand))
	{
	...
	//采集临时表
	$Elves->query("TRUNCATE `{$dbtbpre}melvelinktmp`;");
	//远程发布临时表
	$Elves->query("TRUNCATE `{$dbtbpre}melvepostdata`;");
...
function ClearBreakInfo($add,$userid,$username){
	...
	$tbr=$Elves->fetch1("select datatbs,tbname from {$dbtbpre}melvetable where tid='$tid'");
	if(!$tbr['tbname'])
	{
		printerror('EmptyDocTb','');
	}
	...
	if($tbr['datatbs'])
	{
		...
		for($i=1;$i<$count-1;$i++)
		{
			$dodatatbs.=$andunion."select id from {$dbtbpre}elve_".$tbr['tbname']."_data_".$dtbr[$i];
			$andunion=' union ';
		}
		if($dodatatbs)
		{
			$Elves->query("delete from {$dbtbpre}elve_".$tbr['tbname']." where id not in (".$dodatatbs.")");
			$affnum=$Elves->affectnum();
		}
	}
