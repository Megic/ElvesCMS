...
function AddInfoVote($classid,$id,$add){
	...
	$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}melveinfovote where pubid='$pubid' limit 1");
	...
	if($num)	//修改
	{
		...
		$sql=$Elves->query("update {$dbtbpre}melveinfovote set title='$add[vote_title]',votenum='$t_votenum',votetext='$votetext',voteclass='$voteclass',doip='$doip',dotime='$add[vote_dotime]',tempid='$tempid',width='$width',height='$height',diyotherlink='$diyotherlink',infouptime='$infouptime',infodowntime='$infodowntime' where pubid='$pubid' limit 1");
	}
	else	//增加
	{
...
function UpdateInfoCopyids($classid,$id,$copyids){
	...
	if($num)	//修改
	{
		$Elves->query("update {$dbtbpre}melveinfovote set copyids='$copyids' where pubid='$pubid' limit 1");
	}
	else	//增加
	{
		$Elves->query("insert into {$dbtbpre}melveinfovote(pubid,id,classid,copyids) values('$pubid','$id','$classid','$copyids');");
	}
}
...
function ReturnCheckRetitle($add){
	...
	//已审核
	$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}elve_".$class_r[$classid][tbname]." where title='".addslashes($title)."'".$where." limit 1");
	//未审核
	if(empty($num))
	{
		$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}elve_".$class_r[$classid][tbname]."_check where title='".addslashes($title)."'".$where." limit 1");
	}
	return $num;
}
...
function AddNews($add,$userid,$username){
	global $Elves,$class_r,$class_zr,$bclassid,$public_r,$dbtbpre,$emod_r;
	...
	$ccr=$Elves->fetch1("select classid,modid,listdt,haddlist,sametitle,addreinfo,wburl,repreinfo from {$dbtbpre}melveclass where classid='$add[classid]' and islast=1 limit 1");
	...
	//附加链接参数
	$addelvecheck=empty($add['checked'])?'&elvecheck=1':'';
	//索引表
	$sql=$Elves->query("insert into {$dbtbpre}elve_".$class_r[$add[classid]][tbname]."_index(classid,checked,newstime,truetime,lastdotime,havehtml) values('$add[classid]','$add[checked]','$newstime','$truetime','$lastdotime','$havehtml');");
	$id=$Elves->lastid();
	$pubid=ReturnInfoPubid($add['classid'],$id);
	$infotbr=ReturnInfoTbname($class_r[$add[classid]][tbname],$add['checked'],$ret_r['tb']);
	//主表
	$infosql=$Elves->query("insert into ".$infotbr['tbname']."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard".$ret_r['fields'].") values('$id','$add[classid]','$add[ttid]','$add[onclick]',0,'$add[totaldown]','$newspath','$filename','$userid','".addslashes($username)."','$add[firsttitle]','$add[isgood]','$add[ispic]','$add[istop]','$isqf',0,'$add[isurl]','$truetime','$lastdotime','$havehtml','$add[groupid]','$add[userfen]','".addslashes($add[my_titlefont])."','".addslashes($add[titleurl])."','$ret_r[tb]','$public_r[filedeftb]','$public_r[pldeftb]','".addslashes($add[keyboard])."'".$ret_r['values'].");");
	//副表
	$finfosql=$Elves->query("insert into ".$infotbr['datatbname']."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags".$ret_r['datafields'].") values('$id','$add[classid]','$keyid','$add[dokey]','$add[newstempid]','$add[closepl]',0,'".addslashes($add[infotags])."'".$ret_r['datavalues'].");");
	//更新栏目信息数
	...
	$usql=$Elves->query("update ".$infotbr['tbname']." set filename='$filename'".$updateinfourl.$addtitlepic." where id='$id'");
	//替换图片下一页
	...
	{
		$prer=$Elves->fetch1("select * from {$dbtbpre}elve_".$class_r[$add[classid]][tbname]." where id<$id and classid='$add[classid]' order by id desc limit 1");
	...
	{
		//返回地址
		if($add['elvefrom']&&(stristr($add['elvefrom'],'ListNews.php')||stristr($add['elvefrom'],'ListAllInfo.php')))
		{
			$elvefrom=$add['elvefrom'];
		}
		else
		{
			$elvefrom=$add['elvenfrom']==1?"ListNews.php?bclassid=$add[bclassid]&classid=$add[classid]":"ListAllInfo.php?tbname=".$class_r[$add[classid]][tbname];
		}
		$GLOBALS['elveadderrorurl']=$elvefrom.$addelvecheck;
		insert_dolog("classid=$add[classid]<br>id=".$id."<br>title=".$add[title],$pubid);//操作日志
		printerror("AddNewsSuccess","AddNews.php?melve=AddNews&elvenfrom=$add[elvenfrom]&bclassid=$add[bclassid]&classid=$add[classid]".$addelvecheck);
	}
	else
	{
		printerror("DbError","");
...
function EditNews($add,$userid,$username){
	...
	//索引表
	$index_checkr=$Elves->fetch1("select id,classid,checked from {$dbtbpre}elve_".$class_r[$add[classid]][tbname]."_index where id='$add[id]' limit 1");
	...
	$checkr=$Elves->fetch1("select id,classid,userid,username,ismember,stb,newspath,filename,isqf,fstb,isgood,firsttitle,istop from ".$infotb." where id='$add[id]' limit 1");
	...
	$pubcheckr=$Elves->fetch1("select copyids from {$dbtbpre}melveinfovote where pubid='$pubid' limit 1");
	...
	{
		$qfr=$Elves->fetch1("select checktno from {$dbtbpre}melvewfinfo where id='$add[id]' and classid='$add[classid]' limit 1");
	...
	//附加链接参数
	$addelvecheck=empty($newchecked)?'&elvecheck=1':'';
	//信息地址
	...
	//索引表
	$indexsql=$Elves->query("update {$dbtbpre}elve_".$class_r[$add[classid]][tbname]."_index set newstime='$newstime',lastdotime='$lastdotime'".$aqf." where id='$add[id]' limit 1");
	//主表
	$sql=$Elves->query("update ".$infotbr['tbname']." set classid='$add[classid]',ttid='$add[ttid]',onclick='$add[onclick]',totaldown='$add[totaldown]',firsttitle=$add[firsttitle],isgood=$add[isgood],ispic='$add[ispic]',istop=$add[istop],isurl='$add[isurl]',lastdotime=$lastdotime,groupid=$add[groupid],userfen=$add[userfen],titlefont='".addslashes($add[my_titlefont])."',titleurl='".addslashes($infourl)."',keyboard='".addslashes($add[keyboard])."'".$updatefile.$ret_r[values]." where id='$add[id]' limit 1");
	//副表
	$stb=$checkr['stb'];
	$fsql=$Elves->query("update ".$infotbr['datatbname']." set classid='$add[classid]',keyid='$keyid',dokey=$add[dokey],newstempid=$add[newstempid],closepl=$add[closepl],infotags='".addslashes($add[infotags])."'".$ret_r[datavalues]." where id='$add[id]' limit 1");
	//取第一张图作为标题图片
	if($add['getfirsttitlepic']&&empty($add['titlepic']))
	{
		...
		if($firsttitlepic)
		{
			$usql=$Elves->query("update ".$infotbr['tbname']." set titlepic='".addslashes($firsttitlepic)."',ispic=1 where id='$add[id]'");
		}
	}
	//更新附件
	...
	{
		$prer=$Elves->fetch1("select * from {$dbtbpre}elve_".$class_r[$add[classid]][tbname]." where id<$add[id] and classid='$add[classid]' order by id desc limit 1");
	...
	if($sql)
	{
		...
		{
			$elvefrom="ListNews.php?bclassid=$add[bclassid]&classid=$add[classid]";
		}
		insert_dolog("classid=$add[classid]<br>id=".$add[id]."<br>title=".$add[title],$pubid);//操作日志
		printerror("EditNewsSuccess",$elvefrom.$addelvecheck);
	}
	else
	{
...
function EditInfoSimple($add,$userid,$username){
	...
	//主表
	$infotb=ReturnInfoMainTbname($class_r[$add[classid]][tbname],$index_checkr['checked']);
	$checkr=$Elves->fetch1("select id,classid,userid,username,ismember,stb,newspath,filename,isqf,fstb,isgood,firsttitle,istop,groupid from ".$infotb." where id='$add[id]' limit 1");
	if($doselfinfo['doselfinfo']&&($checkr['userid']<>$userid||$checkr['ismember']))//只能编辑自己的信息
	{
	...
	//副表
	$fsql=$Elves->query("update ".$infotbr['datatbname']." set classid='$add[classid]',closepl='$add[closepl]'".$ret_r[datavalues]." where id='$add[id]' limit 1");
	//更新附件
	UpdateTheFileEdit($add['classid'],$add['id'],$checkr['fstb']);
	//是否改变审核状态
	if($index_checkr['checked']!=$newchecked)
	...
	if($sql)
	{
		...
		$elvefrom=$elvefrom.$addelvecheck;
		insert_dolog("classid=$add[classid]<br>id=".$add[id]."<br>title=".$add[title],$pubid);//操作日志
...
function DelNews($id,$classid,$userid,$username){
	global $Elves,$class_r,$class_zr,$bclassid,$public_r,$dbtbpre,$emod_r,$adddatar;
	$id=(int)$id;
	$classid=(int)$classid;
	if(!$id||!$classid)
	{
	...
	$ccr=$Elves->fetch1("select classid,modid,listdt,haddlist,repreinfo from {$dbtbpre}melveclass where classid='$classid' limit 1");
	if(!$ccr['classid'])
	{
		printerror("ErrorUrl","history.go(-1)");
	}
	//索引表
	$index_r=$Elves->fetch1("select classid,checked from {$dbtbpre}elve_".$class_r[$classid][tbname]."_index  where id='$id' limit 1");
	...
	$r=$Elves->fetch1("select * from ".$infotb." where id='$id' limit 1");
	...
	$pubid=ReturnInfoPubid($classid,$id);
	//附加链接参数
	$addelvecheck=empty($index_r['checked'])?'&elvecheck=1':'';
	...
	$sql=$Elves->query("delete from {$dbtbpre}elve_".$class_r[$classid][tbname]."_index where id='$id'");
	$sql=$Elves->query("delete from ".$infotbr['tbname']." where id='$id'");
	...
	if($sql)
	{
		...
		if($adddatar['causetext'])
		{
			...
			if($adddatar['elvefrom']&&(stristr($adddatar['elvefrom'],'ListNews.php')||stristr($adddatar['elvefrom'],'ListAllInfo.php')))
			{
				$returl=$adddatar['elvefrom'];
			}
			else
			{
				$returl="ListNews.php?bclassid=$adddatar[bclassid]&classid=$adddatar[classid]".$addelvecheck;
			}
		}
		else
		{
			if($_POST['melve']=='DoInfoAndSendNotice')
			{
				$returl="ListNews.php?bclassid=$adddatar[bclassid]&classid=$adddatar[classid]".$addelvecheck;
			}
		}
...
//批量删除信息
function DelNews_all($id,$classid,$userid,$username,$elve=0){
	global $Elves,$class_r,$class_zr,$public_r,$dbtbpre,$emod_r;
	$classid=(int)$classid;
	$count=count($id);
	if(!$count)
	{
	...
	{
		$id[$i]=intval($id[$i]);
		$r=$Elves->fetch1("select * from {$dbtbpre}elve_".$tbname.$doctb." where id='$id[$i]'");
		if($doselfinfo['doselfinfo']&&($r[userid]<>$userid||$r[ismember]))//只能编辑自己的信息
		{
			continue;
		}
		...
		if($donum==1)
		{
			...
			$dolog="classid=".$r['classid']."<br>id=".$r['id']."&elve=$elve<br>title=".$r['title'];
		}
		//分页字段
		if($pf)
		{
			if(strstr($emod_r[$mid]['tbdataf'],','.$pf.','))
			{
				if($elve==1)
				{
					$finfor=$Elves->fetch1("select ".$pf." from {$dbtbpre}elve_".$tbname."_doc_data where id='$id[$i]'");
				}
				elseif($elve==2)
				{
					$finfor=$Elves->fetch1("select ".$pf." from {$dbtbpre}elve_".$tbname."_check_data where id='$id[$i]'");
				}
				else
				{
					$finfor=$Elves->fetch1("select ".$pf." from {$dbtbpre}elve_".$tbname."_data_".$r[stb]." where id='$id[$i]'");
				}
				$r[$pf]=$finfor[$pf];
			}
		}
		...
		if($elve==0)
		{
			$Elves->query("delete from {$dbtbpre}elve_".$tbname."_data_".$r[stb]." where id='$id[$i]'");
		}
		//删除其它表记录和附件
		...
		if($elve==0||$elve==2)
		{
			AddClassInfos($r['classid'],'-1','-1',$elve==2?0:1);
		}
    }
	//删除信息
	$sql=$Elves->query("delete from {$dbtbpre}elve_".$tbname.$doctb." where ".$add);
	if($elve==0)
	{
		$Elves->query("delete from {$dbtbpre}elve_".$tbname."_index where ".$add);
		$ccr=$Elves->fetch1("select classid,modid,listdt,haddlist from {$dbtbpre}melveclass where classid='$classid'");
		hAddListHtml($classid,$ccr['modid'],$ccr['haddlist'],$ccr['listdt']);//生成信息列表
	}
	elseif($elve==1)
	{
		$Elves->query("delete from {$dbtbpre}elve_".$tbname."_doc_index where ".$add);
		$Elves->query("delete from {$dbtbpre}elve_".$tbname."_doc_data where ".$add);
	}
	elseif($elve==2)
	{
		$Elves->query("delete from {$dbtbpre}elve_".$tbname."_index where ".$add);
		$Elves->query("delete from {$dbtbpre}elve_".$tbname."_check_data where ".$add);
	}
	if($sql)
	{
		//操作日志
		...
		{
			insert_dolog("classid=".$classid."<br>classname=".$class_r[$classid][classname]."&elve=$elve");
		}
...
function EditMoreInfoTime($add,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		...
		{
			//索引表
			$index_r=$Elves->fetch1("select classid,checked from {$dbtbpre}elve_".$tbname."_index where id='$doinfoid' limit 1");
			if(!$index_r['classid'])
			{
				continue;
			}
		...
		$Elves->query("update {$dbtbpre}elve_".$tbname."_index set newstime='$donewstime' where id='$doinfoid'");
		$Elves->query("update ".$infotb." set newstime='$donewstime' where id='$doinfoid'");
	}
	//操作日志
	if($donum==1)
	{
...
function AddInfoToCopyInfo($classid,$id,$to_classid,$userid,$username,$usergroupr){
	global $Elves,$public_r,$class_r,$dbtbpre,$emod_r;
	$classid=(int)$classid;
	$id=(int)$id;
	$cr=$to_classid;
	$count=count($cr);
	...
	$stf=$emod_r[$mid]['savetxtf'];
	//索引表
	$index_r=$Elves->fetch1("select id,checked from {$dbtbpre}elve_".$tbname."_index where id='$id' limit 1");
	if(empty($index_r['id']))
	{
		return '';
	}
	...
	$infotb=ReturnInfoMainTbname($tbname,$index_r['checked']);
	//主表
	$r=$Elves->fetch1("select * from ".$infotb." where id='$id'");
	//返回表信息
	$infotbr=ReturnInfoTbname($tbname,$index_r['checked'],$r['stb']);
	//副表
	$fr=$Elves->fetch1("select ".ReturnSqlFtextF($mid)." from ".$infotbr['datatbname']." where id='$id' limit 1");
	$r=array_merge($r,$fr);
	if($stf)//存放文本
	{
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$checked=(int)$checked;
		//索引表
		$Elves->query("insert into {$dbtbpre}elve_".$tbname."_index(classid,checked,newstime,truetime,lastdotime,havehtml) values('$newclassid','$checked','$r[newstime]','$r[truetime]','$r[truetime]','$r[havehtml]');");
		$l_id=$Elves->lastid();
		$infotbr=ReturnInfoTbname($tbname,$checked,$ret_r['tb']);
		//主表
		$Elves->query("insert into ".$infotbr['tbname']."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard".$ret_r[fields].") values('$l_id','$newclassid','$r[ttid]',0,0,0,'$newspath','$filename','$r[userid]','".StripAddsData($r[username])."',0,0,'$r[ispic]',0,'$isqf',0,'$r[isurl]','$r[truetime]',$r[truetime],$r[havehtml],$r[groupid],$r[userfen],'".StripAddsData($r[titlefont])."','".StripAddsData($r[titleurl])."','$ret_r[tb]','$public_r[filedeftb]','$public_r[pldeftb]','".StripAddsData($r[keyboard])."'".$ret_r[values].");");
		//副表
		$Elves->query("insert into ".$infotbr['datatbname']."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags".$ret_r[datafields].") values('$l_id','$newclassid','$r[keyid]',$r[dokey],'".$newstempid."',$r[closepl],0,''".$ret_r[datavalues].");");
		//公共表
		...
		$Elves->query("update ".$infotbr['tbname']." set filename='$filename'".$updateinfourl." where id='$l_id' limit 1");
		//生成信息文件
		if($checked)
		{
			$addr=$Elves->fetch1("select * from ".$infotbr['tbname']." where id='$l_id' limit 1");
...
function EditInfoToCopyInfo($classid,$id,$userid,$username,$usergroupr){
	...
	//公共表
	$pubid=ReturnInfoPubid($classid,$id);
	$pub_r=$Elves->fetch1("select copyids from {$dbtbpre}melveinfovote where pubid='$pubid'");
	...
	for($i=1;$i<$count-1;$i++)
	{
		...
		//索引表
		$index_infor=$Elves->fetch1("select classid,checked from {$dbtbpre}elve_".$tbname."_index where id='$infoid' limit 1");
		//返回表
		...
		if($stf)
		{
			...
			{
				$infor=$Elves->fetch1("select stb,isurl,newspath,filename from ".$update_infotb." where id='$infoid' limit 1");
				if(!$infor[stb])
				{
					continue;
				}
				...
				$infodr=$Elves->fetch1("select ".$stf." from ".$update_infotbr['datatbname']." where id='$infoid' limit 1");
				$r[newstext_url]=$infodr[$stf];
			}
			else
			{
				$infor=$Elves->fetch1("select ".$stf.",stb,isurl,newspath,filename from ".$update_infotb." where id='$infoid' limit 1");
				if(!$infor[stb])
				{
					continue;
				}
		...
		$update_infotbr=ReturnInfoTbname($tbname,$index_infor['checked'],$infor['stb']);
		//索引表
		$Elves->query("update {$dbtbpre}elve_".$tbname."_index set checked='$index_r[checked]',newstime='$r[newstime]',lastdotime='$r[lastdotime]' where id='$infoid'");
		//主表
		$Elves->query("update ".$update_infotb." set ttid='$r[ttid]',ispic='$r[ispic]',isurl='$r[isurl]',lastdotime=$r[lastdotime],groupid=$r[groupid],userfen=$r[userfen],titlefont='".StripAddsData($r[titlefont])."',titleurl='".StripAddsData($r[titleurl])."',keyboard='".StripAddsData($r[keyboard])."'".$ret_r[values]." where id='$infoid'");
		//副表
		$Elves->query("update ".$update_infotbr['datatbname']." set keyid='$r[keyid]',dokey=$r[dokey],closepl=$r[closepl]".$ret_r[datavalues]." where id='$infoid'");
		//是否改变审核状态
		...
		{
			//生成信息文件
			$addr=$Elves->fetch1("select * from {$dbtbpre}elve_".$tbname." where id='$infoid' limit 1");
			GetHtml($addr['classid'],$addr['id'],$addr,1);
		}
	}
}
...
function DelInfoToCopyInfo($classid,$id,$r,$userid,$username,$usergroupr){
	...
	for($i=1;$i<$count-1;$i++)
	{
		...
		//索引表
		$index_infor=$Elves->fetch1("select checked from {$dbtbpre}elve_".$tbname."_index where id='$infoid' limit 1");
		//返回表
		$update_infotb=ReturnInfoMainTbname($tbname,$index_infor['checked']);
		//主表
		$infor=$Elves->fetch1("select * from ".$update_infotb." where id='$infoid' limit 1");
		if(!$infor[stb])
		{
			continue;
		}
		...
		if($selectdataf)
		{
			$infodr=$Elves->fetch1("select ".$selectdataf." from ".$update_infotbr['datatbname']." where id='$infoid' limit 1");
		...
		$Elves->query("delete from {$dbtbpre}elve_".$tbname."_index where id='$infoid'");
		$Elves->query("delete from ".$update_infotbr['tbname']." where id='$infoid'");
		$Elves->query("delete from ".$update_infotbr['datatbname']." where id='$infoid'");
		//更新栏目信息数
...
function TopNews_all($classid,$id,$istop,$userid,$username){
	global $Elves,$bclassid,$class_r,$dbtbpre;
	$classid=(int)$classid;
	if(empty($classid))
	{
		printerror("ErrorUrl","history.go(-1)");
	...
	for($i=0;$i<$count;$i++)
	{
		...
		{
			$index_r=$Elves->fetch1("select checked from {$dbtbpre}elve_".$class_r[$classid][tbname]."_index where id='$infoid' limit 1");
			//返回表
			$infotb=ReturnInfoMainTbname($class_r[$classid][tbname],$index_r['checked']);
		}
		$donum++;
	...
	$istop=(int)$istop;
	$add=substr($add,0,strlen($add)-4);
	$sql=$Elves->query("update ".$infotb." set istop=$istop where ".$add);
	if($index_r['checked'])
	{
		//刷新列表
...
function CheckNews_all($classid,$id,$userid,$username){
	global $Elves,$class_r,$dbtbpre,$emod_r,$adddatar;
	$classid=(int)$classid;
	if(empty($classid))
	{
		printerror("ErrorUrl","history.go(-1)");
	...
	$dolog='';
	//附加链接参数
	$addelvecheck=empty($_POST['elvecheck'])?'&elvecheck=1':'';
	$add='';
	for($i=0;$i<$count;$i++)
	{
		$infoid=(int)$id[$i];
		...
		$infor=$Elves->fetch1("select * from {$dbtbpre}elve_".$class_r[$classid][tbname]."_check where id='$infoid' limit 1");
		if(!$infor['id']||$infor['isqf']==1)
		{
			continue;
		}
		$sql=$Elves->query("update {$dbtbpre}elve_".$class_r[$classid][tbname]."_index set checked=1 where id='$infoid'");
		//投稿增加积分
		if($infor['ismember']&&$infor['userid'])
		{
			$cr=$Elves->fetch1("select classid,addinfofen from {$dbtbpre}melveclass where classid='$infor[classid]'");
			if($cr['addinfofen'])
			{
				$finfor=$Elves->fetch1("select haveaddfen from {$dbtbpre}elve_".$class_r[$classid][tbname]."_check_data where id='$infoid' limit 1");
			...
			$Elves->query("update {$dbtbpre}elve_".$class_r[$classid][tbname]."_check_data set haveaddfen=1 where id='$infoid'");
		}
		//未审核表转换
	...
	if($adddatar['causetext']&&$infoid)
	{
		if(!$infor['id'])
		{
			$infor=$Elves->fetch1("select * from {$dbtbpre}elve_".$class_r[$classid][tbname]." where id='$infoid' limit 1");
		}
...
function NoCheckNews_all($classid,$id,$userid,$username){
	global $Elves,$class_r,$public_r,$dbtbpre,$emod_r,$adddatar;
	$classid=(int)$classid;
	if(empty($classid))
	{
		printerror("ErrorUrl","history.go(-1)");
	...
	for($i=0;$i<$count;$i++)
	{
		$infoid=(int)$id[$i];
		//主表
		$r=$Elves->fetch1("select * from {$dbtbpre}elve_".$class_r[$classid][tbname]." where id='$infoid' limit 1");
		if(!$r['id']||$r['isqf']==1)
		{
			continue;
		}
		$sql=$Elves->query("update {$dbtbpre}elve_".$class_r[$classid][tbname]."_index set checked=0 where id='$infoid'");
		//未审核互转
		...
		if($pf)
		{
			if(strstr($emod_r[$mid]['tbdataf'],','.$pf.','))
			{
				$finfor=$Elves->fetch1("select ".$pf." from {$dbtbpre}elve_".$tbname."_data_".$r[stb]." where id='$infoid'");
				$r[$pf]=$finfor[$pf];
			}
			if($stf&&$stf==$pf)//存放文本
			{
	...
	if($adddatar['causetext']&&$infoid)
	{
		if(!$r['id'])
		{
			$r=$Elves->fetch1("select * from {$dbtbpre}elve_".$class_r[$classid][tbname]."_check where id='$infoid' limit 1");
		}
...
//移动信息
function Movmelve_all($classid,$id,$to_classid,$userid,$username){
	global $Elves,$class_r,$dbtbpre,$emod_r,$adddatar;
	$classid=(int)$classid;
	...
	if(empty($count))
	{
		printerror("NotMovmelveid","history.go(-1)");
	}
	$dopubid=0;
	$donum=0;
	$dolog='';
	...
	for($i=0;$i<$count;$i++)
	{
		...
		{
			//索引表
			$index_r=$Elves->fetch1("select checked from {$dbtbpre}elve_".$tbname."_index where id='".$id[$i]."' limit 1");
			//返回表
			$infotb=ReturnInfoMainTbname($tbname,$index_r['checked']);
		}
		//主表
		$r=$Elves->fetch1("select stb,classid,fstb,restb,id,isurl,filename,groupid,newspath,titleurl,title,ismember,userid,username,newstime,truetime from ".$infotb." where id='".$id[$i]."' limit 1");
		...
		$infotbr=ReturnInfoTbname($tbname,$index_r['checked'],$r['stb']);
		//主表
		$Elves->query("update ".$infotb." set classid='$to_classid',titleurl='$infourl' where id='".$id[$i]."'");
		//副表
		$Elves->query("update ".$infotbr['datatbname']." set classid='$to_classid' where id='".$id[$i]."'");
		//更新栏目信息数
	...
	$add=substr($add,0,strlen($add)-4);
	$sql=$Elves->query("update {$dbtbpre}elve_".$tbname."_index set classid='$to_classid' where ".$add);
	//刷新列表
	ReListHtml($classid,1);
	...
	if($sql)
	{
		...
		printerror("MovmelveSuccess",$returl);
	}
	else
	{printerror("DbError","history.go(-1)");}
}
...
function CopyNews_all($classid,$id,$to_classid,$userid,$username){
    ...
    $truetime=$newstime;
	$newstempid=0;
	$dosql=$Elves->query("select id,classid,checked from {$dbtbpre}elve_".$tbname."_index where ".$add);
	while($index_r=$Elves->fetch($dosql))
	{
		//返回表
		$infotb=ReturnInfoMainTbname($tbname,$index_r['checked']);
		//主表
		$r=$Elves->fetch1("select * from ".$infotb." where id='$index_r[id]' limit 1");
		//返回表信息
		$infotbr=ReturnInfoTbname($tbname,$index_r['checked'],$r['stb']);
		//副表
		...
		$checked=(int)$checked;
		//索引表
		$Elves->query("insert into {$dbtbpre}elve_".$tbname."_index(classid,checked,newstime,truetime,lastdotime,havehtml) values('$to_classid','$checked','$r[newstime]','$truetime','$truetime','$r[havehtml]');");
		$l_id=$Elves->lastid();
		$update_infotbr=ReturnInfoTbname($tbname,$checked,$ret_r['tb']);
		//主表
		$sql=$Elves->query("insert into ".$update_infotbr['tbname']."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard".$ret_r[fields].") values('$l_id','$to_classid','$r[ttid]',0,0,0,'$newspath','$filename',$userid,'$username',0,0,'$r[ispic]',0,'$isqf',0,'$r[isurl]',$truetime,$truetime,$r[havehtml],$r[groupid],$r[userfen],'$r[titlefont]','$r[titleurl]','$ret_r[tb]','$public_r[filedeftb]','$public_r[pldeftb]','$r[keyboard]'".$ret_r[values].");");		
		//副表
		$Elves->query("insert into ".$update_infotbr['datatbname']."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags".$ret_r[datafields].") values('$l_id','$to_classid','$r[keyid]',$r[dokey],'".$newstempid."',$r[closepl],0,'$r[infotags]'".$ret_r[datavalues].");");
		//签发
		if($isqf==1)
		{
		...
		$usql=$Elves->query("update ".$update_infotbr['tbname']." set filename='$filename'".$updateinfourl." where id='$l_id'");
		//更新栏目信息数
		AddClassInfos($to_classid,'+1','+1',$index_r['checked']);
		//生成信息文件
		if($checked)
		{
			$addr=$Elves->fetch1("select * from ".$update_infotbr['tbname']." where id='$l_id'");
			GetHtml($addr['classid'],$addr['id'],$addr,1);
		}
		$donum++;
		if($donum==1)
...
function MoveClassNews($add,$userid,$username){
	global $Elves,$class_r,$dbtbpre,$emod_r;
	$add[classid]=(int)$add[classid];
	...
	//验证权限
	CheckLevel($userid,$username,$classid,"movmelve");
	//终极栏目
	...
	//主表
	$indexsql=$Elves->query("update {$dbtbpre}elve_".$tbname."_index set classid=$add[toclassid] where classid='$add[classid]'");
	$sql=$Elves->query("update {$dbtbpre}elve_".$tbname." set classid=$add[toclassid] where classid='$add[classid]'");
	$Elves->query("update {$dbtbpre}elve_".$tbname."_check set classid=$add[toclassid] where classid='$add[classid]'");
	$Elves->query("update {$dbtbpre}elve_".$tbname."_doc set classid=$add[toclassid] where classid='$add[classid]'");
	//副表
	UpdateAllDataTbField($tbname,"classid='$add[toclassid]'"," where classid='$add[classid]'",1,1);
	//更新栏目信息数
	$cr=$Elves->fetch1("select classid,allinfos,infos from {$dbtbpre}melveclass where classid='$add[classid]'");
	AddClassInfos($add[classid],'-'.$cr[allinfos],'-'.$cr[infos]);
	$tocr=$Elves->fetch1("select classid,allinfos,infos from {$dbtbpre}melveclass where classid='$add[toclassid]'");
	...
	//移动数据
	$opath=elve_PATH.$class_r[$add[classid]][classpath];
...
function GoodInfo_all($classid,$id,$isgood,$doing=0,$userid,$username){
	...
	$add=substr($add,0,strlen($add)-4);
	$sql=$Elves->query("update ".$infotb." set ".$setf." where ".$add);
	if($sql)
	{
		//操作日志
		if($donum==1)
...
function SetAllCheckInfo($bclassid,$classid,$userid,$username){
	...
	//附加链接参数
	$addelvecheck=empty($_POST['elvecheck'])?'&elvecheck=1':'';
	$sql=$Elves->query("select id,classid,userid,ismember,isqf,stb from {$dbtbpre}elve_".$class_r[$classid][tbname]."_check where classid='$classid'");
	$n=0;
	while($r=$Elves->fetch($sql))
	{
		...
		$Elves->query("update {$dbtbpre}elve_".$class_r[$classid][tbname]."_index set checked=1 where id='$r[id]'");
		//审核表转换
...
function DoWfInfo($add,$userid,$username){
	global $Elves,$dbtbpre,$class_r,$emod_r,$lur;
	$id=(int)$add[id];
	$classid=(int)$add[classid];
	$doing=(int)$add['doing'];
	if(!$id||!$classid||!$doing)
	...
	$wfinfor=$Elves->fetch1("select id,checknum,wfid,tid,groupid,userclass,username,checktno from {$dbtbpre}melvewfinfo where id='$id' and classid='$classid' limit 1");
	if(!$wfinfor[id])
	{
		printerror('ErrorUrl','');
	}
	...
	$wfitemr=$Elves->fetch1("select tid,wfid,tno,groupid,userclass,username,lztype,tbdo,tddo,tstatus from {$dbtbpre}melveworkflowitem where tid='$wfinfor[tid]'");
	...
	if($doing==1)//通过
	{
		if($wfitemr[lztype]==0)//普签
		{
			...
			{
				$Elves->query("update {$dbtbpre}elve_".$class_r[$classid][tbname]."_index set checked=1 where id='$id'");
				$ar=$Elves->fetch1("select * from {$dbtbpre}elve_".$class_r[$classid][tbname]."_check where id='$id'");
				//未审核表转换
				MoveCheckInfoData($class_r[$classid][tbname],0,$ar['stb'],"id='$id'");
				//更新栏目信息数
				AddClassInfos($classid,'','+1');
				$Elves->query("update {$dbtbpre}melvewfinfo set tstatus='',checktno='100' where id='$id' and classid='$classid' limit 1");
				//日志
			...
			{
				$newwfitemr=$Elves->fetch1("select tid,wfid,tno,groupid,userclass,username,lztype,tbdo,tddo,tstatus from {$dbtbpre}melveworkflowitem where wfid='$wfitemr[wfid]' and tno>$wfitemr[tno] order by tno limit 1");
				$Elves->query("update {$dbtbpre}melvewfinfo set tid='$newwfitemr[tid]',groupid='$newwfitemr[groupid]',userclass='$newwfitemr[userclass]',username='$newwfitemr[username]',tstatus='$newwfitemr[tstatus]',checktno='0' where id='$id' and classid='$classid' limit 1");
				//日志
		...
		else//会签
		{
			...
			{
				$Elves->query("update {$dbtbpre}melvewfinfo set groupid='$newgroupid',userclass='$newuserclass',username='$newusername' where id='$id' and classid='$classid' limit 1");
				//日志
				InsertWfLog($classid,$id,$wfitemr[wfid],$wfitemr[tid],$username,$checktext,$wfinfor[checknum],1);
			}
		}
	...
	elseif($doing==2)//返工
	{
		...
		{
			$Elves->query("update {$dbtbpre}melvewfinfo set tid=0,tstatus='',checktno='101' where id='$id' and classid='$classid' limit 1");
		}
		else//发给节点
		{
			$newwfitemr=$Elves->fetch1("select tid,wfid,tno,groupid,userclass,username,lztype,tbdo,tddo,tstatus from {$dbtbpre}melveworkflowitem where wfid='$wfitemr[wfid]' and tid='$wfitemr[tbdo]' limit 1");
			$Elves->query("update {$dbtbpre}melvewfinfo set tid='$newwfitemr[tid]',groupid='$newwfitemr[groupid]',userclass='$newwfitemr[userclass]',username='$newwfitemr[username]',tstatus='$newwfitemr[tstatus]',checktno='101' where id='$id' and classid='$classid' limit 1");
		}
		//日志
	...
	else//否决
	{
		...
		$Elves->query("update {$dbtbpre}melvewfinfo set tid=0,tstatus='',checktno='102' where id='$id' and classid='$classid' limit 1");
		//日志
		InsertWfLog($classid,$id,$wfitemr[wfid],$wfitemr[tid],$username,$checktext,$wfinfor[checknum],3);
		if($wfitemr[tddo])//删除信息
		{
			$index_r=$Elves->fetch1("select checked from {$dbtbpre}elve_".$class_r[$classid][tbname]."_index where id='$id' limit 1");
			//返回表
			$infotb=ReturnInfoMainTbname($class_r[$classid][tbname],$index_r['checked']);
			$r=$Elves->fetch1("select * from ".$infotb." where id='$id' limit 1");
			$mid=$class_r[$classid][modid];
			...
			DelNewsFile($r[filename],$r[newspath],$classid,$r[$pf],$r[groupid]);//删除信息文件
			$Elves->query("delete from {$dbtbpre}elve_".$tbname."_index where id='$id'");
			$sql=$Elves->query("delete from ".$infotbr['tbname']." where id='$id'");
			$fsql=$Elves->query("delete from ".$infotbr['datatbname']." where id='$id'");
			//更新栏目信息数
			AddClassInfos($r[classid],'-1','-1',$index_r['checked']);
...
function DelInfoData($start,$classid,$from,$retype,$startday,$endday,$startid,$endid,$tbname,$add,$userid,$username){
	global $Elves,$public_r,$class_r,$fun_r,$dbtbpre,$emod_r;
	//验证权限
	...
	if($infost)
	{
		...
		{
			$infotb="{$dbtbpre}elve_".$tbname."_check";
			$delckinfo=2;
		}
	...
	$b=0;
	$sql=$Elves->query("select * from ".$infotb." where id>$start".$add1." order by id limit ".$public_r[delnewsnum]);
	while($r=$Elves->fetch($sql))
	{
		$b=1;
		$new_start=$r[id];
		...
		if($delckinfo==2)
		{
			$infodatatb="{$dbtbpre}elve_".$tbname."_check_data";
		}
		else
		{
			$infodatatb="{$dbtbpre}elve_".$tbname."_data_".$r['stb'];
		}
		//分页字段
		if($pf)
		{
			if(strstr($emod_r[$mid]['tbdataf'],','.$pf.','))
			{
				$finfor=$Elves->fetch1("select ".$pf." from ".$infodatatb." where id='$r[id]' limit 1");
				$r[$pf]=$finfor[$pf];
			}
		}
		//存文本
		...
		//删除表信息
		$Elves->query("delete from {$dbtbpre}elve_".$tbname."_index where id='$r[id]'");
		$Elves->query("delete from ".$infotb." where id='$r[id]'");
		$Elves->query("delete from ".$infodatatb." where id='$r[id]'");
		//更新栏目信息数
	...
	if(empty($b))
	{
		...
		{
			echo $fun_r[DelDataSuccess]."<script>self.location.href='elveinfo.php?melve=DelInfoData&start=0&from=$from&delhtml=$add[delhtml]&dodelcheck=1".$search."';</script>";
			exit();
		}
	    //操作日志
	    insert_dolog("");
	...
	echo $fun_r[OneDelDataSuccess]."(ID:<font color=red><b>".$new_start."</b></font>)<script>self.location.href='elveinfo.php?melve=DelInfoData&start=$new_start&from=$from&delhtml=$add[delhtml]".$search."';</script>";
	exit();
}
...
function InfoToDoc_class($add,$userid,$username){
	...
	$start=(int)$add['start'];
	$cr=$Elves->fetch1("select tbname,doctime from {$dbtbpre}melveclass where classid='$classid' and islast=1");
	...
	$sql=$Elves->query("select * from {$dbtbpre}elve_".$cr[tbname]." where id>$start and classid='$classid' and truetime<$doctime order by id limit ".$line);
	while($r=$Elves->fetch($sql))
	{
		$b=1;
		$new_start=$r['id'];
	...
	echo $fun_r[OneInfoToDocSuccess]."(ID:<font color=red><b>".$new_start."</b></font>)<script>self.location.href='elveinfo.php?melve=InfoToDoc&elvedoc=1&classid=$classid&start=$new_start&docfrom=$add[docfrom]';</script>";
	exit();
}
...
function InfoToDoc($add,$userid,$username){
	...
	$line=$public_r['docnewsnum'];
	$b=0;
	$sql=$Elves->query("select * from {$dbtbpre}elve_".$selecttbname." where id>$start and ".$where." order by id limit ".$line);
	while($r=$Elves->fetch($sql))
	{
		$b=1;
		$new_start=$r['id'];
	...
	echo $fun_r[OneInfoToDocSuccess]."(ID:<font color=red><b>".$new_start."</b></font>)<script>self.location.href='elveinfo.php?melve=InfoToDoc&elvedoc=2&doing=$add[doing]&tbname=$tbname&start=$new_start&docfrom=$add[docfrom]".$search."';</script>";
	exit();
}
...
function InfoToDoc_info($add,$userid,$username){
	...
	$where="id in (".$ids.")";
	$sql=$Elves->query("select * from {$dbtbpre}elve_".$selecttbname." where ".$where);
	while($r=$Elves->fetch($sql))
	{
		DoDocInfo($tbname,$r,$add['doing']);
	}
...
//处理归档
function DoDocInfo($tb,$r,$elve=0){
	global $Elves,$dbtbpre,$class_r,$emod_r;
	if($elve==1)//还原
	{
		$table=$dbtbpre.'elve_'.$tb.'_doc_index';	//索引表
		$table1=$dbtbpre.'elve_'.$tb.'_doc';	//主表
		$table2=$dbtbpre.'elve_'.$tb.'_doc_data';	//副表
		$ytable=$dbtbpre.'elve_'.$tb.'_index';	//目标索引表
		$ytable1=$dbtbpre.'elve_'.$tb;	//目标主表
		$ytable2=$dbtbpre.'elve_'.$tb.'_data_'.$r[stb];	//目标副表
	}
	else//归档
	{
		$table=$dbtbpre.'elve_'.$tb.'_index';	//索引表
		$table1=$dbtbpre.'elve_'.$tb;	//主表
		$table2=$dbtbpre.'elve_'.$tb.'_data_'.$r[stb];	//副表
		$ytable=$dbtbpre.'elve_'.$tb.'_doc_index';	//目标索引表
		$ytable1=$dbtbpre.'elve_'.$tb.'_doc';	//目标主表
		$ytable2=$dbtbpre.'elve_'.$tb.'_doc_data';	//目标副表
	}
	$mid=$class_r[$r[classid]][modid];
	//索引表
	$index_r=$Elves->fetch1("select * from ".$table." where id='$r[id]' limit 1");
	//副表
	$fr=$Elves->fetch1("select ".ReturnSqlFtextF($mid)." from ".$table2." where id='$r[id]' limit 1");
	$r=array_merge($r,$fr);
	$ret_r=ReturnAddF($r,$mid,$userid,$username,10,0,0);//返回自定义字段
	//索引表
	$Elves->query("insert into ".$ytable."(id,classid,checked,newstime,truetime,lastdotime,havehtml) values('$index_r[id]','$index_r[classid]','$index_r[checked]','$index_r[newstime]','$index_r[truetime]','$index_r[lastdotime]','$index_r[havehtml]');");
	//主表
	$Elves->query("replace into ".$ytable1."(id,classid,ttid,onclick,plnum,totaldown,newspath,filename,userid,username,firsttitle,isgood,ispic,istop,isqf,ismember,isurl,truetime,lastdotime,havehtml,groupid,userfen,titlefont,titleurl,stb,fstb,restb,keyboard".$ret_r[fields].") values('$r[id]','$r[classid]','$r[ttid]','$r[onclick]','$r[plnum]','$r[totaldown]','".StripAddsData($r[newspath])."','".StripAddsData($r[filename])."','$r[userid]','".StripAddsData($r[username])."','$r[firsttitle]','$r[isgood]','$r[ispic]','$r[istop]','$r[isqf]','$r[ismember]','$r[isurl]','$r[truetime]','$r[lastdotime]','$r[havehtml]','$r[groupid]','$r[userfen]','".StripAddsData($r[titlefont])."','".StripAddsData($r[titleurl])."','$r[stb]','$r[fstb]','$r[restb]','".StripAddsData($r[keyboard])."'".$ret_r[values].");");
	//副表
	$Elves->query("replace into ".$ytable2."(id,classid,keyid,dokey,newstempid,closepl,haveaddfen,infotags".$ret_r[datafields].") values('$r[id]','$r[classid]','$r[keyid]','$r[dokey]','$r[newstempid]','$r[closepl]','$r[haveaddfen]','".StripAddsData($r[infotags])."'".$ret_r[datavalues].");");
	//删除
	$Elves->query("delete from ".$table." where id='$r[id]'");
	$Elves->query("delete from ".$table1." where id='$r[id]'");
	$Elves->query("delete from ".$table2." where id='$r[id]'");
	//更新栏目信息数
	if($elve==1)//还原
	{
...
//处理归档(未审核信息)
function DoDocCkInfo($tb,$where,$elve=0){
	global $Elves,$dbtbpre,$class_r,$emod_r;
	if($elve==1)//还原
	{
		$table=$dbtbpre.'elve_'.$tb.'_doc_index';	//主表
		$table1=$dbtbpre.'elve_'.$tb.'_doc';	//主表
		$table2=$dbtbpre.'elve_'.$tb.'_doc_data';	//副表
		$ytable=$dbtbpre.'elve_'.$tb.'_index';	//目标主表
		$ytable1=$dbtbpre.'elve_'.$tb.'_check';	//目标主表
		$ytable2=$dbtbpre.'elve_'.$tb.'_check_data';	//目标副表
	}
	else//归档
	{
		$table=$dbtbpre.'elve_'.$tb.'_index';	//主表
		$table1=$dbtbpre.'elve_'.$tb.'_check';	//主表
		$table2=$dbtbpre.'elve_'.$tb.'_check_data';	//副表
		$ytable=$dbtbpre.'elve_'.$tb.'_doc_index';	//目标主表
		$ytable1=$dbtbpre.'elve_'.$tb.'_doc';	//目标主表
		$ytable2=$dbtbpre.'elve_'.$tb.'_doc_data';	//目标副表
	}
	//转换副表
	$fids='';
	$dh='';
	$sql=$Elves->query("select id,classid from ".$table1." where ".$where);
	while($r=$Elves->fetch($sql))
	{
		$fids.=$dh.$r['id'];
		$dh=',';
	...
	$Elves->query("replace into ".$ytable." select * from ".$table." where ".$where);
	$Elves->query("replace into ".$ytable1." select * from ".$table1." where ".$where);
	$Elves->query("replace into ".$ytable2." select * from ".$table2." where id in (".$fids.")");
	//删除
	$Elves->query("delete from ".$table." where ".$where);
	$Elves->query("delete from ".$table1." where ".$where);
	$Elves->query("delete from ".$table2." where id in (".$fids.")");
}

//发送信息操作通知
function DoInfoSendNotice($userid,$username,$to_userid,$to_username,$causetext,$infor,$elve=0){
	global $Elves,$dbtbpre,$class_r;
	if(!$infor['ismember'])
	{
		return '';
	...
	//操作者
	$user_r=$Elves->fetch1("select truename from {$dbtbpre}melveuser where userid='$userid'");
