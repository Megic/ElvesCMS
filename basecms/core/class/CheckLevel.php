...
function ShowViewInfoMsg($r,$msg){
	...
	$url="<a href='".$public_r[newsurl]."'>首页</a>&nbsp;>&nbsp;<a href='".$public_r[newsurl]."core/member/cp/'>会员中心</a>&nbsp;>&nbsp;查看信息：".$r['title'];
	include($check_path."core/data/template/cp_1.php");
	?>
...
<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="tableborder">
  ...
  <tr bgcolor="#FFFFFF"> 
	...
	<?php
	include($check_path."core/data/template/cp_2.php");
	exit();
}
...
function ViewCheckLogin($infor){
	global $Elves,$public_r,$elve_config,$toreturnurl,$gotourl;
	...
	if(!$userid)
	{
		...
		$msg="您还未登陆，<a href='$gotourl'><u>点击这里</u></a>进行登陆操作；注册请<a href='".$public_r['newsurl']."core/member/register/'><u>点击这里</u></a>。";
		ShowViewInfoMsg($infor,$msg);
	}
	$cr=$Elves->fetch1("select ".eReturnSelectMemberF('checked,userid,username,groupid,userfen,userdate,zgroupid')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' and ".egetmf('rnd')."='$rnd' limit 1");
	if(!$cr['userid'])
	{
		EmptyelveCookie();
		if(!getcvar('returnurl'))
		...
		$msg="同一帐号只能一人在线，<a href='$gotourl'><u>点击这里</u></a>重新登陆；注册请<a href='".$public_r['newsurl']."core/member/register/'><u>点击这里</u></a>。";
		ShowViewInfoMsg($infor,$msg);
	}
	if($cr['checked']==0)
	{
		...
		$msg="您的帐号还未审核通过，<a href='$gotourl'><u>点击这里</u></a>重新登陆；注册请<a href='".$public_r['newsurl']."core/member/register/'><u>点击这里</u></a>。";
		ShowViewInfoMsg($infor,$msg);
	}
	//默认会员组
	if(empty($cr['groupid']))
...
function CheckShowNewsLevel($infor){
	global $check_path,$level_r,$Elves,$gotourl,$toreturnurl,$public_r,$dbtbpre,$class_r;
	...
	{
		$checkcr=$Elves->fetch1("select cgroupid from {$dbtbpre}melveclass where classid='$infor[classid]'");
		if($checkcr['cgroupid'])
		{
			if(!strstr($checkcr[cgroupid],','.$user_r[groupid].','))
			{
				...
				$msg="您没有足够权限查看此信息! <a href='$gotourl'><u>点击这里</u></a>重新登陆；注册请<a href='".$public_r['newsurl']."core/member/register/'><u>点击这里</u></a>。";
				ShowViewInfoMsg($infor,$msg);
			}
		}
	}
	if($groupid)//信息设置
	{
		if($level_r[$groupid][level]>$level_r[$user_r[groupid]][level])
		{
			...
			$msg="您的会员级别不足(您的当前级别：".$level_r[$user_r[groupid]][groupname].")，没有查看此信息的权限! <a href='$gotourl'><u>点击这里</u></a>重新登陆；注册请<a href='".$public_r['newsurl']."core/member/register/'><u>点击这里</u></a>。";
			ShowViewInfoMsg($infor,$msg);
		}
	}
	//扣点
	...
	{
		//是否有历史记录
		$bakr=$Elves->fetch1("select id,truetime from {$dbtbpre}melvedownrecord where id='$id' and classid='$classid' and userid='$user_r[userid]' and online=2 order by truetime desc limit 1");
		...
		else
		{
			...
			else
			{
				if($user_r[userfen]<$userfen)
				{
					...
					$msg="您的点数不足(您当前拥有的点数 ".$user_r[userfen]." 点)，没有查看此信息的权限! <a href='$gotourl'><u>点击这里</u></a>重新登陆；注册请<a href='".$public_r['newsurl']."core/member/register/'><u>点击这里</u></a>。";
					ShowViewInfoMsg($infor,$msg);
				}
				//扣点
				$usql=$Elves->query("update ".eReturnMemberTable()." set ".egetmf('userfen')."=".egetmf('userfen')."-".$userfen." where ".egetmf('userid')."='$user_r[userid]'");
			}
			//备份下载记录
...
if(!defined('PageCheckLevel'))
{
	...
	require_once(elve_PATH.'core/data/dbcache/class.php');
	require_once(elve_PATH.'core/data/dbcache/MemberLevel.php');
	$link=db_connect();
	$Elves=new mysqlquery();
	$check_tbname=RepPostVar($check_tbname);
	$checkinfor=$Elves->fetch1("select * from {$dbtbpre}elve_".$check_tbname." where id='$check_infoid' limit 1");
	...
	$checkfinfor=$Elves->fetch1("select ".ReturnSqlFtextF($check_mid)." from {$dbtbpre}elve_".$check_tbname."_data_".$checkinfor[stb]." where id='$checkinfor[id]' limit 1");
...
require_once(elve_PATH.'core/member/class/user.php');
//验证IP
