...
class mysqlquery
{
	...
	function query($query){
		global $elve_config;
		$this->sql=mysql_query($query,return_dblink($query)) or die($elve_config['db']['showerror']==1?mysql_error().'<br>'.str_replace($GLOBALS['dbtbpre'],'***_',$query):'DbError');
		return $this->sql;
	}
	//执行mysql_query()语句2
	...
	function usequery($query){
		global $elve_config;
		$this->sql=mysql_query($query,$GLOBALS['link']) or die($elve_config['db']['showerror']==1?mysql_error().'<br>'.str_replace($GLOBALS['dbtbpre'],'***_',$query):'DbError');
		if($GLOBALS['linkrd'])
		{
