...
if($tagid)
{
	$tagr=$Elves->fetch1("select tagname,num from {$dbtbpre}melvetags where tagid='$tagid'");
	if(!$tagr['tagname'])
	{
...
else
{
	...
	$tagr=$Elves->fetch1("select tagid,num from {$dbtbpre}melvetags where tagname='$tagname' limit 1");
	if(!$tagr['tagid'])
	{
		printerror('HaveNotTags','',1);
	}
...
if(!empty($add))
{
	...
	if($totalnum<1)
	{
		$totalquery="select count(*) as total from {$dbtbpre}melvetagsdata where tagid='$tagid'".$add;
...
$query="select classid,id from {$dbtbpre}melvetagsdata where tagid='$tagid'".$add;
