...
function AddVote($voteid,$vote){
	...
	$r=$Elves->fetch1("select voteid,voteip,votetext,voteclass,doip,dotime from {$dbtbpre}melvevote where voteid='$voteid'");
	if(empty($r['voteid'])||empty($r['votetext']))
	{
		printerror("NotVote","history.go(-1)",1);
	}
	$re=DoVote($r,$vote);
	$sql=$Elves->query("update {$dbtbpre}melvevote set votetext='".addslashes($re['votetext'])."',voteip='$re[voteip]',votenum=votenum+".$re['votetotal']." where voteid='$voteid'");
	if($sql)
	{
...
function AddInfoVote($classid,$id,$vote){
	...
	$r=$Elves->fetch1("select id,voteip,votetext,voteclass,doip,dotime from {$dbtbpre}melveinfovote where pubid='$pubid' limit 1");
	if(empty($r['id'])||empty($r['votetext']))
	{
		printerror("NotVote","history.go(-1)",1);
	}
	$re=DoVote($r,$vote);
	$sql=$Elves->query("update {$dbtbpre}melveinfovote set votetext='".addslashes($re['votetext'])."',voteip='$re[voteip]',votenum=votenum+".$re['votetotal']." where pubid='$pubid' limit 1");
	if($sql)
	{
...
function AddInfoPfen($add){
	...
	//字段
	$fnum=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvef where tbname='".$class_r[$classid]['tbname']."' and (f='infopfen' or f='infopfennum')");
	if(empty($fnum))
	{
		$doajax==1?ajax_printerror('','','ErrorUrl',1):printerror('ErrorUrl','',1);
	}
	...
	$sql=$Elves->query("update {$dbtbpre}elve_".$class_r[$classid]['tbname']." set infopfen=infopfen+$fen,infopfennum=infopfennum+1 where id='$id' and classid='$classid' limit 1");
	if($sql)
	{
		esetcookie('lastforfenid',$classid.'n'.$id,time()+30*24*3600);	//最后发布
		if($doajax==1)
		{
			$nr=$Elves->fetch1("select infopfen,infopfennum from {$dbtbpre}elve_".$class_r[$classid]['tbname']." where id='$id' and classid='$classid' limit 1");
