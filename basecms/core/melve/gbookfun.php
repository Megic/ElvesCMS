...
function AddGbook($add){
	...
	//版面是否存在
	$br=$Elves->fetch1("select bid,checked,groupid from {$dbtbpre}melvegbookclass where bid='$bid';");
	if(empty($br[bid]))
	{
	...
	$sql=$Elves->query("insert into {$dbtbpre}melvegbook(name,email,`mycall`,lytime,lytext,retext,bid,ip,checked,userid,username) values('$name','$email','$mycall','$lytime','$lytext','','$bid','$ip','$br[checked]','$userid','$username');");
	elveEmptyShowKey($keyvname);//清空验证码
	if($sql)
	{
		esetcookie("lastgbooktime",time(),time()+3600*24);//设置最后发表时间
		$reurl=DoingReturnUrl("../tool/gbook/?bid=$bid",$add['elvefrom']);
