...
function EditSafeInfo($add){
	...
	$num=0;
	$ur=$Elves->fetch1("select ".eReturnSelectMemberF('userid,password,salt')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid'");
	if(empty($ur['userid']))
	{
		printerror('FailOldPassword','',1);
	}
	...
	//邮箱
	$pr=$Elves->fetch1("select regemailonly from {$dbtbpre}melvepublic limit 1");
	if($pr['regemailonly'])
	{
		$num=$Elves->gettotal("select count(*) as total from ".eReturnMemberTable()." where ".egetmf('email')."='$email' and ".egetmf('userid')."<>'$userid' limit 1");
		if($num)
		{
	...
	$sql=$Elves->query("update ".eReturnMemberTable()." set ".egetmf('email')."='$email'".$a." where ".egetmf('userid')."='$userid'");
	if($sql)
    {
		//易通行系统
...
function EditInfo($post){
	...
	$user_r=$Elves->fetch1("select ".eReturnSelectMemberF('groupid')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid'");
	...
	if(empty($addr[userid]))
	{
		...
		$regip=egetip();
		$lasttime=time();
		$sql=$Elves->query("insert into {$dbtbpre}melvememberadd(userid,regip,lasttime,lastip,loginnum".$member_r[0].") values('$userid','$regip','$lasttime','$regip',1".$member_r[1].");");
    }
	else
	{
		$sql=$Elves->query("update {$dbtbpre}melvememberadd set userid='$userid'".$member_r[0]." where userid='$userid'");
    }
	//更新附件
