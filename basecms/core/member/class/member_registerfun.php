...
function register($add){
	...
	//重复用户
	$num=$Elves->gettotal("select count(*) as total from ".eReturnMemberTable()." where ".egetmf('username')."='$username' limit 1");
	if($num)
	{
	...
	if($pr['regemailonly'])
	{
		$num=$Elves->gettotal("select count(*) as total from ".eReturnMemberTable()." where ".egetmf('email')."='$email' limit 1");
		if($num)
		{
	...
	$sql=$Elves->query("insert into ".eReturnMemberTable()."(".eReturnInsertMemberF('username,password,rnd,email,registertime,groupid,userfen,userdate,money,zgroupid,havemsg,checked,salt,userkey').") values('$username','$password','$rnd','$email','$registertime','$groupid','$public_r[reggetfen]','0','0','0','0','$checked','$salt','$userkey');");
	//取得userid
	$userid=$Elves->lastid();
	//附加表
	...
	if($sql)
	{
		...
		if($checked==0)
		{
			$location=DoingReturnUrl("../../",$_POST['elvefrom']);
		...
		$location=DoingReturnUrl($location,$_POST['elvefrom']);
