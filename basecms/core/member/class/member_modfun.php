...
function DelYMemberTranFile($file,$tf,$username=''){
	...
	$count=count($r);
	$filename=$r[$count-1];
	$fr=$Elves->fetch1("select filename,path,fileid,fpath,classid from {$dbtbpre}melvefile_member where no='Member[".$tf."]' and filename='$filename' and adduser='[EditInfo]".$username."' limit 1");
	if($fr['fileid'])
	{
		$sql=$Elves->query("delete from {$dbtbpre}melvefile_member where fileid='$fr[fileid]'");
		DoDelFile($fr);
	}
}
...
//返回会员字段
function ReturnDoMemberF($fid,$add,$mr,$elve=0,$username='',$admin=0){
	global $Elves,$dbtbpre,$elve_config,$public_r;
	$pr=$Elves->fetch1("select openmembertranimg,memberimgsize,memberimgtype,openmembertranfile,memberfilesize,memberfiletype from {$dbtbpre}melvepublic limit 1");
	$formr=$Elves->fetch1("select fid,enter,mustenter,filef,imgf,canaddf,caneditf,checkboxf from {$dbtbpre}melvememberform where fid='$fid'");
	//检测必填字段
	...
	for($i=0;$i<$count-1;$i++)
	{
		...
		$f=$fr1[1];
		if($admin==0&&(($elve==0&&!strstr($formr['canaddf'],','.$f.','))||($elve==1&&!strstr($formr['caneditf'],','.$f.','))))
		{continue;}
		//附件
...
function DoDelMemberF($fid,$userid,$username){
	global $Elves,$dbtbpre;
	$r=$Elves->fetch1("select * from {$dbtbpre}melvememberadd where userid='$userid'");
	$sql=$Elves->query("delete from {$dbtbpre}melvememberadd where userid='$userid'");
	$sql2=$Elves->query("delete from {$dbtbpre}melvememberpub where userid='$userid'");
	if(empty($r[userid]))
	{
		return '';
	}
	$formr=$Elves->fetch1("select fid,filef,imgf from {$dbtbpre}melvememberform where fid='$fid'");
