...
function ChangeSpaceStyle($add){
	...
	$sr=$Elves->fetch1("select styleid,membergroup from {$dbtbpre}melvespacestyle where styleid='$styleid'");
	if(!$sr['styleid']||($sr['membergroup']&&!strstr($sr['membergroup'],','.$user_r[groupid].',')))
	{
		printerror('NotChangeSpaceStyleId','',1);
	}
	$sql=$Elves->query("update {$dbtbpre}melvememberadd set spacestyleid='$styleid' where userid='$user_r[userid]' limit 1");
	if($sql)
	{
...
function DoSetSpace($add){
	...
	$sql=$Elves->query("update {$dbtbpre}melvememberadd set spacename='$spacename',spacegg='$spacegg' where userid='$user_r[userid]' limit 1");
	if($sql)
	{
