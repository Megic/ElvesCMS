...
if($doaction=='dozt')
{
	...
	$n_r=$Elves->fetch1("select ztid,ztname,intro,ztimg,ztpagekey,restb from {$dbtbpre}melvezt where ztid='$classid'");
	if(!$n_r['ztid'])
	{
		printerror("ErrorUrl","history.go(-1)",1);
	}
...
else
{
	...
	$n_r=$Elves->fetch1("select * from {$dbtbpre}elve_".$class_r[$classid][tbname]." where id='$id' limit 1");
...
if($_GET['tempid'])
{
	...
	$tempnum=$Elves->gettotal("select count(*) as total from ".GetTemptb("melvepltemp")." where tempid='$tempid'");
...
$query="select * from {$dbtbpre}melvepl_".$n_r['restb']." where pubid='$pubid' and checked=0";
//总数量
if($doaction=='dozt')
{
	$totalquery="select count(*) as total from {$dbtbpre}melvepl_".$n_r['restb']." where pubid='$pubid' and checked=0";
...
@require(elve_PATH.'core/data/filecache/template/pl'.$tempid.'.php');
db_close();
$Elves=null;
?>