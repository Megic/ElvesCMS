...
function AddPl($username,$password,$nomember,$key,$saytext,$id,$classid,$repid,$add){
	global $Elves,$dbtbpre,$public_r,$class_r,$level_r;
	//验证本时间允许操作
	...
	else
	{
		if(empty($nomember))//非匿名
		{
			...
			$ur=$Elves->fetch1("select ".eReturnSelectMemberF('userid,salt,password,checked,groupid')." from ".eReturnMemberTable()." where ".egetmf('username')."='$username' limit 1");
			if(empty($ur['userid']))
			{
				printerror("FailPassword","history.go(-1)",1);
			}
	...
	if($doaction=='dozt')
	{
		...
		//是否关闭评论
		$r=$Elves->fetch1("select ztid,closepl,checkpl,restb from {$dbtbpre}melvezt where ztid='$classid'");
		if(!$r['ztid'])
		{
	...
	else//信息
	{
		...
		//是否关闭评论
		$r=$Elves->fetch1("select classid,stb,restb from {$dbtbpre}elve_".$class_r[$classid][tbname]." where id='$id' limit 1");
		...
		$finfor=$Elves->fetch1("select closepl from {$dbtbpre}elve_".$class_r[$classid][tbname]."_data_".$r['stb']." where id='$id' limit 1");
		if($finfor['closepl'])
		{
	...
	//设置参数
	$plsetr=$Elves->fetch1("select pltime,plsize,plincludesize,plclosewords,plmustf,plf,plmaxfloor,plquotetemp from {$dbtbpre}melvepl_set limit 1");
	...
	//主表
	$sql=$Elves->query("insert into {$dbtbpre}melvepl_".$restb."(pubid,username,sayip,saytime,id,classid,checked,zcnum,fdnum,userid,isgood,saytext".$ret_r['fields'].") values('$pubid','".$username."','$sayip','$saytime','$id','$classid','$checked',0,0,'$muserid',0,'".addslashes($saytext)."'".$ret_r['values'].");");
	$plid=$Elves->lastid();
	if($doaction!='dozt')
	{
		//信息表加1
		$usql=$Elves->query("update {$dbtbpre}elve_".$class_r[$classid][tbname]." set plnum=plnum+1 where id='$id' limit 1");
	}
	//更新新评论数
	...
	if($sql)
	{
		$reurl=DoingReturnUrl($returl,$_POST['elvefrom']);
...
function RepPlTextQuote($repid,$saytext,$pr,$restb){
	global $public_r,$Elves,$dbtbpre,$fun_r;
	$quotetemp=stripSlashes($pr['plquotetemp']);
	$r=$Elves->fetch1("select userid,username,saytime,saytext from {$dbtbpre}melvepl_".$restb." where plid='$repid'");
	if(empty($r['username']))
	{
...
//返回字段
function ReturnPlAddF($add,$pr,$elve=0){
	global $Elves,$dbtbpre;
	$fr=explode(',',$pr['plf']);
	$count=count($fr)-1;
	$ret_r['fields']='';
	...
	for($i=1;$i<$count;$i++)
	{
		...
		if(strstr($pr[plmustf],','.$f.','))
		{
			if(!trim($fval))
			{
				$chfr=$Elves->fetch1("select fname from {$dbtbpre}melveplf where f='$f' limit 1");
...
function DoForPl($add){
	...
	if($doaction=='dozt')
	{
		...
		$infor=$Elves->fetch1("select ztid,restb from {$dbtbpre}melvezt where ztid='$classid'");
		if(!$infor['ztid'])
		{
			$doajax==1?ajax_printerror('','','ErrorUrl',1):printerror('ErrorUrl','',1);
		}
	...
	else//信息
	{
		...
		$infor=$Elves->fetch1("select classid,restb from {$dbtbpre}elve_".$class_r[$classid][tbname]." where id='$id' limit 1");
		if(!$infor['classid'])
		{
			$doajax==1?ajax_printerror('','','ErrorUrl',1):printerror('ErrorUrl','',1);
		}
	...
	$sql=$Elves->query("update {$dbtbpre}melvepl_".$infor['restb']." set ".$f."=".$f."+1 where plid='$plid' and pubid='$pubid'");
	if($sql)
	{
		esetcookie('lastforplid'.$plid,$plid,time()+30*24*3600);	//最后发布
		if($doajax==1)
		{
			$nr=$Elves->fetch1("select ".$f." from {$dbtbpre}melvepl_".$infor['restb']." where plid='$plid' and pubid='$pubid'");
