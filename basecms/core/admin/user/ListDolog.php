...
require "../".LoadLang("pub/melve.php");
...
function DelDoLog($logid,$userid,$username){
	...
	$sql=$Elves->query("delete from {$dbtbpre}melvedolog where logid='$logid'");
	if($sql)
	{
		//操作日志
...
function DelDoLog_all($logid,$userid,$username){
	...
	$sql=$Elves->query("delete from {$dbtbpre}melvedolog where".$add);
	if($sql)
	{
		//操作日志
		insert_dolog("");
...
function DelDoLog_date($add,$userid,$username){
	...
	$endday=$end.' 23:59:59';
	$sql=$Elves->query("delete from {$dbtbpre}melvedolog where logtime<='$endday' and logtime>='$startday'");
	if($sql)
	{
		//操作日志
...
//批量删除日志
elseif($melve=="DelDoLog_all")
{
	$logid=$_POST['logid'];
	DelDoLog_all($logid,$logininid,$loginin);
}
elseif($melve=="DelDoLog_date")
{
...
$query="select logid,logip,logtime,username,melve,doing from {$dbtbpre}melvedolog".$where;
$totalquery="select count(*) as total from {$dbtbpre}melvedolog".$where;
...
<form name="form2" method="post" action="ListDolog.php" onsubmit="return confirm('确认要删除?');">
  ...
  ?>
    <tr bgcolor="#DBEAF5" id=log<?=$r[logid]?>> 
      ...
      <td height="25"> 
        <div align="center">[<a href=ListDolog.php?melve=DelDoLog&logid=<?=$r[logid]?> onclick="return confirm('确认要删除此日志?');">删除</a> 
    ...
    <tr bgcolor="#FFFFFF"> 
      <td height="25" colspan="5"><table width="100%" border="0" cellspacing="1" cellpadding="3">
          <tr>
            <td width="9%">动作：</td>
            <td width="25%"><?=$melve_r[$r[melve]]?></td>
            <td width="10%">操作对像：</td>
  ...
  ?>
    <tr bgcolor="#FFFFFF"> 
      <td height="25" colspan="5"> 
        <?=$returnpage?>&nbsp;&nbsp;
        <input type="submit" name="Submit" value="批量删除"> <input name="melve" type="hidden" id="mgxe" value="DelDoLog_all">&nbsp;
...
<form action="ListDolog.php" method="post" name="dellogform" id="dellogform" onsubmit="return confirm('确认要删除?');">
  <table width="100%" border="0" cellspacing="1" cellpadding="3">
    <tr> 
      <td><div align="center">
          <input name="melve" type="hidden" id="melve" value="DelDoLog_date">
          删除从 
