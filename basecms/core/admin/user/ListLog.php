...
function DelLog($loginid,$userid,$username){
	...
	$sql=$Elves->query("delete from {$dbtbpre}melvelog where loginid='$loginid'");
	if($sql)
	{
		//操作日志
...
function DelLog_all($loginid,$userid,$username){
	...
	$sql=$Elves->query("delete from {$dbtbpre}melvelog where".$add);
	if($sql)
	{
		//操作日志
		insert_dolog("");
...
function DelLog_date($add,$userid,$username){
	...
	$endday=$end.' 23:59:59';
	$sql=$Elves->query("delete from {$dbtbpre}melvelog where logintime<='$endday' and logintime>='$startday'");
	if($sql)
	{
		//操作日志
...
//批量删除日志
elseif($melve=="DelLog_all")
{
	$loginid=$_POST['loginid'];
	DelLog_all($loginid,$logininid,$loginin);
}
elseif($melve=="DelLog_date")
{
...
$query="select loginid,username,loginip,logintime,status,password,loginauth from {$dbtbpre}melvelog";
$totalquery="select count(*) as total from {$dbtbpre}melvelog";
//搜索
$search='';
$where='';
if($_GET['sear']==1)
...
<form name="form2" method="post" action="ListLog.php" onsubmit="return confirm('确认要删除?');">
  ...
  ?>
    <tr bgcolor="#FFFFFF" id=log<?=$r[loginid]?>>
      ...
      <td height="25"><div align="center">[<a href=ListLog.php?melve=DelLog&loginid=<?=$r[loginid]?> onclick="return confirm('确认要删除此日志?');">删除</a> 
  ...
  ?>
    <tr bgcolor="#FFFFFF"> 
      <td height="25" colspan="6"> 
        <?=$returnpage?>
        &nbsp;&nbsp; <input type="submit" name="Submit" value="批量删除"> <input name="melve" type="hidden" id="mgxe" value="DelLog_all"> 
...
<form action="ListLog.php" method="post" name="dellogform" id="dellogform" onsubmit="return confirm('确认要删除?');">
  <table width="100%" border="0" cellspacing="1" cellpadding="3">
    <tr> 
      <td><div align="center">
          <input name="melve" type="hidden" id="melve" value="DelLog_date">
          删除从 
