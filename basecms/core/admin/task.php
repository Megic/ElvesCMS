...
$tasksql="select id,filename,lastdo,doweek,doday,dohour,dominute,userid,taskname from {$dbtbpre}melvetask where isopen=1 and (userid=0 or (userid<>0 and userid='$logininid'))";
...
if($elve=='dotask')
{
	...
	$r=$Elves->fetch1("select id,filename,lastdo,doweek,doday,dohour,dominute,userid,taskname from {$dbtbpre}melvetask where id=$id and isopen=1 limit 1");
	...
	if($lasttime)
	{
		...
		require_once($file);
		$usql=$Elves->query("update {$dbtbpre}melvetask set lastdo='$lasttime' where id=$id");
...
elseif($elve=='retask')
{
	echo"<meta http-equiv=\"refresh\" content=\"".$retasktime.";url=task.php?elve=retask\">";
	?>
	<script>
	function AddTaskDiv(divid,taskid){
		if(parent.document.getElementById(divid)!=null)
		{
			eval("parent."+divid+".location.href='task.php?elve=dotask&id="+taskid+"';");
		}
		else
		{
			parent.document.getElementById("alltaskdiv").innerHTML+="<div id='"+divid+"'><iframe frameborder='0' id='"+divid+"' scrolling='no' src='task.php?elve=dotask&id="+taskid+"' style='HEIGHT:0;VISIBILITY:inherit;WIDTH:0;Z-INDEX:1'></iframe></div>";
		}
	}
	</script>
	<script>
	<?php
	$sql=$Elves->query($tasksql);
...
elseif($elve=='TodoTask')
{
	...
	$r=$Elves->fetch1("select id,filename,taskname from {$dbtbpre}melvetask where id=$id limit 1");
	$file='../tasks/'.$r['filename'];
...
else
{
	...
	<IFRAME frameBorder="0" id="retaskfile" name="retaskfile" scrolling="no" src="task.php?elve=retask" style="HEIGHT:0;VISIBILITY:inherit;WIDTH:0;Z-INDEX:1"></IFRAME>
	<table width='100%' border=0 align=center cellpadding=3 cellspacing=1 class='tableborder'>
		<form name="tasklogform" method="post">
		<tr class=header><td>任务日志&nbsp;&nbsp;&nbsp;[<a href="#elve" onclick="ClearTaskLog();">清除日志内容</a>]</td></tr>
