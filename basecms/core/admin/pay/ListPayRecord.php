...
function DelPayRecord_all($id,$userid,$username){
	...
	$sql=$Elves->query("delete from {$dbtbpre}melvepayrecord where".$add);
	if($sql)
	{
		//操作日志
		insert_dolog("");
...
$query="select id,userid,username,orderid,money,posttime,paybz,type,payip from {$dbtbpre}melvepayrecord";
$totalquery="select count(*) as total from {$dbtbpre}melvepayrecord";
//搜索
$search='';
$where='';
if($_GET['sear']==1)
...
<form name="form2" method="post" action="ListPayRecord.php" onsubmit="return confirm('确认要删除?');">
  ...
  while($r=$Elves->fetch($sql))
  {
  	if($r['userid'])
	{
		$username="<a href='../member/AddMember.php?melve=EditMember&userid=$r[userid]'>$r[username]</a>";
	}
	else
	{
  ...
  ?>
    <tr bgcolor="#FFFFFF"> 
      <td height="25" colspan="8">&nbsp;
        <?=$returnpage?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <input type="submit" name="Submit" value="批量删除"> <input name="melve" type="hidden" id="melve" value="DelPayRecord_all"></td>
    </tr>
  </table>
</form>
<?
