...
{
	$j=0;
	$tsql=$Elves->query("select tbname from {$dbtbpre}melvetable where intb=0 order by tid");
...
$url="../elvechtml.php?melve=RmelveHtml&classid=$add[classid]&from=$add[from]&retype=$add[retype]&startday=$add[startday]&endday=$add[endday]&startid=$add[startid]&endid=$add[endid]&havehtml=$havehtml&reallinfotime=".time();
