...
function Ebak_Rep($tablename,$dbname,$userid,$username){
	...
	$Elves->usequery("use `$dbname`");
	$count=count($tablename);
	if(empty($count))
	{printerror("MustChangeOneTable","history.go(-1)");}
	for($i=0;$i<$count;$i++)
	{
		//$sql1=$Elves->query("OPTIMIZE TABLE `$tablename[$i]`;");
		//$sql2=$Elves->query("CHECK TABLE `$tablename[$i]`;");
		//$sql3=$Elves->query("ANALYZE TABLE `$tablename[$i]`;");
		$sql4=$Elves->query("REPAIR TABLE `$tablename[$i]`;");
    }
	$Elves->usequery("use `".$elve_config['db']['dbname']."`");
	//操作日志
...
function Ebak_Drop($tablename,$dbname,$userid,$username){
	...
	$sql1=$Elves->query("DROP TABLE IF EXISTS ".$a.";");
	$Elves->usequery("use `".$elve_config['db']['dbname']."`");
	//操作日志
	insert_dolog("dbname=".$dbname);
...
function Ebak_DropDb($dbname,$userid,$username){
	...
	$sql=$Elves->query("DROP DATABASE `$dbname`");
	if($sql)
	{
		//操作日志
		insert_dolog("dbname=".$dbname);
...
function Ebak_CreatDb($dbname,$dbchar,$userid,$username){
	...
	$a="";
	if($dbchar&&$elve_config['db']['dbver']>='4.1'){
		$a=" DEFAULT CHARACTER SET ".$dbchar;
	}
	$sql=$Elves->query("CREATE DATABASE IF NOT EXISTS `$dbname`".$a);
	if($sql){
		//操作日志
		insert_dolog("dbname=".$dbname);
...
function Ebak_EmptyTable($tablename,$dbname,$userid,$username){
	...
	for($i=0;$i<$count;$i++)
	{
		$sql1=$Elves->query("TRUNCATE `".$tablename[$i]."`;");
    }
	$Elves->usequery("use `".$elve_config['db']['dbname']."`");
	//操作日志
	insert_dolog("dbname=".$dbname);
...
function Ebak_DoEbak($add,$userid,$username){
	global $Elves,$public_r,$fun_r,$elve_config;
	//验证权限
	CheckLevel($userid,$username,$classid,"dbdata");
	...
	if($elve_config['db']['dbver']=='4.0'&&$add['dbchar']=='auto')
	{
		$add['dbchar']='';
	}
	$string="<?php
	...
	echo $fun_r['FirstBakSuccess']."<script>self.location.href='mgxe.php?mgxe=$mgxe&t=0&s=0&p=0&mypath=$add[mypath]&waitbaktime=$waitbaktime';</script>";
	exit();
}

//执行备份(按文件大小)
function Ebak_BakExe($t,$s,$p,$mypath,$alltotal,$thenof,$fnum,$stime=0,$userid,$username){
	global $Elves,$public_r,$fun_r;
	//验证权限
	CheckLevel($userid,$username,$classid,"dbdata");
	if(empty($mypath)){
...
?>";
	...
	$sql=$Elves->query("select * from `".$btb[$t]."` limit $s,$num");
	//取得字段数
	if(empty($fnum))
	{
	...
	while($r=$Elves->fetch($sql))
	{
		...
		if(strlen($dumpsql)>=$b_filesize*1024)
		{
			...
			$Elves->free($sql);
			//echo $fun_r['BakOneDataSuccess'].Ebak_EchoBakSt($btb[$t],$count,$t,$num,$s)."<script>self.location.href='mgxe.php?mgxe=BakExe&s=$s&p=$p&t=$t&mypath=$mypath&alltotal=$num&thenof=$noautof&fieldnum=$fieldnum&stime=$stime';</script>";

			echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=mgxe.php?mgxe=BakExe&s=$s&p=$p&t=$t&mypath=$mypath&alltotal=$num&thenof=$noautof&fieldnum=$fieldnum&stime=$stime&waitbaktime=$waitbaktime&collation=$collation\">".$fun_r['BakOneDataSuccess'].Ebak_EchoBakSt($btb[$t],$count,$t,$num,$s);
			exit();
		}
	}
	//最后一个备份
	...
	$Elves->free($sql);
	//进入下一个表
	//echo $btb[$t-1].$fun_r['OneTableBakSuccess']."<script>self.location.href='mgxe.php?mgxe=BakExe&s=0&p=0&t=$t&mypath=$mypath&stime=$stime';</script>";

	echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=mgxe.php?mgxe=BakExe&s=0&p=0&t=$t&mypath=$mypath&stime=$stime&waitbaktime=$waitbaktime\">".$btb[$t-1].$fun_r['OneTableBakSuccess'];
	exit();
}
...
?>";
	...
	if($b_autoauf==1&&$auf)
	{
		$sql=$Elves->query("select * from `".$btb[$t]."` where ".$auf.">".$aufval." order by ".$auf." limit $b_bakline");
	}
	else
	{
		$sql=$Elves->query("select * from `".$btb[$t]."` limit $s,$b_bakline");
	}
	//完整插入
	$inf='';
	if($b_beover==1)
	...
	if(empty($b))
	{
		...
		$Elves->free($sql);
		//进入下一个表
		//echo $btb[$t-1].$fun_r['OneTableBakSuccess']."<script>self.location.href='mgxe.php?mgxe=BakExeT&s=0&p=0&t=$t&mypath=$mypath&stime=$stime';</script>";

		echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=mgxe.php?mgxe=BakExeT&s=0&p=0&t=$t&mypath=$mypath&stime=$stime&waitbaktime=$waitbaktime\">".$btb[$t-1].$fun_r['OneTableBakSuccess'];
		exit();
	}
	//进入下一组
	$p++;
	...
	WriteFiletext_n($sfile,$dumpsql);
	$Elves->free($sql);
	//echo $fun_r['BakOneDataSuccess'].Ebak_EchoBakSt($btb[$t],$count,$t,$num,$s)."<script>self.location.href='mgxe.php?mgxe=BakExeT&s=$s&p=$p&t=$t&mypath=$mypath&alltotal=$num&thenof=$noautof&fieldnum=$fieldnum&auf=$auf&aufval=$lastaufval&stime=$stime';</script>";

	echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=mgxe.php?mgxe=BakExeT&s=$s&p=$p&t=$t&mypath=$mypath&alltotal=$num&thenof=$noautof&fieldnum=$fieldnum&auf=$auf&aufval=$lastaufval&stime=$stime&waitbaktime=$waitbaktime&collation=$collation\">".$fun_r['BakOneDataSuccess'].Ebak_EchoBakSt($btb[$t],$count,$t,$num,$s);
	exit();
}
...
function Ebak_GetTotal($dbname,$tbname){
	global $Elves;
	/*
	$tr=$Elves->fetch1("select count(*) AS total from ".$btb[$t]);
	$num=$tr[total];
	*/
	$tr=$Elves->fetch1("SHOW TABLE STATUS LIKE '".$tbname."';");
	return $tr;
}
...
function Ebak_GetSetChar($char){
	...
	$r=$Elves->fetch1("SHOW COLLATION LIKE '".$char."';");
	return $r['Charset'];
}
...
function Ebak_ReturnTbfield($dbname,$tbname,$autofield){
	global $Elves;
	$sql=$Elves->query("SHOW FIELDS FROM `".$tbname."`");
	$i=0;//字段数
	$autof=",";//去除自增字段列表
	$f='';//自增字段名
	while($r=$Elves->fetch($sql))
...
function E_C($sql){
	global $Elves;
	$Elves->query(Ebak_AddDbchar($sql));
}
...
function Ebak_Returnstru($table,$strufour){
	...
	//设置引号
	$usql=$Elves->query("SET SQL_QUOTE_SHOW_CREATE=1;");
	//数据表结构
	$r=$Elves->fetch1("SHOW CREATE TABLE `$table`;");
...
function Ebak_ReplaceFieldChar($sql){
	global $elve_config;
	if($elve_config['db']['dbver']=='4.0'&&strstr($sql,' character set '))
	{
...
function Ebak_AddDbchar($sql){
	global $elve_config,$b_dbchar;
	//加编码
	if($elve_config['db']['dbver']>='4.1'&&!strstr($sql,'ENGINE=')&&($elve_config['db']['setchar']||$b_dbchar)&&$b_dbchar!='auto')
	{
		$dbcharset=$b_dbchar?$b_dbchar:$elve_config['db']['setchar'];
		$sql=Ebak_DoCreateTable($sql,$elve_config['db']['dbver'],$dbcharset);
	}
	elseif($elve_config['db']['dbver']=='4.0'&&strstr($sql,'ENGINE='))
	{
...
/*
		SoftName : ElvesBak
		Author   : wm_chief
		Copyright: Powered by www.webelves.org
*/
