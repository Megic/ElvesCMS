...
if($_GET['showretitle']==1)
{
	...
	$srtid="";
	$first=1;
	$srtsql=$Elves->query("select id,title from ".$infotb." where classid='$classid' group by title having(count(*))>1");
	...
	{
		printerror("HaveNotReInfo","ListNews.php?bclassid=$bclassid&classid=$classid".$addelvecheck);
	}
}
//只能编辑自己的信息
...
//导入页面
$deftempfile=elve_PATH.'core/data/html/list/listinfo.php';
if($infomod_r[listfile])
{
	$tempfile=elve_PATH.'core/data/html/list/'.$infomod_r[listfile].'.php';
