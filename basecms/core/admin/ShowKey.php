...
//显示验证码
function ShowKey($v){
	$vname=elveReturnKeyVarname($v);
	$key=strtolower(domake_password(4));
	elveSetShowKey($vname,$key,0);
	//是否支持gd库
	...
	{
		elveSetShowKey($vname,'elve',0);
		echo ReadFiletext("../data/images/elve.jpg");
	}
}

//返回变量名
function elveReturnKeyVarname($v){
	$name='checkkey';
	return $name;
}
