...
$infomod_r=$Elves->fetch1("select enter,tbname,sonclass,listfile from {$dbtbpre}melvemod where mid=".$modid);
...
//导航
$url=AdminReturnClassLink($classid).'&nbsp;>&nbsp;管理归档&nbsp;&nbsp;(<a href="AddNews.php?melve=AddNews&bclassid='.$bclassid.'&classid='.$classid.$addelvecheck.'">增加信息</a>)';
$start=0;
$page=(int)$_GET['page'];
...
{
	$singletablenum=$Elves->gettotal("select count(*) as total from {$dbtbpre}melveclass where tbname='".$class_r[$classid][tbname]."' and islast=1");
...
//统计
$totalquery="select count(*) as total from {$dbtbpre}elve_".$class_r[$classid][tbname]."_doc".$ewhere;
...
{
	$thisclassr=$Elves->fetch1("select listorder from {$dbtbpre}melveclass where classid='$classid'");
...
//标题分类
$tts='';
$ttsql=$Elves->query("select typeid,tname from {$dbtbpre}melveinfotype where mid='$modid' order by myorder");
...
//导入页面
$deftempfile=elve_PATH.'core/data/html/list/doclistinfo.php';
if($infomod_r[listfile])
{
	$tempfile=elve_PATH.'core/data/html/list/doc'.$infomod_r[listfile].'.php';
