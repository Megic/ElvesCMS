...
function DoCropImage($add,$userid,$username){
	global $Elves,$dbtbpre,$public_r,$class_r,$elve_config,$efileftp_fr,$efileftp_dr;
	//参数处理
	$pic_x=(int)$add['pic_x'];
	$pic_y=(int)$add['pic_y'];
	$pic_w=(int)$add['pic_w'];
	...
	$filer=$Elves->fetch1("select fileid,path,filename,classid,fpath,no from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid'");
	...
	$big_image_name=elve_PATH.$fspath['filepath'].$path.$filer['filename'];
	...
	$new_savepath=elve_PATH.$new_fspath['filepath'].$new_path;
	...
	if(file_exists($insert_file))
	{
		if(!$doing)
		{
			$Elves->query("delete from ".eReturnFileTable($modtype,$fstb)." where fileid='$fileid'");
