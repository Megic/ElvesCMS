...
$elve=ehtmlspecialchars($_GET['elve']);
if($elve=="pic")
{
	$title="插入图片";
}
...
function SetPicOk(elve){
	...
	imgstr="<div style='padding:6px'><fieldset><legend>"+picsay+"</legend><table cellpadding=0 cellspacing=0 border=0><tr><td><img src='<?=$public_r[newsurl]?>core/data/images/downfile.jpg' alt='文件类型: <?=$filetype?>' border=0 style='vertical-align:baseline'></td><td><a href='<?=$filename?>' title='"+picsay+"' target='_blank'><?=$fname?></a>&nbsp;(<?=$filesize?>)</td></tr></table></fieldset></div>";
	if(elve==0)
	{
		window.viewpic.innerHTML=imgstr;
	}
