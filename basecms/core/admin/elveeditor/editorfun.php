<?php
//提示信息
function elve_EditorPrintError($errorNumber,$fileUrl,$fileName,$customMsg,$fileno,$filesize){
...
function eTranMorePic($file,$file_name,$file_type,$file_size,$add,$userid,$username){
	...
	else
	{
		...
		if($count==0)
		{
			$tranfrom==0?printerror("MustChangeTranOneFile","history.go(-1)",8):elve_EditorPrintError(1,'','','MustChangeTranOneFile','','');
	    }
		$GLOBALS['doetran']=1;
		for($i=0;$i<$count;$i++)
		{
...
function TranFile($file,$file_name,$file_type,$file_size,$tranurl,$no,$classid,$type,$post,$userid,$username){
	global $Elves,$public_r,$loginrnd,$dbtbpre,$elve_config;
	if(!$no)
	{
		$no=$file_name;
	}
	...
	if(!$file_name)
	{
		...
		{
			$tranfrom==0?printerror("EmptyHttp","history.go(-1)",8):elve_EditorPrintError(1,'','','EmptyHttp','','');
		}
	...
	if(CheckSaveTranFiletype($filetype))
	{
		$tranfrom==0?printerror("TranPHP","history.go(-1)",8):elve_EditorPrintError(1,'','','TranPHP','','');
	}
	$type_r=explode("|".$filetype."|",$public_r['filetype']);
	if(count($type_r)<2)
	{
		$tranfrom==0?printerror("TranFiletypeFail","history.go(-1)",8):elve_EditorPrintError(1,'','','TranFiletypeFail','','');
	}
	if($file_size>$public_r['filesize']*1024)
	{
		$tranfrom==0?printerror("TranFilesizeFail","history.go(-1)",8):elve_EditorPrintError(1,'','','TranFilesizeFail','','');
	}
	if($type==1)//上传图片
	{
		if(!strstr($elve_config['sets']['tranpicturetype'],','.$filetype.','))
		{
			$tranfrom==0?printerror("NotTranImg","history.go(-1)",8):elve_EditorPrintError(1,'','','NotTranImg','','');
		}
	}
	elseif($type==2)//上传flash
	{
		...
		{
			$tranfrom==0?printerror("NotTranFlash","history.go(-1)",8):elve_EditorPrintError(1,'','','NotTranFlash','','');
		}
	}
	elseif($type==3)//上传多媒体
	{}
	...
	if(empty($file_name))
	{
		...
		if(empty($r[tran]))
		{
			$tranfrom==0?printerror("TranHttpFail","history.go(-1)",8):elve_EditorPrintError(1,'','','TranHttpFail','','');
		}
	}
	//本地上传
	else
	{
		...
		if(empty($r[tran]))
		{
			$tranfrom==0?printerror("TranFail","history.go(-1)",8):elve_EditorPrintError(1,'','','TranFail','','');
		}
	}
	if(!$no)
	{
	...
	if($sql)
	{
		if($tranfrom==1)//编辑器上传
		{
			...
			elve_EditorPrintError(0,$r[url],$r[filename],'',$no,$r[filesize]);
			db_close();
			exit();
		}
	...
	{
		$tranfrom==0?printerror("InTranRecordFail","history.go(-1)",8):elve_EditorPrintError(1,'','','InTranRecordFail','','');
	}
}
