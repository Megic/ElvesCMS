...
$changetbs='';
$havetb=0;
$tbsql=$Elves->query("select tid,tbname,tname from {$dbtbpre}melvetable order by tid");
...
//取得相应的信息
$user_r=$Elves->fetch1("select groupid,adminclass from {$dbtbpre}melveuser where userid='$logininid'");
//取得用户组
$gr=$Elves->fetch1("select doall,doselfinfo from {$dbtbpre}melvegroup where groupid='$user_r[groupid]'");
//管理员
$where='';
$and='';
$ewhere='';
...
$url="<a href=ListAllInfo.php?tbname=".$tbname.$addelvecheck.">管理信息</a>";
$start=0;
$page=(int)$_GET['page'];
...
//模型
$infomod_r=$Elves->fetch1("select mid,listfile from {$dbtbpre}melvemod where mid='$modid'");
//标题分类
$ttid=(int)$_GET['ttid'];
if($ttid)
{
...
//标题分类
$tts='';
$ttsql=$Elves->query("select typeid,tname from {$dbtbpre}melveinfotype where mid='$infomod_r[mid]' order by myorder");
...
if($_GET['showretitle']==1)
{
	...
	$srtid="";
	$first=1;
	$srtsql=$Elves->query("select id,title from ".$infotb." group by title having(count(*))>1");
	while($srtr=$Elves->fetch($srtsql))
	{
		if($first==1)
		{
	...
	{
		printerror("HaveNotReInfo","ListAllInfo.php?tbname=".$tbname.$addelvecheck);
	}
}
//排序
...
//表信息数
$tbinfos=eGetTableRowNum("{$dbtbpre}elve_".$tbname);
$tbckinfos=eGetTableRowNum("{$dbtbpre}elve_".$tbname."_check");
//取得总条数
...
//导入页面
$deftempfile=elve_PATH.'core/data/html/list/alllistinfo.php';
if($infomod_r[listfile])
{
	$tempfile=elve_PATH.'core/data/html/list/all'.$infomod_r[listfile].'.php';
