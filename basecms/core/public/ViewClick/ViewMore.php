...
$r=$Elves->fetch1("select ".$selectf." from {$dbtbpre}elve_".$cr['tbname']." where id='$id' limit 1");
//统计浏览次数
if($_GET['addclick']==1)
{
	$Elves->query("update {$dbtbpre}elve_".$cr['tbname']." set onclick=onclick+1 where id='$id' limit 1");
}
$r['onclick']=$r['onclick']+1;
if($cr['checkpl'])
{
	$pubid=ReturnInfoPubid(0,$id,$cr['tid']);
	$pl=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvepl_".$r['restb']." where pubid='$pubid' and checked=0");
	$r['plnum']=$pl;
}
if($pfen)
{
