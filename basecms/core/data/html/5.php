...
<a onclick="window.open('elveeditor/FileMain.php?type=1&classid=<?=$classid?>&infoid=<?=$id?>&filepass=<?=$filepass?>&sinfo=1&doing=1&field=titlepic','','width=700,height=550,scrollbars=yes');" title="选择已上传的图片"><img src="../data/images/changeimg.gif" border="0" align="absbottom"></a> 
  ...
  <tr> 
    ...
    <td bgcolor=ffffff><select name="movietype" id="movietype"><option value="港台影视"<?=$r[movietype]=="港台影视"?' selected':''?>>港台影视</option><option value="海外影视"<?=$r[movietype]=="海外影视"?' selected':''?>>海外影视</option><option value="大陆影视"<?=$r[movietype]=="大陆影视"?' selected':''?>>大陆影视</option><option value="日韩影视"<?=$r[movietype]=="日韩影视"?' selected':''?>>日韩影视</option></select>，推荐等级：<select name="star" id="star"><option value="1"<?=$r[star]=="1"?' selected':''?>>1星</option><option value="2"<?=$r[star]=="2"||$elvefirstpost==1?' selected':''?>>2星</option><option value="3"<?=$r[star]=="3"?' selected':''?>>3星</option><option value="4"<?=$r[star]=="4"?' selected':''?>>4星</option><option value="5"<?=$r[star]=="5"?' selected':''?>>5星</option></select>，扣除点数：
...
<input name="playadmin" type="text" id="playadmin" value="<?=$elvefirstpost==1?"":ehtmlspecialchars(stripSlashes($r[playadmin]))?>" size="">
  ...
  <tr> 
    ...
    <td bgcolor=ffffff>影片格式：<input name="filetype" type="text" id="filetype" value="<?=$elvefirstpost==1?"":ehtmlspecialchars(stripSlashes($r[filetype]))?>" size="8">
<select name="select2" onchange="document.add.filetype.value=this.value">
      ...
      </select>，影片大小：<input name="filesize" type="text" id="filesize" value="<?=$elvefirstpost==1?"":ehtmlspecialchars(stripSlashes($r[filesize]))?>" size="12">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr> 
    ...
    <td bgcolor=ffffff><textarea name="moviesay" cols="80" rows="10" id="moviesay"><?=$elvefirstpost==1?"":ehtmlspecialchars(stripSlashes($r[moviesay]))?></textarea>
</td>
  </tr>
</table>
