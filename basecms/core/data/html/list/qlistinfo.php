...
$url="<a href='../../'>首页</a>&nbsp;>&nbsp;<a href='../member/cp/'>会员中心</a>&nbsp;>&nbsp;<a href='ListInfo.php?mid=$mid".$addelvecheck."'>管理信息</a>&nbsp;(".$mr[qmname].")";
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    ...
    <tr>
            <td width="25%" height="27"> 
              <input type="button" name="Submit" value="增加信息" onclick="self.location.href='ChangeClass.php?mid=<?=$mid?><?=$addelvecheck?>';">            </td>
...
<table width="100%" border="0" cellspacing="1" cellpadding="0">
  <tr>
    ...
    <td width="9%"<?=$indexchecked==0?' class="header"':' bgcolor="#C9F1FF"'?>><div align="center"><a href="ListInfo.php?mid=<?=$mid?>&elvecheck=1">待审核</a></div></td>
    <td width="23%">&nbsp;</td>
    <td width="47%">&nbsp;</td>
    <td width="6%">&nbsp;</td>
    <td width="6%">&nbsp;</td>
...
<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="tableborder">
  ...
  <?
	while($r=$Elves->fetch($sql))
	{
		...
		if($indexchecked==0)
		{
			...
			$titleurl='AddInfo.php?melve=MEditInfo&classid='.$r[classid].'&id='.$r[id].'&mid='.$mid.$addelvecheck;//链接
		}
		else
		{
			$checked='√';
  ...
  <tr bgcolor="#FFFFFF" id=news<?=$r[id]?>> 
    ...
    <td height="25"><div align="center"><a href="AddInfo.php?melve=MEditInfo&classid=<?=$r[classid]?>&id=<?=$r[id]?>&mid=<?=$mid?><?=$addelvecheck?>">修改</a> | <a href="elve.php?melve=MDelInfo&classid=<?=$r[classid]?>&id=<?=$r[id]?>&mid=<?=$mid?><?=$addelvecheck?>" onclick="return confirm('确认要删除?');">删除</a> 
      </div></td>
  </tr>
  <?
	}
