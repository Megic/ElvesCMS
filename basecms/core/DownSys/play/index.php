...
function ViewOnlineKFen($showdown_r,$u,$userid,$classid,$id,$pathid,$r){
	global $level_r,$class_r,$dbtbpre,$public_r,$Elves,$have_bak,$have_fen;
	if($showdown_r[2])
	{
		//下载次数限制
		$setuserday="";
...
if($downgroup)
{
	...
	{
		//---------是否有历史记录
			$bakr=$Elves->fetch1("select id,truetime from {$dbtbpre}melvedownrecord where id='$id' and classid='$classid' and userid='$user[userid]' and pathid='$pathid' and online=1 order by truetime desc limit 1");
...
$op=GetOnlinePass();
$url="../doaction.php?melve=GetSofturl&classid=$classid&id=$id&pathid=$pathid&pass=".$pass."&p=".$user[userid].":::".$user[rnd]."&onlinetime=".$op[0]."&onlinepass=".$op[1];
...
if(empty($r[playerid]))
{
	...
	elseif(strstr($elve_config['sets']['realplayertype'],','.$ftype.','))
	{
...
{
	$playerr=$Elves->fetch1("select filename from {$dbtbpre}melveplayer where id='$r[playerid]'");
	if($playerr['filename'])
	{
