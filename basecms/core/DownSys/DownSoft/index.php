...
$query="select * from {$dbtbpre}elve_".$tbname." where id='$id' limit 1";
$r=$Elves->fetch1($query);
...
if($downgroup)
{
	$user=islogin();
	//取得会员资料
	$u=$Elves->fetch1("select ".eReturnSelectMemberF('*')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$user[userid]' and ".egetmf('rnd')."='$user[rnd]' limit 1");
	if(empty($u['userid']))
	{
	...
	{
		//---------是否有历史记录
		$bakr=$Elves->fetch1("select id,truetime from {$dbtbpre}melvedownrecord where id='$id' and classid='$classid' and userid='$user[userid]' and pathid='$pathid' and online=0 order by truetime desc limit 1");
...
$titlepic=$r['titlepic']?$r['titlepic']:$public_r[newsurl]."core/data/images/notimg.gif";
$ip=egetip();
$pass=md5(ReturnDownSysCheckIp()."wm_chief".$public_r[downpass].$user[userid]);	//验证码
$url="../doaction.php?melve=DownSoft&classid=$classid&id=$id&pathid=$pathid&pass=".$pass."&p=".$user[userid].":::".$user[rnd];	//下载地址
