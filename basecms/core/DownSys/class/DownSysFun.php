...
function DownSoft($classid,$id,$pathid,$p,$pass){
	global $Elves,$dbtbpre,$public_r,$level_r,$class_r,$emod_r,$elve_config;
	//验证IP
	...
	if($downgroup)
	{
		...
		$rnd=RepPostVar($rnd);
		//取得会员资料
		$u=$Elves->fetch1("select ".eReturnSelectMemberF('*')." from ".eReturnMemberTable()." where ".egetmf('userid')."='$userid' and ".egetmf('rnd')."='$rnd' limit 1");
		if(empty($u['userid']))
		...
		{
			//---------是否有历史记录
			$bakr=$Elves->fetch1("select id,truetime from {$dbtbpre}melvedownrecord where id='$id' and classid='$classid' and userid='$userid' and pathid='$pathid' and online=0 order by truetime desc limit 1");
			...
			else
			{
				...
				else
				{
					...
					//去除点数
					$usql=$Elves->query("update ".eReturnMemberTable()." set ".egetmf('userfen')."=".egetmf('userfen')."-".$showdown_r[3]." where ".egetmf('userid')."='$userid'");
				}
				//备份下载记录
		...
		if($setuserday)
		{
			$usql=$Elves->query($setuserday);
		}
	}
	//总下载数据增一
    $usql=$Elves->query("update {$dbtbpre}elve_".$class_r[$classid][tbname]." set totaldown=totaldown+1 where id='$id'");
    $downurl=stripSlashes($showdown_r[1]);
	$downurlr=ReturnDownQzPath($downurl,$showdown_r[4]);
	$downurl=$downurlr['repath'];
	//防盗链
	@include(elve_PATH."core/DownSys/class/enpath.php");
...
function GetSofturl($classid,$id,$pathid,$p,$pass,$onlinetime,$onlinepass){
	global $Elves,$dbtbpre,$public_r,$class_r,$emod_r,$level_r,$elve_config;
	//验证IP
	...
	if($downgroup)
	{
		...
		{
			//---------是否有历史记录
		    $bakr=$Elves->fetch1("select id,truetime from {$dbtbpre}melvedownrecord where id='$id' and classid='$classid' and userid='$userid' and pathid='$pathid' and online=1 order by truetime desc limit 1");
			if($bakr[id]&&(time()-$bakr[truetime]<=$public_r[redodown]*3600))
			{}
			else
			{
	...
	if(strstr($elve_config['sets']['realplayertype'],','.$ftype.','))
	{
