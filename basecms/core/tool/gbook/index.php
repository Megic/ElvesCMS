...
//分类id
$bid=(int)$_GET['bid'];
$gbr=$Elves->fetch1("select bid,bname,groupid from {$dbtbpre}melvegbookclass where bid='$bid'");
if(empty($gbr['bid']))
{
...
{
	$totalquery="select count(*) as total from {$dbtbpre}melvegbook where bid='$bid' and checked=0";
	$num=$Elves->gettotal($totalquery);//取得总条数
}
$search.="&totalnum=$num";
$query="select lyid,name,email,`mycall`,lytime,lytext,retext from {$dbtbpre}melvegbook where bid='$bid' and checked=0";
