...
function MemberConnect_CheckApptype($apptype){
	...
	$appr=$Elves->fetch1("select * from {$dbtbpre}melvemember_connect_app where apptype='$apptype' and isclose=0 limit 1");
	if(!$appr['id'])
	{
		printerror2('请选择登录方式','../../../');
	}
...
function MemberConnect_CheckOpenid($apptype,$openid){
	...
	$mcr=$Elves->fetch1("select id,userid from {$dbtbpre}melvemember_connect where apptype='$apptype' and bindkey='$openid' limit 1");
	return $mcr;
}
...
function MemberConnect_DoLogin($apptype,$openid){
	...
	{
		$lifetime=0;
		$r=$Elves->fetch1("select ".eReturnSelectMemberF('*')." from ".eReturnMemberTable()." where ".egetmf('userid')."='".$mcr['userid']."' limit 1");
		DoelveMemberLogin($r,$lifetime);
...
function MemberConnect_UpdateBindLogin($id){
	...
	$id=(int)$id;
	$lasttime=time();
	$Elves->query("update {$dbtbpre}melvemember_connect set loginnum=loginnum+1,lasttime='$lasttime' where id='$id' limit 1");
}
...
function MemberConnect_InsertBind($apptype,$openid,$userid){
	...
	$Elves->query("insert into {$dbtbpre}melvemember_connect(userid,apptype,bindkey,bindtime,loginnum,lasttime) values('$userid','$apptype','$openid','$time',1,'$time');");
}

//验证是否绑定过
function MemberConnect_CheckReBind($apptype,$userid){
	global $Elves,$dbtbpre;
	$num=$Elves->gettotal("select count(*) as total from {$dbtbpre}melvemember_connect where userid='$userid' and apptype='$apptype' limit 1");
	if($num)
	{
...
function MemberConnect_DelBind($id){
	...
	$user_r=islogin();//是否登陆
	$id=(int)$id;
	$sql=$Elves->query("delete from {$dbtbpre}melvemember_connect where id='$id' and userid='$user_r[userid]';");
	if($sql)
	{
...
function MemberConnect_GetBindKey($apptype,$openid){
	global $elve_config;
	$checkpass=md5(md5('check-'.$apptype.'-Elvescms-'.$openid).'-#Elves.cms!-'.$openid.'-|-Elvescms-|-'.$elve_config['cks']['ckrndtwo']);
	return $checkpass;
}
...
function MemberConnect_CheckBindKey($apptype,$openid){
	global $elve_config;
	$pass=md5(md5('check-'.$apptype.'-Elvescms-'.$openid).'-#Elves.cms!-'.$openid.'-|-Elvescms-|-'.$elve_config['cks']['ckrndtwo']);
